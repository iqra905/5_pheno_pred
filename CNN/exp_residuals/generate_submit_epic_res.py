...
kernel_sizes = ["4096,1,1","2048,1,1","1024,1,1","512,1,1","256,1,1"]
stride = ["2048,1,1","1024,1,1","512,1,1","256,1,1","128,1,1"]
...
#executable    = epic_cnn_model_pros_res.sh
executable    = epic_cnn_model_pros_skip_res.sh

arguments = $(ID) $(bs) $(dropout) $(epochs) $(lr) $(peak_lr) $(final_lr) $(act) $(sch) $(df) $(opt) $(wd) $(ks) $(oc) $(st) $(fc) $(exp_dir) $(genotype_dir) $(phenotype_file) $(label_col)
...
log    = /vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/results/pros/residual/logs/c$(Cluster).$(Process).log
output = /vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/results/pros/residual/$(ID)/c$(Cluster).$(Process).out
error  = /vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/results/pros/residual/logs/c$(Cluster).$(Process).error
                        ...
                        for o in opt:
                            for w in wd:
                                for l, plr, flr in zip(lr,peak_lr,final_lr):
                                    for ks, oc, st, fc in zip(kernel_sizes, conv_channels, stride, fc_layers):
                                        experiment_count += 1
...
st = {st}
fc = {fc}
exp_dir = /vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/results/pros/residual/
...
with open("/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/epic_cnn_model_pros_skip_res.submit_file", "w") as f:
#with open("/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/exp_residuals/epic_cnn_model_pros_res.submit_file", "w") as f:
