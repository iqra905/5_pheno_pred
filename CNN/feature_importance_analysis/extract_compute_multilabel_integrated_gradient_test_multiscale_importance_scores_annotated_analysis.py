...
def parse_args():
    ...
    # Threshold methods
    parser.add_argument("-threshold_methods", type=str, default="std", help="Comma-separated threshold methods (choices:elbow,percentile,gap,std)")
    parser.add_argument("-percentile_threshold", type=float, default=99.0)
    parser.add_argument("-std_multiplier", type=float, default=5.0)
    ...
    # Manhattan plot customization
    parser.add_argument("-manhattan_top_labels", type=int, default=0, 
                       help="Number of top SNPs to label on Manhattan plot (default: 10, use 0 to disable)")
...
def format_entity_name(entity):
    """
    Format entity name for better display in titles
    Splits entity name and maps first part to proper disease name
    """
    ...
    first_part = entity.split('_')[0].lower()
    
    # Return mapped name if exists, otherwise capitalize first part
    if first_part in disease_map:
        return disease_map[first_part]
    else:
        # Fallback: capitalize first letter
        return first_part[0].upper() + first_part[1:] if first_part else entity
...
class ThresholdSelector:
    ...
    @staticmethod
    def std_method(scores, multiplier=5.0):
        mean = scores.mean()
        std = scores.std()
...
class AnnotatedImportanceVisualizer:
    ...
    def plot_manhattan(self, df_annotated, entity, threshold_results=None, 
                      top_n_labels=10):
        """
        Create clean Manhattan plot with alternating colors (reference-style)
        
        Parameters:
        -----------
        df_annotated : DataFrame with columns ['chromosome', 'bp', 'Importance_Score']
        entity : str, name of the entity (disease)
        threshold_results : dict, threshold results to plot as horizontal lines
        top_n_labels : int, number of top SNPs to label with values (default: 10)
        """
        ...
        # Define TWO alternating colors (standard Manhattan plot style)
        color1 = '#0C4C8A'  # Dark blue
        color2 = '#7F7F7F'  # Gray
        
        # Plot points with alternating colors - smaller points, more transparent
        for i, chrom in enumerate(chromosomes):
            chrom_data = df_plot[df_plot['chromosome'] == chrom]
            if len(chrom_data) > 0:
                ...
                ax.scatter(chrom_data['pos_cumulative'], 
                          ...
                          c=color, s=8, alpha=0.6, edgecolors='none', rasterized=True)
        
        # Add threshold line
        if threshold_results:
            # Show only the first threshold method
            method, result = list(threshold_results.items())[0]
            ax.axhline(result['threshold'], color='red',
                      linestyle='--', linewidth=2, alpha=0.9,
                      label=f"Threshold ({method}): {result['threshold']:.2e}",
                      zorder=10)
        
        # Label top N SNPs with their importance scores
        if top_n_labels > 0:
            # Get top N SNPs
            top_snps = df_plot.nlargest(top_n_labels, 'Importance_Score')
            
            for idx, row in top_snps.iterrows():
                # Add text label with value
                ax.annotate(f"{row['Importance_Score']:.2e}",
                           xy=(row['pos_cumulative'], row['Importance_Score']),
                           xytext=(0, 8), textcoords='offset points',
                           ha='center', fontsize=8, color='black', weight='bold',
                           bbox=dict(boxstyle='round,pad=0.4', 
                                   facecolor='yellow', alpha=0.8, edgecolor='orange', linewidth=1.5),
                           arrowprops=dict(arrowstyle='->', color='red', lw=1.2),
                           zorder=15)
        
        # Formatting
        ax.set_xlabel('Chromosome', fontsize=16, fontweight='bold')
        ax.set_ylabel('Importance Score', fontsize=16, fontweight='bold')
        
        # Format entity name for better title display
        formatted_entity = format_entity_name(entity)
        ax.set_title(f'{formatted_entity} - Manhattan Plot of SNP Importance', 
                    fontsize=18, fontweight='bold', pad=15)
        
        # Set x-axis labels at chromosome centers
        ax.set_xticks([chrom_centers[c] for c in chromosomes])
        ax.set_xticklabels(chromosomes, fontsize=13)
        
        # Set y-axis limits for better visualization
        # Calculate appropriate limits based on data
        y_max = df_plot['Importance_Score'].max()
        y_min = df_plot['Importance_Score'].min()
        
        # Use 0 or slightly below min as baseline for cleaner look
        if y_min < 1e-6:
            baseline = 0
        else:
            baseline = y_min * 0.8  # Start slightly below minimum
        
        y_range = y_max - baseline
        ax.set_ylim(baseline, y_max + 0.05 * y_range)
        
        # Y-axis formatting - use matplotlib's automatic scientific notation
        ...
        # Add very subtle vertical lines between chromosomes
        for i, chrom in enumerate(chromosomes[:-1]):
            chrom_end = chrom_centers[chrom] + chrom_lengths[chrom] / 2
            ax.axvline(chrom_end + 2.5e6, color='lightgray', linestyle='-', 
    ...
    if args.create_plots:
        ...
        if has_annotation:
            ...
            visualizer.plot_manhattan(full_df, entity, threshold_results, 
                                    top_n_labels=args.manhattan_top_labels)
...
def main():
    ...
    print("SNP FEATURE IMPORTANCE - ANNOTATED ANALYSIS (FIXED VERSION)")
    print("="*80)
