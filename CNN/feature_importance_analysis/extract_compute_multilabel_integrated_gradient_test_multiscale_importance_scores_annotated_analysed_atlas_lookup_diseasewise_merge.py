...
Input files:
- breacancer_all_snps_importance_integrated_gradients_disease_wise_test_set_std_filtered_altas_lookup_cancer_diabetes.csv
- crc_all_snps_importance_integrated_gradients_disease_wise_test_set_std_filtered_altas_lookup_cancer_diabetes.csv
- panca_all_snps_importance_integrated_gradients_disease_wise_test_set_std_filtered_altas_lookup_cancer_diabetes.csv
- pros01_all_snps_importance_integrated_gradients_disease_wise_test_set_std_filtered_altas_lookup_cancer_diabetes.csv
- t2dm_all_snps_importance_integrated_gradients_disease_wise_test_set_std_filtered_altas_lookup_cancer_diabetes.csv
"""
import pandas as pd
...
def main():
    ...
    # Configuration
    # =========================
    input_dir = Path("/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/5d_multilabel/multiscale/hardcoded/parallel/transformer/feat_imp_threshold/feat_imp/feat_imp_disease_wise_no_cov/ig_50/analysis_20260102_122403/filtered_snps")  
    output_file = input_dir / (
        "mega_all_snps_importance_integrated_gradients_test_set_std_filtered_altas_lookup_cancer_diabetes_deduplicated.csv"
    )
    ...
    exclude_cols = ['EA', 'NEA', 'Importance_Score', 'Rank','ref_allele', 'alt_allele']
    ...
    if num_duplicate_rows > 0:
        ...
        print(duplicate_examples[['rsid', 'atlas_id', 'trait', 'Importance_Score', 'EA', 'NEA']].to_string())
    ...
    # =========================
    # Sort final output by pval in ascending order
    # =========================
    dedup_df = dedup_df.sort_values('pval', ascending=True)
    print("\nFinal data sorted by pval (ascending order)")
