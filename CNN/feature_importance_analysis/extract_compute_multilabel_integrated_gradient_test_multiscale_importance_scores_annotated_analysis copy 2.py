...
class AnnotatedImportanceVisualizer:
    ...
    def plot_manhattan(self, df_annotated, entity, threshold_results=None, 
        ...
        # Y-axis formatting - force display in 1e-6 units
        from matplotlib.ticker import FuncFormatter
        
        def format_y_axis(value, pos):
            """Format y-axis to show values in 1e-6 units"""
            return f'{value*1e6:.1f}'
        
        ax.yaxis.set_major_formatter(FuncFormatter(format_y_axis))
        ax.tick_params(axis='y', labelsize=12)
        
        # Add text to indicate the scale
        ax.text(0.02, 0.98, '×10⁻⁶', transform=ax.transAxes, 
               fontsize=14, verticalalignment='top', 
               bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        
        # Set y-axis limits for better visualization
        # Start from 1e-6 to make the plot look fuller and threshold line more visible
        y_max = df_plot['Importance_Score'].max()
        y_min = 1e-6  # Fixed minimum at 1.0 × 10⁻⁶
        y_range = y_max - y_min
