...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-GPU Multilabel Genotype Model Training with Mamba")
    ...
    parser.add_argument("-bs", type=int, default=5, help="Batch size PER GPU for training")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            if world_size > 1:
                ...
                for disease in disease_labels:
                    # Convert to tensors
                    labels_tensor = torch.tensor(phase_labels[phase][disease], device=device)
                    preds_tensor = torch.tensor(phase_preds[phase][disease], device=device)
                    ...
                    # Gather
                    labels_gathered = [torch.zeros(max_size, device=device) for _ in range(world_size)]
                    preds_gathered = [torch.zeros(max_size, device=device) for _ in range(world_size)]
...
def main():
    ...
    # Validate parameters (only rank 0 prints)
    ...
    # Create model (all ranks create the same model)
