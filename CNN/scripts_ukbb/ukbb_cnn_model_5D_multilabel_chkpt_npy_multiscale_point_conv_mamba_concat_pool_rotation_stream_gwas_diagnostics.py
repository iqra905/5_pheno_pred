...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/mnt/fast/nobackup/scratch4weeks/if00208/ukbb/multiscale/mamba/chunked_pool/disease_wise/snv_only', help="Directory to save experiment results")
    ...
    parser.add_argument("-dataloader_type", type=str, default="snv_only", 
                        choices=["standard", "epic", "gwas", "snv_only"],
                        help="Dataloader type: 'standard' (all SNPs), 'epic' (EPIC filtered), 'gwas' (GWAS disease-specific), 'snv_only' (SNVs only)")
...
def train_epoch_detailed_diagnostics(model, dataloader, criterion, optimizer, device, epoch):
    """
    Training with DETAILED diagnostics to see what's actually happening
    """
    model.train()
    total_loss = 0
    batch_losses = []
    ...
    for batch_idx, (genotypes, covariates, labels) in enumerate(dataloader):
        ...
        # Forward pass
        optimizer.zero_grad()
        outputs = model(genotypes, covariates)
        loss = criterion(outputs, labels)
        
        # Backward pass
        loss.backward()
        
        # *** CRITICAL: Check gradients ***
        total_grad_norm = 0
        grad_norms_by_layer = {}
        for name, param in model.named_parameters():
            if param.grad is not None:
                ...
                grad_norms_by_layer[name] = grad_norm
        ...
        with torch.no_grad():
            ...
            all_outputs_raw.append(outputs.cpu())
        
        # *** FIRST BATCH DIAGNOSTICS ***
        if batch_idx == 0:
            print(f"\n  {'='*70}")
            print(f"  EPOCH {epoch} - BATCH 0 DETAILED DIAGNOSTICS")
            print(f"  {'='*70}")
            ...
            if total_grad_norm < 1e-5:
                print(f"     ❌ VANISHING GRADIENTS! Model is not learning.")
            elif total_grad_norm > 1e3:
                print(f"     ❌ EXPLODING GRADIENTS! Training is unstable.")
            else:
                print(f"     ✓ Gradient norm looks reasonable")
            
            print(f"\n  2. MODEL OUTPUTS (raw logits):")
            ...
            print(f"     First 10: {outputs.flatten()[:10].cpu().numpy()}")
            
            if outputs.std().item() < 0.01:
                print(f"     ❌ All outputs nearly identical! Model is stuck.")
            
            print(f"\n  3. PREDICTIONS (after sigmoid):")
            print(f"     Min: {preds.min().item():.4f}")
            print(f"     Max: {preds.max().item():.4f}")
            print(f"     Mean: {preds.mean().item():.4f}")
            print(f"     Std: {preds.std().item():.4f}")
            print(f"     First 10: {preds.flatten()[:10].cpu().numpy()}")
            
            if preds.std().item() < 0.01:
                print(f"     ❌ All predictions ~{preds.mean().item():.3f}! Not learning.")
            
            print(f"\n  4. LABELS:")
            print(f"     Mean: {labels.mean().item():.4f}")
            print(f"     First 10: {labels.flatten()[:10].cpu().numpy()}")
            
            print(f"\n  5. TOP GRADIENT LAYERS:")
            sorted_grads = sorted(grad_norms_by_layer.items(), key=lambda x: x[1], reverse=True)
    ...
    all_outputs_raw = torch.cat(all_outputs_raw).numpy()
...
def evaluate_detailed(model, dataloader, device):
    ...
    with torch.no_grad():
        for genotypes, covariates, labels in dataloader:
            genotypes = genotypes.to(device)
            covariates = covariates.to(device)
    ...
    print(f"  Val prediction mean: {all_preds.mean():.4f}")
    print(f"  Val prediction std: {all_preds.std():.4f}")
...
def main():
    ...
    if args.dataloader_type == "gwas":
        from ukbb_dataloader_with_rotation_multigpu_filtered_gwas_summ_stats import prepare_data_splits, create_dataloaders_with_rotation, create_dataloaders
    ...
    elif args.dataloader_type == "snv_only":
        from ukbb_dataloader_with_rotation_multigpu_stratified_snv_only import prepare_data_splits, create_dataloaders_with_rotation, create_dataloaders
        print("Using SNV-only filtered dataloader (single nucleotide variants)")
    ...
    if args.dataloader_type == "gwas":
        # GWAS dataloader - pass disease_for_snp_filter parameter
        ...
        # Standard/EPIC dataloader - no disease filter parameter
        train_subjects, test_subjects, phenotype_data, input_size = prepare_data_splits(
            disease_labels=disease_labels,
            phenotype_file=args.phenotype_file,
            test_size=0.2,
    ...
    # Train model
    start_time = time.time()
    for epoch in range(5):  # Just 5 epochs to see what's happening
        train_loss, train_auc = train_epoch_detailed_diagnostics(
            model, dataloaders['train'], criterion, optimizer, device, epoch
        )
        val_auc = evaluate_detailed(model, dataloaders['test'], device)
        
        print(f"\n  >>> Epoch {epoch}: Train Loss={train_loss:.4f}, Train AUC={train_auc:.4f}, Val AUC={val_auc:.4f}")
