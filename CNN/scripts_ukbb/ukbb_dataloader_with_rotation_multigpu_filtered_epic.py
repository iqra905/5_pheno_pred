...
AND PREFILTERED SNP INDICES FOR FAST LOADING
...
- FAST SNP filtering using precomputed indices (no runtime computation!)
- Direct loading of filtered indices from disk
- Use preprocess_snp_filtering.py to generate filtered indices first
"""
...
# Global cache for prefiltered indices (loaded once from disk)
_PREFILTERED_INDICES_CACHE = {}


def load_prefiltered_indices(chr_num, base_path=BASE_PATH, rank=0):
    """
    Load prefiltered SNP indices from disk (computed by preprocess_snp_filtering.py)
    
    This function loads pre-computed filter indices, eliminating the need for
    ...
    """
    cache_key = f'chr{chr_num}_prefiltered'
    
    # Check if already loaded in cache
    if cache_key in _PREFILTERED_INDICES_CACHE:
        return _PREFILTERED_INDICES_CACHE[cache_key]
    
    # Try to load prefiltered indices
    chr_dir = os.path.join(base_path, f"chr{chr_num}")
    
    # Try .npy format first (binary, faster)
    npy_file = os.path.join(chr_dir, f"chr{chr_num}_filtered_indices_epic.npy")
    ...
    if os.path.exists(npy_file):
        ...
        if rank == 0:
            print(f"    Loaded {len(indices):,} filtered SNP indices")
        return indices
    elif os.path.exists(txt_file):
        if rank == 0:
            print(f"  Loading prefiltered indices for chr{chr_num} from text file...")
        ...
        if rank == 0:
            print(f"     WARNING: No prefiltered indices found for chr{chr_num}!")
            print(f"     Expected: {npy_file}")
            print(f"     Run preprocess_snp_filtering.py first to generate filtered indices")
            print(f"     Will load ALL SNPs for this chromosome (no filtering)")
        return None
...
def load_genotype_data_for_subject(id1, id2, return_shapes=False, apply_snp_filter=True, rank=0):
    ...
    WITH SNP FILTERING using PREFILTERED INDICES (very fast!)
    
    Args:
        id1: First ID
        id2: Second ID
        return_shapes: If True, return chromosome shapes for debugging
        apply_snp_filter: If True, filter SNPs using prefiltered indices (default: True)
        rank: Process rank for distributed training (default 0)
    
    Returns:
        Concatenated genotype array of shape (total_filtered_snps, 3)
    ...
    for chr_num in range(1, 23):
        ...
        # Apply SNP filtering if requested (using prefiltered indices)
        if apply_snp_filter:
            # Load prefiltered indices (cached after first load)
            filter_indices = load_prefiltered_indices(
                chr_num, 
                rank=rank)
            
            if filter_indices is not None:
                # Filter the data to keep only matched SNPs
                ...
                # No prefiltered indices found - use all SNPs
                if return_shapes:
...
def get_input_size_from_subject(id1, id2, apply_snp_filter=True):
    """
    ...
    Args:
        id1: First ID
        id2: Second ID
        apply_snp_filter: If True, show filtered sizes (default: True)
    
    Returns:
        Total number of SNPs across all chromosomes (after filtering if applied)
    """
    genotype_data, shapes = load_genotype_data_for_subject(
        id1, id2, 
        return_shapes=True, 
        apply_snp_filter=apply_snp_filter
        )
    ...
    if apply_snp_filter:
        ...
        print(f"  Total (Filtered): {total_filtered:,} SNPs")
...
class MultilabelGenotypeDataset(Dataset):
    ...
    def __init__(self, subject_list, phenotype_data, disease_labels, 
                 ...
                 fit_normalizers=True, normalizers=None, apply_snp_filter=True, rank=0):
        """
        Args:
            ...
            apply_snp_filter: Whether to apply SNP filtering using master file (default: True)
            rank: Process rank for distributed training (default: 0)
        """
        ...
        self.apply_snp_filter = apply_snp_filter
        self.rank = rank
    ...
    def __getitem__(self, idx):
        ...
        # Load genotype data (with SNP filtering if enabled)
        genotype_data = load_genotype_data_for_subject(
            id1, id2, 
            return_shapes=False, 
            apply_snp_filter=self.apply_snp_filter, 
            rank=self.rank
            )
...
def prepare_data_splits(disease_labels,
    ...
    if rank == 0:
        ...
        # Suppress prints on non-rank-0 processes
        import sys
        import os
...
def create_dataloaders_with_rotation(train_subjects, test_subjects, phenotype_data, disease_labels,
                                     ...
                                     use_covariates=True, use_age=True, use_gender=True, use_bmi=True,
                                     norm_age="standard", norm_pcs="standard", norm_gender="none", norm_bmi="standard",
                                     use_rotation=True, target_ratio=5, epoch=0, rank=0, apply_snp_filter=True):
    """
    Create training and test dataloaders WITH ROTATION SUPPORT AND SNP FILTERING
...
def create_dataloaders(train_subjects, test_subjects, phenotype_data, disease_labels,
    ...
    dataloaders, _ = create_dataloaders_with_rotation(
        ...
        use_rotation=False, target_ratio=5, epoch=0, apply_snp_filter=apply_snp_filter
    )
