"""
Multi-GPU Distributed Training Version
Key Changes:
1. Added torch.distributed setup for multi-GPU training
2. Model wrapped with DistributedDataParallel (DDP)
3. DataLoaders use DistributedSampler
4. Only rank 0 saves checkpoints and creates outputs
5. Proper gradient synchronization across GPUs
"""
...
# ===== UTILITY FOR SUPPRESSING OUTPUT ON NON-MAIN PROCESSES =====
@contextlib.contextmanager
def suppress_output_if_not_main(is_main):
    """Suppress stdout/stderr if not main process"""
    if is_main:
        yield
    else:
...
def setup_distributed():
    """
    Initialize distributed training environment using Slurm environment variables
    """
    ...
    if world_size > 1:
        ...
        if dist.is_initialized():
            print(f"Process {rank}/{world_size} initialized on {device}")
    else:
...
def parse_args():
    ...
    parser.add_argument("-use_pooling", type=int, default=1, choices=[0, 1], help="Whether to use Pooling after convolution layers (0: no, 1: yes)")
    parser.add_argument("-pool_size", type=int, default=1224, help="Size of the adaptive pooling output")
    ...
    parser.add_argument("-use_rotation", type=int, default=1, choices=[0, 1], help="Whether to use rotation-based control subsampling (0: no, 1: yes)")
...
def validate_hardcoded_parameters(args, rank=0):
    ...
    Note: This function MUST be called on all ranks to ensure args are consistent!
    """
...
def train_multilabel_model_distributed(model, dataloaders, criterion, optimizer, scheduler, 
                            num_epochs, disease_labels, device, rank, world_size,
                            early_stopping=None, checkpoint_dir=None, start_epoch=0, 
                            keep_last_n=2, history=None, initial_best_loss=None,
                            use_rotation=False, train_subjects=None, test_subjects=None, 
                            phenotype_data=None, target_ratio=5, batch_size=5, args=None):
    """
    Modified training function for distributed multi-GPU training
    
    Key changes:
    - Only rank 0 saves checkpoints and creates plots
    - Gradient synchronization happens automatically with DDP
    - DistributedSampler handles data distribution across GPUs
    """
    ...
    since = time.time()
    best_model_wts = copy.deepcopy(model.state_dict() if rank == 0 else None)
    best_loss = initial_best_loss if initial_best_loss is not None else float('inf')
    scaler = GradScaler('cuda')
    completed_epochs = start_epoch
    
    # Initialize history (only on rank 0 for logging)
    if history is None:
        history = {
            'train_loss': [], 'test_loss': [], 'learning_rates': []
        }
        for disease in disease_labels:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # Recreate dataloaders with rotation if enabled
        if use_rotation and train_subjects is not None:
            if rank == 0:
                print(f"\nRecreating DataLoaders with rotation for epoch {epoch}")
            
            # Create new dataloaders for this epoch with rotation
            new_dataloaders, rotation_info = create_dataloaders_with_rotation(
            ...
            # IMPORTANT: Wrap with DistributedSampler for multi-GPU
            if world_size > 1:
                # Create distributed samplers
                train_sampler = DistributedSampler(
                    new_dataloaders['train'].dataset,
                    num_replicas=world_size,
                    rank=rank,
                    shuffle=True
                )
                test_sampler = DistributedSampler(
                    new_dataloaders['test'].dataset,
                    num_replicas=world_size,
                    rank=rank,
                    shuffle=False
                )
                ...
                # Set epoch for samplers (important for shuffling)
        ...
        phase_preds = {
            'train': {disease: [] for disease in disease_labels},
            'test': {disease: [] for disease in disease_labels}
        }
        phase_labels = {
            'train': {disease: [] for disease in disease_labels},
            'test': {disease: [] for disease in disease_labels}
        ...
        for phase in ['train', 'test']:
            ...
            if rank == 0:
                print(f"\nStarting {phase} phase: {batch_count} batches to process (per GPU)")
            ...
            for i in range(len(dataloaders[phase])):
                ...
                inputs = next_inputs.to(device, non_blocking=True)
                covariates = next_covariates.to(device, non_blocking=True)
                ...
                if rank == 0 and ((i + 1) % 500 == 0 or i == 0 or i == len(dataloaders[phase]) - 1):
                    ...
                    print(f"GPU {rank} | {phase} Batch {i+1}/{len(dataloaders[phase])} | "
            ...
            if world_size > 1:
                ...
                for disease in disease_labels:
                    # Convert to tensors with explicit dtype=torch.float32 to avoid AMP dtype issues
                    labels_tensor = torch.tensor(phase_labels[phase][disease], dtype=torch.float32, device=device)
                    preds_tensor = torch.tensor(phase_preds[phase][disease], dtype=torch.float32, device=device)
                    
                    # Gather sizes first
                    local_size = torch.tensor([len(phase_labels[phase][disease])], device=device)
                    ...
                    if labels_tensor.size(0) < max_size:
                        padding = max_size - labels_tensor.size(0)
                        labels_tensor = F.pad(labels_tensor, (0, padding), value=-1)
                        preds_tensor = F.pad(preds_tensor, (0, padding), value=-1)
                    
                    # Gather - buffers must match tensor dtype (float32)
                    labels_gathered = [torch.zeros(max_size, dtype=torch.float32, device=device) for _ in range(world_size)]
                    preds_gathered = [torch.zeros(max_size, dtype=torch.float32, device=device) for _ in range(world_size)]
                    
                    dist.all_gather(labels_gathered, labels_tensor)
                    dist.all_gather(preds_gathered, preds_tensor)
                    
                    # Combine and filter padding
                    all_labels = []
                    all_preds = []
                    for idx, (l, p, s) in enumerate(zip(labels_gathered, preds_gathered, size_list)):
                        valid_size = s.item()
                        all_labels.extend(l[:valid_size].cpu().numpy())
                        all_preds.extend(p[:valid_size].cpu().numpy())
                    
                    phase_labels[phase][disease] = all_labels
                    phase_preds[phase][disease] = all_preds
            ...
            if phase == 'test':
                if rank == 0:
                    if epoch_loss < best_loss:
                        ...
                        best_model_wts = copy.deepcopy(model.module.state_dict() if world_size > 1 else model.state_dict())
                ...
                if stop_training:
                    ...
                    if world_size > 1:
                        # Broadcast best model weights from rank 0
                        if rank == 0:
                        ...
                        # Load on all ranks (DDP will handle this)
        ...
        # Save regular checkpoint (only rank 0)
    ...
    # Compute final metrics (only rank 0)
    if rank == 0:
...
def main():
    ...
    if rank == 0:
        print("Starting Multi-GPU distributed training for multilabel disease prediction")
    ...
    # Validate parameters (ALL ranks must run this to ensure args consistency!)
    if args.use_multi_scale and args.multi_scale_mode == "hardcoded":
        validate_hardcoded_parameters(args, rank)
    ...
    # Prepare data splits (all ranks do this to ensure consistency)
    ...
    if rank == 0:
        ...
        print(f"Effective batch size: {args.bs} per GPU × {world_size} GPUs = {args.bs * world_size}")
    
    # Get pooling configuration
    pooling_kwargs = get_pooling_kwargs(args)
    
    # Create model (all ranks create the same model, but only rank 0 prints)
    with suppress_output_if_not_main(rank == 0):
    ...
    if world_size > 1:
        model = DDP(
            ...
            find_unused_parameters=False  # Set to True if you have conditional paths
        )
        if rank == 0:
    ...
    if rank == 0:
        ...
        # Save model architecture (only rank 0)
    ...
    if args.use_class_weights:
        if rank == 0:
            ...
            # Other ranks create dummy pos_weight, will be broadcast from rank 0
            pos_weight = torch.zeros(len(disease_labels), device=device)
        
        # Broadcast pos_weight from rank 0 to all ranks
        if world_size > 1:
    ...
    # Check for existing checkpoints (only rank 0 loads, then broadcasts)
    if args.resume:
        if rank == 0:
            ...
            if latest_checkpoint:
                ...
                try:
                    ...
                    # For DDP, need to handle 'module.' prefix
                    state_dict = checkpoint['model_state_dict']
                    if world_size > 1:
                        # If saved from DDP, state_dict has 'module.' prefix
                        # If loading into DDP, it's fine. If saved without DDP and loading into DDP,
                        # need to add prefix
                        from collections import OrderedDict
                        new_state_dict = OrderedDict()
                        for k, v in state_dict.items():
                            if not k.startswith('module.') and world_size > 1:
                    ...
                    if 'history' in checkpoint and checkpoint['history'] is not None:
                        ...
                        print("No valid history found in checkpoint")
                        history = None
                    ...
                    # Store checkpoint for loading scheduler and early stopping later
        ...
        # Broadcast checkpoint loading status
        if world_size > 1:
            checkpoint_status = torch.tensor([1 if checkpoint_loaded else 0], device=device)
            dist.broadcast(checkpoint_status, src=0)
            checkpoint_loaded = bool(checkpoint_status.item())
            
            if checkpoint_loaded:
                # Broadcast start_epoch, best_loss
    ...
    # Setup early stopping (only rank 0 tracks)
    # Note: This will be updated from checkpoint if resuming
    if rank == 0:
        early_stopping = EarlyStopping(patience=args.patience, min_delta=args.min_delta, verbose=True)
    else:
        early_stopping = None
    
    # Load scheduler and early stopping states from checkpoint if resuming
    if args.resume and checkpoint_loaded and rank == 0 and loaded_checkpoint is not None:
        # Load scheduler state
        if 'scheduler_state_dict' in loaded_checkpoint and loaded_checkpoint['scheduler_state_dict'] is not None:
            try:
                if hasattr(scheduler, 'load_state_dict'):
                    ...
                    scheduler.load_state_dict(loaded_checkpoint['scheduler_state_dict'])
                    print("Loaded custom scheduler state from checkpoint")
            except Exception as e:
                print(f"Warning: Could not load scheduler state: {e}")
        
        # Load early stopping state
        if 'early_stopping_state' in loaded_checkpoint and loaded_checkpoint['early_stopping_state'] is not None:
            try:
                early_stopping.load_state_dict(loaded_checkpoint['early_stopping_state'])
                print(f"Loaded early stopping state from checkpoint")
                print(f"  - Counter: {early_stopping.counter}/{early_stopping.patience}")
                print(f"  - Best loss: {early_stopping.best_loss:.6f}")
            except Exception as e:
                print(f"Warning: Could not load early stopping state: {e}")
    ...
    if world_size > 1:
        ...
        # Recreate dataloaders with samplers
        dataloaders = {
            'train': DataLoader(
                dataloaders['train'].dataset,
                batch_size=args.bs,
        ...
        if rank == 0:
            print("DataLoaders wrapped with DistributedSampler for multi-GPU training")
    ...
    model, history, final_metrics, all_preds, all_labels, completed_epochs = train_multilabel_model_distributed(
        model, dataloaders, criterion, optimizer, scheduler,
        args.epochs, disease_labels, device, rank, world_size,
