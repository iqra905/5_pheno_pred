...
class NatureGWASVisualizer:
    def __init__(self, excel_path, output_dir):
        ...
        self.method_colors = {
            ...
            'Multilabel (without Covariates)': 'red',                
            'Multilabel (with Covariates)': 'red'                  
        }
        
        # Line styles for differentiation
        self.line_styles = {
            'Disease-wise (without Covariates)': ':',     # Dotted
            'Disease-wise (with Covariates)': '-',       # Solid
            'Multilabel (without Covariates)': ':',       # Dotted
            'Multilabel (with Covariates)': '-'          # Solid
        }
    ...
    def create_figure02_combined(self):
        """Figure 2: Population Dataset vs Train Set comparison - separated by covariate conditions"""
        ...
        # TOP SUBPLOT: Without Covariates (Population Dataset vs Train Set Only)
        for i, disease in enumerate(diseases):
            base_color = self.disease_colors[disease]
            
            # Simulate error bars
            error_pop = pop_no_cov[i] * 0.03
            error_train = train_no_cov[i] * 0.05
            
            # Population Dataset - solid bars
            ax1.bar(x[i] - width/2 * spacing, pop_no_cov[i], width, 
                color=base_color, edgecolor='red', linewidth=1.5, 
                yerr=error_pop, capsize=2, error_kw={'linewidth': 0.8},
                label='Population Dataset' if i == 0 else "")
                
            # Label for population dataset bar
            ax1.text(x[i] - width/2 * spacing, pop_no_cov[i] + error_pop + 0.01, 
                    f"{pop_no_cov[i]:.3f}", ha='center', va='bottom', fontweight='bold', fontsize=6)
            
            # Train Set Only - hatched bars
            ax1.bar(x[i] + width/2 * spacing, train_no_cov[i], width, 
                color=base_color, edgecolor='black', linewidth=1.5, hatch='///', 
                yerr=error_train, capsize=2, error_kw={'linewidth': 0.8},
                label='Train Set Only' if i == 0 else "")
            
            # Label for train set bar
            ax1.text(x[i] + width/2 * spacing, train_no_cov[i] + error_train + 0.01, 
                    f"{train_no_cov[i]:.3f}", ha='center', va='bottom', fontweight='bold', fontsize=6)

        # BOTTOM SUBPLOT: With Covariates (Population Dataset vs Train Set Only)
        for i, disease in enumerate(diseases):
            base_color = self.disease_colors[disease]
            
            # Simulate error bars
            error_pop = pop_cov[i] * 0.03
            error_train = train_cov[i] * 0.05
            
            # Population Dataset - solid bars
            ax2.bar(x[i] - width/2 * spacing, pop_cov[i], width, 
                color=base_color, edgecolor='red', linewidth=1.5,
                yerr=error_pop, capsize=2, error_kw={'linewidth': 0.8})
                
            # Label for population dataset bar
            ax2.text(x[i] - width/2 * spacing, pop_cov[i] + error_pop + 0.01, 
            ...
            # Label for train set bar
            ax2.text(x[i] + width/2 * spacing, train_cov[i] + error_train + 0.01, 
                    f"{train_cov[i]:.3f}", ha='center', va='bottom', fontweight='bold', fontsize=6)

        # Create custom legend
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor='white', edgecolor='red', 
                        linewidth=1.0, label='Population Dataset (Data Leakage)'),
            plt.Rectangle((0,0),1,1, facecolor='white', edgecolor='black', 
                        linewidth=1.0, hatch='///', label='Train Set Only (Proper Training)')
        ]
        
        # Configure TOP subplot (Without Covariates)
        ax1.spines['top'].set_visible(False)
        ax1.spines['right'].set_visible(False)
        ax1.margins(x=0.05)
        ax1.set_ylabel('AUC', fontweight='bold', fontsize=10)
        ax1.set_title('Without Covariates', fontweight='bold', fontsize=11, pad=10)
        ax1.set_xticks(x)
    ...
    def create_figure02_without_covariates(self):
        """Figure 2: Population Dataset vs Train Set comparison - without covariates only"""
        ...
        # Single plot: Without Covariates (Population Dataset vs Train Set Only)
        for i, disease in enumerate(diseases):
            base_color = self.disease_colors[disease]
            ...
            # Label for population dataset bar
            ax.text(x[i] - width/2 * spacing, pop_no_cov[i] + error_pop + 0.01, 
                    f"{pop_no_cov[i]:.3f}", ha='center', va='bottom', fontweight='bold', fontsize=8)
            ...
            # Label for train set bar
            ax.text(x[i] + width/2 * spacing, train_no_cov[i] + error_train + 0.01, 
                    f"{train_no_cov[i]:.3f}", ha='center', va='bottom', fontweight='bold', fontsize=8)

        # Create custom legend
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor='white', edgecolor='red', 
                        linewidth=1.5, label='Population Dataset (Data Leakage)'),
            plt.Rectangle((0,0),1,1, facecolor='white', edgecolor='black', 
                        linewidth=1.0, hatch='///', label='Train Set Only (Proper Training)')
        ]
        ...
        ax.set_title('Population Dataset vs Train Set Only', 
                     fontweight='bold', fontsize=14, pad=15)
        ax.set_xticks(x)
        ax.set_xticklabels(diseases, fontsize=10, color='black', fontweight='bold')
        ax.grid(True, alpha=0.3, axis='y')
        ax.set_ylim(0.2, 1.2)
        ax.set_yticks(np.arange(0.2, 1.2, 0.2)) 
        ax.set_yticklabels([f"{tick:.1f}" for tick in np.arange(0.2, 1.2, 0.2)], 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'fig_02_without_covariates.jpg'), format='jpg', bbox_inches='tight', dpi=600)
        print("Figure 2 saved: fig_02_without_covariates.jpg")
    
    def create_radar_plot_with_global_max(self, data_dict, global_max_dict, title, filename):
        """Create radar plot using global maximum values for scaling"""
        fig, ax = plt.subplots(figsize=(8, 8), subplot_kw=dict(projection='polar'))
        
        diseases = list(data_dict[list(data_dict.keys())[0]].keys())
        methods = list(data_dict.keys())
        
        # Use the provided global max values instead of calculating from current data
        disease_max_values = global_max_dict
        
        # Calculate minimum values from current data only
        disease_min_values = {}
        ...
        # Create normalized data for plotting (each disease scaled to its global max)
        normalized_data = {}
        ...
        for method in methods:
            ...
            for disease in diseases:
                ...
                # Scale to plot radius based on global maximum
        ...
        grid_radii = [0.60, 0.65, 0.70, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]
        ...
        # Set up the plot
        ax.set_ylim(0.8, plot_radius + 0.04)
        ...
        for i, (angle, disease) in enumerate(zip(angles[:-1], diseases)):
            max_value = disease_max_values[disease]  # Use global max
            min_value = disease_min_values[disease]   # Use local min
            ...
            # Add maximum value at the edge of each axis (GLOBAL MAX)
            ...
            # Add Minimum value at the inner edge of each axis (LOCAL MIN)
        ...
        # Enhanced legend
        ax.legend(loc='lower right', bbox_to_anchor=(0.5, -0.1), ncol=1, 
    ...
    def create_radar_plot(self, data_dict, title, filename, show_covariates_label=False):
        ...
        for i, (angle, disease) in enumerate(zip(angles[:-1], diseases)):
            ...
            ax.text(angle, max_label_radius, f'{max_value:.3f}', 
                fontsize=9, fontweight='bold', ha='center', va='center',
                color='red',  # Green for max values
                )
    ...
    def create_figure03_disease_wise_global_scale(self):
        """Figure 3: Disease-wise methods only but scaled with global maximum values"""
        df = self.data['Multilabel']
        diseases = df['Disease'].tolist()
        
        # First, get ALL method data to calculate global max values
        all_data_dict = {
        ...
        for i, disease in enumerate(diseases):
            all_data_dict['Disease-wise (without Covariates)'][disease] = df.iloc[i]['Disease-wise (without Covariates)']
            all_data_dict['Disease-wise (with Covariates)'][disease] = df.iloc[i]['Disease-wise (with Covariates)']
            all_data_dict['Multilabel (without Covariates)'][disease] = df.iloc[i]['Multilabel (without Covariates)'] 
            all_data_dict['Multilabel (with Covariates)'][disease] = df.iloc[i]['Multilabel (with Covariates)']
        
        # Calculate global maximum values across ALL methods
        global_max_values = {}
        for disease in diseases:
            disease_values = [all_data_dict[method][disease] for method in all_data_dict.keys()]
            global_max_values[disease] = max(disease_values)
        
        # Now create disease-wise only data dictionary
        disease_wise_data_dict = {
            'Disease-wise (without Covariates)': {},
            'Disease-wise (with Covariates)': {}
        }
        
        for i, disease in enumerate(diseases):
            disease_wise_data_dict['Disease-wise (without Covariates)'][disease] = df.iloc[i]['Disease-wise (without Covariates)']
            disease_wise_data_dict['Disease-wise (with Covariates)'][disease] = df.iloc[i]['Disease-wise (with Covariates)']
        
        # Create radar plot with disease-wise data but global scaling
        self.create_radar_plot_with_global_max(disease_wise_data_dict, global_max_values,
                             'Disease-wise Methods with Global Scale', 
                             'fig_03_disease_wise_global_scale.jpg')
        print("Figure 3 saved: fig_03_disease_wise_global_scale.jpg")
    ...
    def generate_all_figures(self):
        ...
        print("\nGenerating Figure 2: Population Dataset vs Train Set (Without Covariates Only)...")
        self.create_figure02_without_covariates()
        
        print("\nGenerating Figure 2: Combined Population Dataset and Train Set...")
        self.create_figure02_combined()
        
        print("\nGenerating Figure 3: Disease-wise Methods with Global Scale...")
        self.create_figure03_disease_wise_global_scale()
        ...
        print("- fig_02_without_covariates.jpg") 
        print("- fig_02_combined.jpg") 
        print("- fig_03_disease_wise_global_scale.jpg")
...
if __name__ == "__main__":
    ...
    output_dir = '/vol/research/fmodal_mmmed/Codes/5_disease_experiments/GWAS_Results_AIiH_2025/1'
