...
plt.rcParams.update({
    ...
    'hatch.linewidth': 0.5,  # Control hatch pattern line width
    'figure.dpi': 500,
    'savefig.dpi': 500,
    'savefig.bbox': 'tight',
...
class NatureGWASVisualizer:
    def __init__(self, excel_path, output_dir):
        ...
        self.method_colors = {
            ...
            'Multilabel (without Covariates)': '#DE8F05',                
            'Multilabel (with Covariates)': '#DE8F05'                  
        }
    ...
    def create_figure02a_population_dataset(self):
        ...
        plt.savefig(os.path.join(self.output_dir, 'fig_02a_population_dataset.jpg'), format='jpg', bbox_inches='tight', dpi=300)
    ...
    def create_figure02b_train_set(self):
        ...
        plt.tight_layout()
        plt.subplots_adjust(left=0.01, right=0.99, top=0.85, bottom=0.25)
        plt.savefig(os.path.join(self.output_dir, 'fig_02b_train_set.jpg'), format='jpg', bbox_inches='tight', dpi=300)
    ...
    def create_radar_plot(self, data_dict, title, filename, show_covariates_label=False):
        ...
        for i, (angle, disease) in enumerate(zip(angles[:-1], diseases)):
            ...
            ax.text(angle, max_label_radius, f'{max_value:.3f}', 
                ...
                color='#DE8F05',  # Green for max values
                )
