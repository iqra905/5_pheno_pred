...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Mamba_pretrained_Exp_01", help="ID of the experiment")
    ...
    # Pretrained Mamba initialization arguments
    parser.add_argument("-init_from_pretrained", type=int, default=1, choices=[0, 1], help="Whether to initialize Mamba layers from pretrained weights (0: no, 1: yes)")
    parser.add_argument("-pretrained_model_name", type=str, default="state-spaces/mamba-130m", help="Pretrained Mamba model name or path (HuggingFace or local checkpoint)")
    parser.add_argument("-pretrained_source", type=str, default="auto", choices=["auto", "huggingface", "checkpoint"], help="Source of pretrained weights: auto (detect), huggingface, or checkpoint file")
    parser.add_argument("-init_layers_fraction", type=float, default=1.0, help="Fraction of Mamba layers to initialize from pretrained (0.0-1.0)")
    parser.add_argument("-layer_init_strategy", type=str, default="middle", choices=["first", "middle", "last", "random", "custom"], help="Which layers to initialize: first N, middle N, last N, random N, or custom indices")
    parser.add_argument("-custom_layer_indices", type=str, default="", help="Comma-separated layer indices for 'custom' strategy (e.g., '0,2,4')")
...
class GenomicMambaBlock(nn.Module):
    ...
    def __init__(self, input_dim, d_model, d_state=16, d_conv=4, expand=2, 
                 ...
                 init_from_pretrained=False, pretrained_model_name="state-spaces/mamba-130m",
                 pretrained_source="auto", init_layers_fraction=1.0, 
                 layer_init_strategy="first", custom_layer_indices="",
                 **pooling_kwargs):
        ...
        # Initialize from pretrained weights if requested
        ...
        if init_from_pretrained:
            ...
            print(f"  - Layer init strategy: {layer_init_strategy}")
    ...
    def _initialize_from_pretrained(self):
        """Initialize Mamba weights from pretrained model"""
        print(f"\n  Initializing Mamba weights from pretrained model: {self.pretrained_model_name}")
        print(f"  Pretrained source: {self.pretrained_source}")
        
        try:
            source = self._detect_pretrained_source()
    ...
    def _detect_pretrained_source(self):
        """Detect the source of pretrained weights"""
        if self.pretrained_source.lower() != "auto":
            return self.pretrained_source.lower()
        
        # Auto-detect based on model name
        if self.pretrained_model_name.endswith(('.pt', '.pth', '.ckpt', '.bin')):
            return "checkpoint"
        elif '/' in self.pretrained_model_name or self.pretrained_model_name.startswith('state-spaces'):
            return "huggingface"
        else:
            print(f"  Warning: Could not auto-detect source for '{self.pretrained_model_name}'. Defaulting to 'huggingface'")
            return "huggingface"
    
    def _initialize_from_huggingface(self):
        """Initialize from HuggingFace Mamba model"""
        try:
            from transformers import AutoModel
            print(f"  - Loading pretrained Mamba model from HuggingFace: {self.pretrained_model_name}")
        except ImportError:
            print(f"  - Warning: transformers library not available. Trying mamba_ssm...")
            try:
                from mamba_ssm.models.mixer_seq_simple import MambaLMHeadModel
                print(f"  - Loading pretrained Mamba model using mamba_ssm: {self.pretrained_model_name}")
                pretrained_model = MambaLMHeadModel.from_pretrained(self.pretrained_model_name)
                pretrained_layers = pretrained_model.backbone.layers
            except ImportError:
                raise ImportError("Neither transformers nor mamba_ssm is available for loading pretrained models")
        else:
            # Try loading with transformers
            pretrained_model = AutoModel.from_pretrained(self.pretrained_model_name, trust_remote_code=True)
            
            # Extract Mamba layers from the pretrained model
            if hasattr(pretrained_model, 'backbone') and hasattr(pretrained_model.backbone, 'layers'):
                pretrained_layers = pretrained_model.backbone.layers
            elif hasattr(pretrained_model, 'layers'):
                pretrained_layers = pretrained_model.layers
            elif hasattr(pretrained_model, 'encoder') and hasattr(pretrained_model.encoder, 'layers'):
                pretrained_layers = pretrained_model.encoder.layers
            else:
                raise AttributeError("Could not find Mamba layers in the pretrained model structure")
        
        print(f"  - Loaded pretrained model with {len(pretrained_layers)} layers")
        
        # Get pretrained config info
        if hasattr(pretrained_model, 'config'):
            config = pretrained_model.config
            if hasattr(config, 'd_model'):
                print(f"  - Pretrained d_model: {config.d_model}")
            if hasattr(config, 'd_state'):
                print(f"  - Pretrained d_state: {config.d_state}")
        
        # Calculate how many layers to initialize
        num_layers_to_init = int(self.num_layers * self.init_layers_fraction)
        print(f"  - Initializing {num_layers_to_init} out of {self.num_layers} custom layers ({self.init_layers_fraction*100:.0f}%)")
        ...
        # Copy weights from pretrained layers
        self._copy_mamba_weights(pretrained_layers, pretrained_layer_indices)
        
        print(f"  - HuggingFace Mamba weight initialization completed successfully")
    
    def _initialize_from_checkpoint(self):
        """Initialize from a local checkpoint file"""
        print(f"  - Loading pretrained Mamba model from checkpoint: {self.pretrained_model_name}")
        
        import os
        if not os.path.exists(self.pretrained_model_name):
            raise FileNotFoundError(f"Checkpoint file not found: {self.pretrained_model_name}")
        
        # Load checkpoint
        checkpoint = torch.load(self.pretrained_model_name, map_location='cpu')
        ...
        self._copy_weights_from_state_dict(state_dict, num_layers_to_init)
        
        print(f"  - Checkpoint weight initialization completed successfully")
    ...
    def _copy_mamba_weights(self, pretrained_layers, pretrained_layer_indices):
        """Copy weights from pretrained Mamba layers to custom layers"""
        for i, pretrained_idx in enumerate(pretrained_layer_indices):
            if i < len(self.mamba_layers):
                print(f"    Initializing custom layer {i} from pretrained layer {pretrained_idx}")
                self._copy_single_mamba_layer(
                    pretrained_layers[pretrained_idx], 
                    self.mamba_layers[i],
                    layer_idx=i
                )
    
    def _copy_single_mamba_layer(self, pretrained_layer, custom_layer, layer_idx):
        """Copy weights from a single pretrained Mamba layer"""
        try:
            with torch.no_grad():
                # Mamba layer structure varies, so we try to intelligently copy weights
                pretrained_state = pretrained_layer.state_dict()
                custom_state = custom_layer.state_dict()
                ...
                for key in custom_state.keys():
                    if key in pretrained_state:
                        ...
                        if pretrained_param.shape == custom_param.shape:
                            ...
                            custom_state[key].copy_(pretrained_param)
                            transferred += 1
                        else:
                            # Shape mismatch - try to adapt
                            adapted_param = self._adapt_weight_tensor(
                                pretrained_param, 
                                custom_param.shape, 
                                key
                            )
                            custom_state[key].copy_(adapted_param)
                            partial += 1
                    else:
                        skipped += 1
                
                # Load the updated state dict
                custom_layer.load_state_dict(custom_state)
                
                print(f"      Layer {layer_idx}: {transferred} weights copied exactly, {partial} adapted, {skipped} randomly initialized")
                
        except Exception as e:
            print(f"      Error copying layer {layer_idx}: {e}")
            print(f"      Keeping random initialization for this layer")
    
    def _copy_weights_from_state_dict(self, state_dict, num_layers_to_init):
        """Copy weights from a state dict (checkpoint format)"""
        # Try to find Mamba layer weights in the state dict
        mamba_keys = [k for k in state_dict.keys() if 'mamba' in k.lower() or 'mixer' in k.lower()]
        
        if not mamba_keys:
            print(f"    Warning: No Mamba layer weights found in checkpoint")
            return
        
        print(f"    Found {len(mamba_keys)} Mamba-related keys in checkpoint")
        ...
        for key in mamba_keys:
            ...
            import re
            match = re.search(r'layer[s]?[\._]?(\d+)', key, re.IGNORECASE)
            if match:
                layer_idx = int(match.group(1))
                if layer_idx not in layer_weights:
                    layer_weights[layer_idx] = {}
                layer_weights[layer_idx][key] = state_dict[key]
        
        # Copy weights for specified number of layers
        for i in range(min(num_layers_to_init, len(layer_weights))):
            if i in layer_weights and i < len(self.mamba_layers):
                print(f"    Initializing custom layer {i} from checkpoint")
                self._load_layer_from_dict(self.mamba_layers[i], layer_weights[i])
    
    def _load_layer_from_dict(self, layer, weight_dict):
        """Load a layer's weights from a dictionary"""
        try:
            layer_state = layer.state_dict()
            updated_count = 0
            
            for key, value in weight_dict.items():
                # Extract parameter name from full key
                param_name = key.split('.')[-1]
                
                if param_name in layer_state:
                    if value.shape == layer_state[param_name].shape:
                        layer_state[param_name].copy_(value)
                        updated_count += 1
            ...
            print(f"      Error loading layer weights: {e}")
    ...
    def _adapt_weight_tensor(self, pretrained_weight, target_shape, weight_name):
        ...
        if len(target_shape) == 2 and len(pretrained_shape) == 2:
            ...
            # Initialize remaining weights with Xavier/Glorot
        ...
        # For 1D weights (bias vectors, parameters)
        ...
        # For higher-dimensional tensors (convolutions, etc.)
        elif len(pretrained_shape) >= 3:
            adapted_weight = torch.zeros(target_shape, dtype=pretrained_weight.dtype, device=pretrained_weight.device)
            
            # Copy overlapping regions along each dimension
            slices_pretrained = [slice(0, min(p, t)) for p, t in zip(pretrained_shape, target_shape)]
            slices_adapted = [slice(0, min(p, t)) for p, t in zip(pretrained_shape, target_shape)]
            
            adapted_weight[tuple(slices_adapted)] = pretrained_weight[tuple(slices_pretrained)]
            
            # Initialize remaining with small random values
            mask = torch.ones_like(adapted_weight, dtype=torch.bool)
            mask[tuple(slices_adapted)] = False
            if mask.any():
                adapted_weight[mask] = torch.randn(mask.sum()) * 0.02
            
            return adapted_weight
        
        # Fallback: Xavier initialization for unsupported shapes
        else:
            print(f"         Unsupported shape adaptation for {weight_name}, using Xavier initialization")
            adapted_weight = torch.zeros(target_shape, dtype=pretrained_weight.dtype, device=pretrained_weight.device)
...
class MultilabelGenomicMambaModel(nn.Module):
    def __init__(self, input_size, num_diseases, kernel_sizes, stride, conv_channels, fc_layers, act, dropout_rate, 
                 ...
                 init_from_pretrained=False, pretrained_model_name="state-spaces/mamba-130m", pretrained_source="auto",
                 init_layers_fraction=1.0, layer_init_strategy="first", custom_layer_indices=""):
...
def main():
    ...
    hyperparameters = {
        ...
        'Init_From_Pretrained': bool(args.init_from_pretrained) if args.use_mamba else 'N/A',
        'Pretrained_Model_Name': args.pretrained_model_name if (args.use_mamba and args.init_from_pretrained) else 'N/A',
        'Pretrained_Source': args.pretrained_source if (args.use_mamba and args.init_from_pretrained) else 'N/A',
        'Init_Layers_Fraction': args.init_layers_fraction if (args.use_mamba and args.init_from_pretrained) else 'N/A',
        'Layer_Init_Strategy': args.layer_init_strategy if (args.use_mamba and args.init_from_pretrained) else 'N/A',
