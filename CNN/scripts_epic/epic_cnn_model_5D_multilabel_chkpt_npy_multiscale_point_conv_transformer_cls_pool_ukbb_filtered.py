...
def parse_args():
    ...
    parser.add_argument("-snp_indices_file", type=str, default=None, help="Path to .npy file of EPIC SNP indices common with UKBB (e.g., epic_indices_all_chr.npy). If None, all SNPs are used.")
...
class MultilabelGenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, disease_labels, use_covariates=True, use_age=True,
                 ...
                 fit_normalizers=True, normalizers=None, snp_indices=None):
        ...
        self.snp_indices = snp_indices  # numpy int64 array of EPIC indices to keep, or None for all SNPs
        ...
        if snp_indices is not None:
            print(f"- SNP filtering: {len(snp_indices):,} common EPIC-UKBB SNPs")
        else:
            print(f"- SNP filtering: disabled (all SNPs used)")
    ...
    def __getitem__(self, idx):
        ...
        if '.npy' in genotype_file:
            genotype_data = np.load(genotype_file)  # Shape: (n_snps, 3)
            if self.snp_indices is not None:
                genotype_data = genotype_data[self.snp_indices]  # Shape: (n_common_snps, 3)
...
def main():
    ...
    # Load SNP indices for EPIC-UKBB common SNP filtering
    snp_indices = None
    if args.snp_indices_file is not None:
        print(f"\nLoading EPIC-UKBB common SNP indices from: {args.snp_indices_file}")
        snp_indices = np.load(args.snp_indices_file).astype(np.int64)
        print(f"  Loaded {len(snp_indices):,} common SNP indices")
    ...
    if first_genotype_file:
        ...
        print(f"Full genotype input size: {input_size:,} SNPs")
        if snp_indices is not None:
            input_size = len(snp_indices)
            print(f"Input size after EPIC-UKBB SNP filtering: {input_size:,} SNPs")
    else:
