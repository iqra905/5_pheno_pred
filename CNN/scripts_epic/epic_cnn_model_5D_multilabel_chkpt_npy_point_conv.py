...
from sklearn.metrics import roc_auc_score, f1_score, precision_recall_curve, accuracy_score, confusion_matrix, roc_curve, auc
...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_2_cov_sss_point", help="ID of the experiment")
    ...
    parser.add_argument("-use_pointwise_conv", type=int, default=1, choices=[0, 1], help="Whether to use pointwise (1x1) convolution before concatenation (0: no, 1: yes)")
...
class MultilabelGenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, disease_labels, use_covariates=True, use_age=False, 
...
class MultilabelGenotypeModel(nn.Module):
    def __init__(self, input_size, num_diseases, kernel_sizes, stride, conv_channels, fc_layers, act, dropout_rate, 
                 ...
                 use_pooling=True, pool_size=16, use_pointwise_conv=True, pointwise_channels=8):
        ...
        if self.use_pointwise_conv:
            self.pointwise_conv = nn.Conv1d(
                in_channels=conv_channels[-1],  # Last conv layer's output channels
                out_channels=pointwise_channels,
                kernel_size=1,  # 1x1 convolution
                stride=1,
                padding=0
            )
            self.pointwise_bn = nn.BatchNorm1d(pointwise_channels)
            self.pointwise_activation = self.get_activation(act)
            print(f"Using pointwise convolution: {conv_channels[-1]} -> {pointwise_channels} channels")
        else:
            self.pointwise_conv = None
            print("Not using pointwise convolution")
        ...
        pointwise_status = f"with pointwise conv ({conv_channels[-1]}->{pointwise_channels})" if use_pointwise_conv else "without pointwise conv"
        print(f"MultilabelGenotypeModel initialized with {num_diseases} disease outputs "
              f"(using covariates: {use_covariates}, age: {use_age}, gender: {use_gender}, bmi: {use_bmi}) "
              f"Pooling: {pooling_status}, Pointwise: {pointwise_status}")
    ...
    def _get_conv_output_size(self, input_size):
        ...
        if self.use_pointwise_conv and self.pointwise_conv is not None:
    ...
    def forward(self, x, covariates=None):
        ...
        # Conditionally apply pooling
        if self.use_pooling and self.pool is not None:
            x = self.pool(x)

        # Conditionally apply pointwise convolution
        if self.use_pointwise_conv and self.pointwise_conv is not None:
            x = self.pointwise_conv(x)
            x = self.pointwise_bn(x)
            x = self.pointwise_activation(x)
...
def main():
    ...
    # Print final metrics for each disease
