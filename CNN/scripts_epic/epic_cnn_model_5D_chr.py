...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="exp_final2", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/pros/pros_0.01/chr_wise/', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/pros_can_0.01', help="Directory containing genotype files")
    ...
    parser.add_argument("-snp_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/meta_data/first_5_columns_pros_can_0.01.gen', help="Path to SNP information file")
    ...
    parser.add_argument("-label_col", type=str, default="pros01", help="Column name in phenotype file to use as label (e.g., 'pros01', etc.)")
...
class ChromosomeWiseGenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, snp_info, label_column, use_covariates=True):
...
class ChromosomeWiseGenotypeModel(nn.Module):
    def __init__(self, chr_input_sizes, conv_channels, fc_layers, act, dropout_rate, use_covariates=True, num_covariates=10):
        ...
        in_features = total_conv_output + (num_covariates if use_covariates else 0)
        ...
        print(f"ChromosomeWiseGenotypeModel initialized (using covariates: {use_covariates})")
            ...
            for i in range(len(dataloaders[phase])):
                ...
                if i + 1 < len(dataloaders[phase]):
                    with torch.cuda.stream(stream):
                        ...
                        next_inputs = {k: v.to(device, non_blocking=True) for k, v in next_inputs.items()}
...
def main():
   ...
   # Load SNP info
    ...
    train_dataset = ChromosomeWiseGenotypeDataset(train_files, phenotype_data, snp_info, label_column=args.label_col, use_covariates=bool(args.cov))
    val_dataset = ChromosomeWiseGenotypeDataset(val_files, phenotype_data, snp_info, label_column=args.label_col,use_covariates=bool(args.cov))
    test_dataset = ChromosomeWiseGenotypeDataset(test_files, phenotype_data, snp_info, label_column=args.label_col, use_covariates=bool(args.cov))
    ...
    try:
        ...
        results.update({
            ...
            'top_snp_importance_score': significant_snps['Importance_Score'].max()
        })
    except Exception as e:
        print(f"Error during importance analysis: {str(e)}")
