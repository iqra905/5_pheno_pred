...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="exp_saved_model_without_cov", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/pros/pros_full/full/', help="Directory to save experiment results")
    ...
    parser.add_argument("-label_col", type=str, default="pros01", help="Column name in phenotype file to use as label (e.g., 'crc')")
    ...
    # Transfer Learning-related arguments
    parser.add_argument("-mode", type=str, default="transfer", choices=["train", "transfer"], help="Mode: 'train' from scratch, 'transfer' from pretrained model")
    parser.add_argument("-pretrained_model", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/t2d/t2d_full/full/exp_saved_model_with_cov/trained_model.pth', help="Path to pretrained model weights file (.pth)")
    parser.add_argument("-freeze_layers", type=int, default=0, choices=[0, 1], help="Whether to freeze convolutional layers during transfer learning")
    parser.add_argument("-transfer_lr", type=float, default=0.0001, help="Learning rate to use for transfer learning (typically lower than initial training)")
...
def initialize_model(args, input_size, device):
    ...
    # If using pretrained model for transfer learning or inference
    if args.mode in ["transfer"] and args.pretrained_model:
        print(f"Loading pretrained weights from {args.pretrained_model}")
        
        # Load pretrained weights
        pretrained_weights = torch.load(args.pretrained_model, map_location=device)

        model_dict = model.state_dict()

        # Print the complete layer list from the pretrained model
        print("\n--- PRETRAINED MODEL LAYERS ---")
        for layer_name in pretrained_weights.keys():
            if hasattr(pretrained_weights[layer_name], 'shape'):
                layer_shape = pretrained_weights[layer_name].shape
                print(f"{layer_name}: {layer_shape}")
            else:
                print(f"{layer_name}: (Parameter without shape)")
        print("-------------------------------\n")

        # Print the new model's layer structure
        print("\n--- NEW MODEL LAYERS ---")
        for layer_name in model_dict.keys():
            if hasattr(model_dict[layer_name], 'shape'):
                layer_shape = model_dict[layer_name].shape
                print(f"{layer_name}: {layer_shape}")
            else:
                print(f"{layer_name}: (Parameter without shape)")
        print("------------------------\n")
        ...
        # # Filter out final layer if we're doing transfer learning (different task)
        # # The output layer dimensions might be different if the class distributions are different
        # if args.mode == "transfer":
        #     pretrained_dict = {k: v for k, v in pretrained_weights.items() 
        #                        if k in model_dict and 'output_layer' not in k}
        #     print(f"Transfer learning mode: Loaded {len(pretrained_dict)}/{len(pretrained_weights)} layers from pretrained model")

                  
        # Filter for conv layers and handle parameter mismatches
        if args.mode == "transfer":
            adapted_weights = {}
            mismatch_count = 0
            adaptation_details = []
            ...
            for k, v in model_dict.items():
                ...
                if 'conv_layers' in k:
                    # Check if this layer exists in pretrained model
                    if k in pretrained_weights:
                        pretrained_param = pretrained_weights[k]
                        new_param = model_dict[k]
                        
                        # If shapes match, direct transfer
                        if pretrained_param.shape == new_param.shape:
                            adapted_weights[k] = pretrained_param
                        else:
                            mismatch_count += 1
                            # Handle weights for convolutional layers
                            if '.weight' in k and len(new_param.shape) == 3:  # Conv1d weights
                                # Shape is typically [out_channels, in_channels, kernel_size]
                                p_out, p_in, p_kernel = pretrained_param.shape
                                n_out, n_in, n_kernel = new_param.shape
                                
                                # Create a new tensor for adapted weights
                                adapted_param = torch.zeros_like(new_param)
                                
                                # Handle output channel dimension
                                out_channels = min(p_out, n_out)
                                
                                # Handle input channel dimension
                                in_channels = min(p_in, n_in)
                                
                                # Handle kernel size dimension
                                if p_kernel > n_kernel:
                                    # Trim kernel from center
                                    start = (p_kernel - n_kernel) // 2
                                    kernel_data = pretrained_param[:out_channels, :in_channels, start:start+n_kernel]
                                    adaptation_details.append(
                                        f"Layer {k}: Trimmed kernel from {p_kernel} to {n_kernel} (center)"
                                    )
                                elif p_kernel < n_kernel:
                                    # Use replication-based padding instead of zeros
                                    kernel_data = torch.zeros(out_channels, in_channels, n_kernel)
                                    start = (n_kernel - p_kernel) // 2
                                    
                                    # First, place the original kernel in the center
                                    kernel_data[:out_channels, :in_channels, start:start+p_kernel] = \
                                        pretrained_param[:out_channels, :in_channels, :]
                                    
                                    # Now replicate edges outward to fill the left side
                                    for i in range(start):
                                        kernel_data[:out_channels, :in_channels, i] = \
                                            pretrained_param[:out_channels, :in_channels, 0]
                                    
                                    # Replicate edges outward to fill the right side
                                    for i in range(start+p_kernel, n_kernel):
                                        kernel_data[:out_channels, :in_channels, i] = \
                                            pretrained_param[:out_channels, :in_channels, -1]
                                            
                                    adaptation_details.append(
                                        f"Layer {k}: Expanded kernel from {p_kernel} to {n_kernel} with edge replication"
                                    )
                                else:
                                    # Same kernel size
                                    kernel_data = pretrained_param[:out_channels, :in_channels, :]
                                
                                # Copy the adapted data
                                adapted_param[:out_channels, :in_channels, :] = kernel_data
                                
                                # If new model has more output channels, replicate to fill
                                if n_out > p_out:
                                    # Replicate existing channels to fill the additional ones
                                    for i in range(p_out, n_out):
                                        # Cyclically copy from existing channels
                                        src_idx = i % p_out
                                        adapted_param[i, :in_channels, :] = adapted_param[src_idx, :in_channels, :]
                                    adaptation_details.append(
                                        f"Layer {k}: Replicated output channels from {p_out} to {n_out}"
                                    )
                                
                                # If new model has more input channels, initialize randomly or replicate
                                if n_in > p_in:
                                    # Replicate existing input channels
                                    for i in range(p_in, n_in):
                                        # Cyclically copy from existing channels
                                        src_idx = i % p_in
                                        adapted_param[:, i, :] = adapted_param[:, src_idx, :]
                                    adaptation_details.append(
                                        f"Layer {k}: Replicated input channels from {p_in} to {n_in}"
                                    )
                                
                                adapted_weights[k] = adapted_param
                            
                            # Handle bias terms and BatchNorm parameters - these are 1D and easier to adapt
                            elif any(term in k for term in ['.bias', '.running_mean', '.running_var']):
                                p_size = pretrained_param.shape[0]
                                n_size = new_param.shape[0]
                                
                                if p_size >= n_size:
                                    # Take first n_size elements
                                    adapted_weights[k] = pretrained_param[:n_size]
                                    if p_size > n_size:
                                        adaptation_details.append(
                                            f"Layer {k}: Trimmed from size {p_size} to {n_size}"
                                        )
                                else:
                                    # Need to expand
                                    adapted_param = torch.zeros_like(new_param)
                                    adapted_param[:p_size] = pretrained_param
                                    
                                    # For remaining elements, replicate cyclically
                                    for i in range(p_size, n_size):
                                        adapted_param[i] = pretrained_param[i % p_size]
                                    
                                    adapted_weights[k] = adapted_param
                                    adaptation_details.append(
                                        f"Layer {k}: Expanded from size {p_size} to {n_size}"
                                    )
                            else:
                                # Other parameters - use random initialization
                                print(f"Layer {k}: Keeping random initialization due to incompatible shapes")
                                # Don't add to adapted_weights, which means it keeps random init
                    else:
                        print(f"Layer {k} not found in pretrained model, using random initialization")
            ...
            if mismatch_count > 0:
                print(f"Adapted {mismatch_count} layers with mismatched parameters")
            
            print(f"Transfer learning mode: Transferred {len(adapted_weights)} convolutional layers")
            
            # Update model with adapted weights
            model_dict.update(adapted_weights)
        else:
            # For inference, try to load all layers (with error messages for mismatches)
            print("Inference mode: Attempting to load all layers from pretrained model")
            try:
                model.load_state_dict(pretrained_weights)
                print("All layers loaded successfully")
            except Exception as e:
                print(f"Error loading full model: {str(e)}")
                print("Falling back to partial loading with compatible layers only")
                
                # Filter and load only compatible layers
                compatible_weights = {k: v for k, v in pretrained_weights.items() 
                                     if k in model_dict and v.shape == model_dict[k].shape}
                model_dict.update(compatible_weights)
                print(f"Loaded {len(compatible_weights)}/{len(pretrained_weights)} compatible layers")
        
        # Load the final state into the model
        model.load_state_dict(model_dict, strict=False)
        
        # Freeze convolutional layers if specified
        if args.mode == "transfer" and args.freeze_layers:
            ...
            print(f"Frozen {frozen_count} convolutional layer parameters")
...
def main():
    ...
    learning_rate = args.lr if args.mode != "transfer" else args.transfer_lr
    act = args.act
    opt = args.opt
    sch = args.sch
    wd = args.wd
   ...
   # Split data
    ...
    # Use the new function to initialize model based on selected mode
    model = initialize_model(args, input_size, device)
    ...
    hyperparameters = {
        'Exp_ID': id,
        'Mode': args.mode,
        'Pretrained_Model': args.pretrained_model if args.mode in ["transfer"] else "None",
        'BS': batch_size,
        'Epochs': num_epochs,
