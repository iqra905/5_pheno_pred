...
def parse_args():
    ...
    parser.add_argument("-init_from_pretrained", type=int, default=1, choices=[0, 1], help="Whether to initialize from pretrained model")
    ...
    parser.add_argument("-use_class_weights", type=int, default=1, choices=[0, 1], help="Whether to use class weights in loss function (0: no, 1: yes)")
    parser.add_argument("-class_weight_method", type=str, default="inverse", choices=["inverse", "sqrt_inverse", "log_inverse", "effective"], help="Method to calculate class weights")
    parser.add_argument("-use_weighted_sampler", type=int, default=0, choices=[0, 1], help="Whether to use weighted random sampler (0: no, 1: yes)")
    parser.add_argument("-sampler_weight_multiplier", type=float, default=5.0, help="Multiplier for positive class weights in sampler")
    parser.add_argument("-gradient_clip_norm", type=float, default=1.0, help="Max norm for gradient clipping (0 to disable)")
    
    return parser.parse_args()

def calculate_class_weights(phenotype_data, disease_labels, method="inverse"):
    """
    Calculate class weights for imbalanced datasets
    
    Args:
        phenotype_data: DataFrame with disease labels
        disease_labels: List of disease column names
        method: Weight calculation method
            - 'inverse': neg_count / pos_count
            - 'sqrt_inverse': sqrt(neg_count / pos_count)
            - 'log_inverse': log(neg_count / pos_count)
            - 'effective': effective number of samples
    
    Returns:
        dict: Disease name -> weight value
    """
    weights = {}
    
    print(" CALCULATING CLASS WEIGHTS")
    print(f"Method: {method}")
    print(f"\n{'Disease':<15} {'Pos':<8} {'Neg':<8} {'Ratio':<10} {'Weight':<10}")
    
    for disease in disease_labels:
        pos_count = int(phenotype_data[disease].sum())
        neg_count = len(phenotype_data) - pos_count
        total = len(phenotype_data)
        
        if pos_count == 0:
            print(f"  Warning: {disease} has no positive samples!")
            weights[disease] = 1.0
            continue
        ...
        if method == "inverse":
            ...
            # Softer weighting (better for extreme imbalance)
            weight = np.sqrt(neg_count / pos_count)
            
        elif method == "log_inverse":
            # Even softer weighting
            weight = np.log1p(neg_count / pos_count)
            
        elif method == "effective":
            # Effective number of samples (from Class-Balanced Loss paper)
            beta = 0.9999
            effective_num_pos = (1 - beta**pos_count) / (1 - beta)
            effective_num_neg = (1 - beta**neg_count) / (1 - beta)
        ...
        print(f"{disease:<15} {pos_count:<8} {neg_count:<8} {ratio:<10.2f} {weight:<10.2f}")
        
    return weights


def create_weighted_sampler(dataset, disease_labels, weight_multiplier=5.0):
    """
    Create weighted random sampler that oversamples minority class
    
    Args:
        dataset: Dataset object
        disease_labels: List of disease names
        weight_multiplier: How much to oversample positive samples (5.0 = 5x more likely)
    
    Returns:
        WeightedRandomSampler
    """
    print("  CREATING WEIGHTED SAMPLER")
    print(f"Weight multiplier for positive samples: {weight_multiplier}x")
    ...
    print(f"\n{'Disease':<15} {'Pos Samples':<15} {'Weight Boost'}")
    
    for i, disease in enumerate(disease_labels):
        pos_mask = all_labels[:, i] == 1
        num_pos = pos_mask.sum()
        
        # Increase weight for samples with this positive disease
        sample_weights[pos_mask] *= weight_multiplier
        
        print(f"{disease:<15} {num_pos:<15} {weight_multiplier}x")
...
class UnifiedSequenceBlock(nn.Module):
    ...
    def _forward_hybrid(self, x):
        """Forward pass for hybrid mode with interleaved layers"""
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if phase == 'train':
                            ...
                            if gradient_clip_norm > 0:
                                ...
                                # if total_norm > gradient_clip_norm * 5:
                                #     print(f"  Large gradient: {total_norm:.2f} (clipped to {gradient_clip_norm})")
...
def main():
    ...
    # Set up loss function with optional class weights (BCELoss for multilabel)
    if args.use_class_weights:
        class_weights = calculate_class_weights(phenotype_data, disease_labels, method=args.class_weight_method)
        pos_weights = torch.tensor([class_weights[disease] for disease in disease_labels], dtype=torch.float32).to(device)
        criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weights)
        print(f" Using weighted BCE loss with {args.class_weight_method} class weights")
    else:
    ...
    # Create dataloaders with optional weighted sampler
    if args.use_weighted_sampler:
        train_sampler = create_weighted_sampler(
            train_dataset, 
            disease_labels, 
            weight_multiplier=args.sampler_weight_multiplier
        )
        ...
        print(" Using standard random shuffling")
