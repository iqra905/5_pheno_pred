...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_01_tra", help="ID of the experiment")
    ...
    parser.add_argument("-fc_layers", type=parse_int_list, default=[64], help="Fully connected layers")
    ...
    parser.add_argument("-pointwise_channels", type=int, default=32, help="Number of output channels for pointwise convolution (applied to each branch)")
    ...
    parser.add_argument("-pool_size", type=int, default=256, help="Size of the adaptive pooling output (smaller values = more aggressive pooling)")
    ...
    parser.add_argument("-pretrained_model_name", type=str, default="WinKawaks/vit-small-patch16-224", help="Name of pretrained model for weight initialization")
...
class GenomicTransformerBlock(nn.Module):
    ...
    def __init__(self, input_dim, transformer_dim, num_layers=2, num_heads=4, 
        ...
        #print(f"  Creating custom transformer (training from scratch)")
        #print(f"  - Transformer layers: {num_layers}")
        #print(f"  - Transformer heads: {num_heads}")
        #print(f"  - Transformer dimension: {transformer_dim}")
        #print(f"  - Input to transformer: (batch_size, {input_dim}, seq_len)")
    ...
    def _copy_layer_weights(self, pretrained_layer, custom_layer, layer_idx):
        ...
        try:
            ...
            #print(f"     Successfully adapted and copied layer norm weights")
            ...
            #print(f"    Successfully adapted and copied feedforward weights")
...
class MultilabelGenotypeModel(nn.Module):
    def __init__(self, input_size, num_diseases, kernel_sizes, stride, conv_channels, fc_layers, act, dropout_rate, 
        ...
        if use_transformer:
            architecture_info.append(f"Custom transformer: {transformer_layers} layers, {transformer_heads} heads, dim={transformer_dim}")
            ...
            if init_from_pretrained:
                ...
                #architecture_info.append(f"Layers to initialize: {init_layers_fraction * 100:.1f}%")
