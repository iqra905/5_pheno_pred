...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="1_mean", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/mnt/fast/nobackup/users/if00208/5_disease_experiments/CNN/results/5d_multilabel/multiscale/transformer', help="Directory to save experiment results")
    ...
    parser.add_argument("-transformer_dropout", type=float, default=0.2, help="Transformer dropout rate")
    ...
    parser.add_argument("-pooling_strategy", type=str, default="mean", choices=["mean", "max", "attention", "concat", "chunked", "multiscale", "multihead", "conv", "hierarchical"], help="Pooling strategy for transformer output")
    ...
    parser.add_argument("-multihead_head_dim", type=int, default=64, help="Dimension of each attention head")
    ...
    parser.add_argument("-pretrained_model_path", type=str, default=None, help="Path to pretrained model checkpoint for transfer learning")
    parser.add_argument("-freeze_conv_layers", type=int, default=0, choices=[0, 1], help="Freeze convolutional layers during transfer learning")
    parser.add_argument("-freeze_fc_layers", type=int, default=0, choices=[0, 1], help="Freeze FC layers (except final) during transfer learning")
    parser.add_argument("-freeze_transformer", type=int, default=0, choices=[0, 1], help="Freeze transformer layers during transfer learning")
    parser.add_argument("-transfer_learning_lr", type=float, default=0.0001, help="Learning rate for transfer learning (usually lower)")
    parser.add_argument("-conv_channel_adaptation", type=str, default="average", choices=["average", "first", "replicate", "skip"], help="How to handle conv input channel mismatch")
    parser.add_argument("-fc_adaptation", type=str, default="interpolate", choices=["interpolate", "truncate", "random_projection", "skip"], help="How to handle FC layer input size mismatch")    
    
    return parser.parse_args()

def adapt_conv_channels(pretrained_weight, target_in_channels, adaptation_method="average"):
    """
    Adapt pretrained conv weights to different input channel count
    
    Args:
        pretrained_weight: [out_channels, in_channels, kernel_size]
        target_in_channels: desired input channels
        adaptation_method: "average", "first", "replicate"
    """
    ...
    print(f"    Adapting conv channels: {pretrained_in_channels} -> {target_in_channels} using '{adaptation_method}'")
    
    if adaptation_method == "average":
        if pretrained_in_channels > target_in_channels:
            # Average multiple channels into fewer
            if target_in_channels == 1:
                return pretrained_weight.mean(dim=1, keepdim=True)
            else:
                # More complex averaging for other cases
                channels_per_group = pretrained_in_channels // target_in_channels
                adapted = []
                for i in range(target_in_channels):
                    start_idx = i * channels_per_group
                    end_idx = start_idx + channels_per_group
                    if i == target_in_channels - 1:  # Last group gets remaining channels
                        end_idx = pretrained_in_channels
                    group_avg = pretrained_weight[:, start_idx:end_idx, :].mean(dim=1, keepdim=True)
                    adapted.append(group_avg)
                return torch.cat(adapted, dim=1)
        else:
            # Replicate channels to get more
            repeat_factor = target_in_channels // pretrained_in_channels
            remainder = target_in_channels % pretrained_in_channels
            adapted = pretrained_weight.repeat(1, repeat_factor, 1)
            if remainder > 0:
                adapted = torch.cat([adapted, pretrained_weight[:, :remainder, :]], dim=1)
            return adapted
            
    elif adaptation_method == "first":
        # Take only the first channel(s)
        return pretrained_weight[:, :target_in_channels, :]
        
    elif adaptation_method == "replicate":
        # Replicate the first channel
        if target_in_channels > pretrained_in_channels:
            first_channel = pretrained_weight[:, 0:1, :]  # Take first channel
            return first_channel.repeat(1, target_in_channels, 1)
        else:
            return pretrained_weight[:, :target_in_channels, :]
...
def interpolate_fc_weights(weight, target_input_size):
    """Interpolate FC weights using 1D interpolation"""
    import torch.nn.functional as F
    
    current_input_size = weight.shape[1]
    output_size = weight.shape[0]
    
    # Reshape for interpolation: [output_size, 1, input_size]
    ...
    # Reshape back: [output_size, target_input_size]
    return adapted_weight.squeeze(1)

def adapt_fc_layer(pretrained_weight, pretrained_bias, target_input_size, method="interpolate"):
    """
    Adapt FC layer weights to different input size
    
    Args:
        pretrained_weight: [output_size, input_size] 
        pretrained_bias: [output_size] or None
        target_input_size: desired input size
        method: adaptation strategy
    
    Returns:
        adapted_weight, adapted_bias
    """
    pretrained_input_size = pretrained_weight.shape[1]
    output_size = pretrained_weight.shape[0]
    
    if pretrained_input_size == target_input_size:
        return pretrained_weight, pretrained_bias
    
    print(f"    Adapting FC layer: input {pretrained_input_size} -> {target_input_size} using '{method}'")
    
    if method == "interpolate":
        # Interpolate weights to match new input size
        adapted_weight = interpolate_fc_weights(pretrained_weight, target_input_size)
        return adapted_weight, pretrained_bias
        
    elif method == "truncate":
        # Truncate or pad with zeros
        if target_input_size <= pretrained_input_size:
            # Truncate
            adapted_weight = pretrained_weight[:, :target_input_size]
        else:
            # Pad with small random values
            pad_size = target_input_size - pretrained_input_size
            padding = torch.randn(output_size, pad_size) * 0.01  # Small random values
            adapted_weight = torch.cat([pretrained_weight, padding], dim=1)
        return adapted_weight, pretrained_bias
        
    elif method == "random_projection":
        # Use a random projection to map from new size to pretrained size
        # Then apply pretrained weights
        projection = torch.randn(pretrained_input_size, target_input_size) * (1.0 / math.sqrt(target_input_size))
        adapted_weight = pretrained_weight @ projection
        return adapted_weight, pretrained_bias
        
    elif method == "skip":
        # Don't transfer this layer - return None to skip
        return None, None
        
    else:
        raise ValueError(f"Unknown FC adaptation method: {method}")

def handle_concat_pooling_mismatch(model, pretrained_state_dict, current_state_dict, 
                                   fc_adaptation_method="interpolate"):
    """
    Special handling for concat pooling with different sequence lengths
    Detects and adapts the first FC layer to handle different flattened sizes
    """
    # Try different naming conventions for the first FC layer
    possible_names = [
        'shared_layers.0.weight',  # Current naming
        'fc_shared.0.weight',       # Older naming
        'fc_layers.0.weight',       # Alternative naming
    ]
    
    first_fc_name = None
    pretrained_fc_input = None
    current_fc_input = None
    current_fc_name = None
    
    # Find the first FC layer in pretrained model
    for name in possible_names:
        if name in pretrained_state_dict:
            first_fc_name = name
            pretrained_fc_input = pretrained_state_dict[name].shape[1]
            break
    ...
    # Find corresponding layer in current model
    for name in possible_names:
        if name in current_state_dict:
            current_fc_name = name
            current_fc_input = current_state_dict[name].shape[1]
            break
    
    if current_fc_name is None:
        print("  No FC layer found in current model")
        return None
    
    if pretrained_fc_input == current_fc_input:
        return None  # No size mismatch
    
    print(f"\n  CONCAT POOLING DETECTED - Sequence length mismatch:")
    
    # Calculate sequence lengths (assuming transformer_dim is consistent)
    if hasattr(model, 'transformer_block') and model.transformer_block is not None:
        transformer_dim = model.transformer_block.transformer_dim
    else:
        # Try to infer from the dimensions
        # Common transformer_dim values: 256, 384, 512, 768
        for dim in [384, 256, 512, 768]:
            if pretrained_fc_input % dim == 0 and current_fc_input % dim == 0:
                transformer_dim = dim
                break
        else:
            transformer_dim = 384  # fallback
    
    pretrained_seq_len = pretrained_fc_input // transformer_dim
    current_seq_len = current_fc_input // transformer_dim
    
    print(f"    Pretrained FC layer: {first_fc_name}")
    print(f"    Current FC layer: {current_fc_name}")
    print(f"    Pretrained: seq_len={pretrained_seq_len}, input_size={pretrained_fc_input}")
    print(f"    Current: seq_len={current_seq_len}, input_size={current_fc_input}")
    print(f"    Transformer dim: {transformer_dim}")
    print(f"    Adaptation method: {fc_adaptation_method}")
    
    # Get pretrained FC layer weights and bias
    pretrained_weight = pretrained_state_dict[first_fc_name]
    bias_name = first_fc_name.replace('weight', 'bias')
    pretrained_bias = pretrained_state_dict.get(bias_name)
    
    # Adapt the weights
    adapted_weight, adapted_bias = adapt_fc_layer(
        pretrained_weight, 
        pretrained_bias, 
        current_fc_input,
        method=fc_adaptation_method
    )
    
    if adapted_weight is not None:
        return {
            'pretrained_name': first_fc_name,
            'current_name': current_fc_name,
            'weight_name': current_fc_name,  # Use current model's naming
            'bias_name': current_fc_name.replace('weight', 'bias'),
...
def load_pretrained_model_for_transfer(model, pretrained_path, freeze_conv=False, 
                                      freeze_fc=False, freeze_transformer=False,
                                      device='cuda', fc_adaptation_method="interpolate", 
                                      channel_adaptation_method="average"):
    """
    Load pretrained weights and adapt model for transfer learning
    Handles the unified transformer-enhanced architecture with automatic name mapping
    """
    print("LOADING PRETRAINED MODEL FOR TRANSFER LEARNING")
    print(f"Pretrained model path: {pretrained_path}")
    print(f"Channel adaptation: {channel_adaptation_method}")
    print(f"FC adaptation: {fc_adaptation_method}")
    print(f"Freeze conv: {freeze_conv}, FC: {freeze_fc}, Transformer: {freeze_transformer}")
    ...
    try:
        ...
        if 'best_model_state_dict' in checkpoint:
            pretrained_state_dict = checkpoint['best_model_state_dict']
            print("Using best model weights from checkpoint")
        elif 'model_state_dict' in checkpoint:
            pretrained_state_dict = checkpoint['model_state_dict']
            print("Using regular model weights from checkpoint")
        else:
    ...
    # Define name mappings (old -> new)
    name_mapping = {
        'fc_shared': 'shared_layers',
        'disease_outputs': 'output_layer',
        'disease_heads_list': 'disease_heads',
    }
    
    # STEP 1: Detect and handle concat pooling mismatch
    print("\nStep 1: Checking for concat pooling sequence length mismatch...")
    concat_adapted_layers = {}
    
    # Find first FC layer in both models
    fc_patterns = ['shared_layers.0.weight', 'fc_shared.0.weight', 'fc_layers.0.weight']
    pretrained_fc_name = None
    pretrained_fc_weight = None
    current_fc_name = None
    current_fc_weight = None
    
    for pattern in fc_patterns:
        if pattern in pretrained_state_dict and pretrained_fc_weight is None:
            pretrained_fc_name = pattern
            pretrained_fc_weight = pretrained_state_dict[pattern]
        
        if pattern in current_state_dict and current_fc_weight is None:
            current_fc_name = pattern
            current_fc_weight = current_state_dict[pattern]
    
    if pretrained_fc_weight is not None and current_fc_weight is not None:
        pretrained_fc_input = pretrained_fc_weight.shape[1]
        current_fc_input = current_fc_weight.shape[1]
        
        if pretrained_fc_input != current_fc_input:
            print(f"  CONCAT POOLING MISMATCH DETECTED!")
            
            # Infer transformer_dim
            if hasattr(model, 'transformer_block') and model.transformer_block is not None:
                transformer_dim = model.transformer_block.transformer_dim
            else:
                # Try common dimensions
                for dim in [384, 256, 512, 768, 128, 1024]:
                    if pretrained_fc_input % dim == 0 and current_fc_input % dim == 0:
                        transformer_dim = dim
                        break
                else:
            ...
            print(f"    Pretrained FC: {pretrained_fc_name}, input={pretrained_fc_input} (seq={pretrained_seq_len} × dim={transformer_dim})")
            print(f"    Current FC: {current_fc_name}, input={current_fc_input} (seq={current_seq_len} × dim={transformer_dim})")
            print(f"    Adapting using method: {fc_adaptation_method}")
            
            # Adapt weights
            pretrained_bias_name = pretrained_fc_name.replace('weight', 'bias')
            pretrained_bias = pretrained_state_dict.get(pretrained_bias_name)
            
            adapted_weight, adapted_bias = adapt_fc_layer(
                pretrained_fc_weight,
                pretrained_bias,
                current_fc_input,
                method=fc_adaptation_method
            )
            ...
            if adapted_weight is not None:
                # Store adapted layers
                concat_adapted_layers[pretrained_fc_name] = {
                    'target_name': current_fc_name,
                    'weight': adapted_weight,
                    'bias': adapted_bias,
                    'bias_name': pretrained_bias_name
                }
                print(f"    ✓ Successfully adapted FC layer")
            else:
                print(f"    ✗ Adaptation returned None")
        else:
            print(f"  No size mismatch detected (both have input size {pretrained_fc_input})")
    else:
        print(f"  Could not find FC layers for comparison")
    
    # STEP 2: Load and adapt all layers
    print("\nStep 2: Loading and adapting layers...")
    loaded_layers = []
    skipped_layers = []
    adapted_layers = []
    ...
    for name, param in pretrained_state_dict.items():
        ...
        for old_prefix, new_prefix in name_mapping.items():
            if name.startswith(old_prefix):
                mapped_name = name.replace(old_prefix, new_prefix, 1)
                break
        
        # Handle concat-adapted FC layer
        if name in concat_adapted_layers:
            adaptation_info = concat_adapted_layers[name]
            current_state_dict[adaptation_info['target_name']] = adaptation_info['weight']
            adapted_layers.append(f"{name} -> {adaptation_info['target_name']} (concat pooling: {param.shape[1]}->{adaptation_info['weight'].shape[1]})")
            continue
        
        # Handle concat-adapted FC bias
        if name in [info['bias_name'] for info in concat_adapted_layers.values()]:
            for adapted_name, info in concat_adapted_layers.items():
                if name == info['bias_name'] and info['bias'] is not None:
                    target_bias_name = info['target_name'].replace('weight', 'bias')
                    current_state_dict[target_bias_name] = info['bias']
                    adapted_layers.append(f"{name} -> {target_bias_name} (concat pooling bias)")
                    break
            continue
        
        # Check if mapped name exists in current model
        if mapped_name not in current_state_dict:
            skipped_layers.append(f"{name} (not found in current model)")
            continue
        
        current_param = current_state_dict[mapped_name]
        
        # Exact shape match - direct copy
        if param.shape == current_param.shape:
            current_state_dict[mapped_name] = param
            if mapped_name != name:
                loaded_layers.append(f"{name} -> {mapped_name}")
            else:
                loaded_layers.append(name)
            continue
        
        # Shape mismatch - try adaptations
        
        # Skip output layers (task-specific)
        if any(keyword in name for keyword in ['output_layer', 'disease_outputs', 'disease_heads']):
            skipped_layers.append(f"{name} (output layer - task specific)")
            continue
        
        # First conv layer input channel adaptation
        if 'conv_layers.0' in name and 'weight' in name and len(param.shape) == 3:
            if channel_adaptation_method != "skip":
                try:
                    adapted_weight = adapt_conv_channels(
                        param, 
                        current_param.shape[1], 
                        adaptation_method=channel_adaptation_method
                    )
                    current_state_dict[mapped_name] = adapted_weight
                    adapted_layers.append(f"{name} (conv channels: {param.shape[1]}->{current_param.shape[1]})")
                except Exception as e:
                    skipped_layers.append(f"{name} (conv adaptation failed: {e})")
            else:
                skipped_layers.append(f"{name} (conv - skipped by user)")
            continue
        
        # Multi-scale conv branches
        if 'branches' in name and 'conv_layers' in name and 'weight' in name and len(param.shape) == 3:
            if channel_adaptation_method != "skip":
                try:
                    adapted_weight = adapt_conv_channels(
                        param,
                    ...
                    current_state_dict[mapped_name] = adapted_weight
                    adapted_layers.append(f"{name} (multi-scale conv: {param.shape[1]}->{current_param.shape[1]})")
                except Exception as e:
                    skipped_layers.append(f"{name} (multi-scale conv adaptation failed: {e})")
            else:
                skipped_layers.append(f"{name} (multi-scale conv - skipped)")
            continue
        
        # Transformer input projection adaptation
        if 'transformer_block' in name and 'input_projection' in name and 'weight' in name:
            if fc_adaptation_method != "skip":
                try:
                    ...
                    if adapted_weight is not None:
                        current_state_dict[mapped_name] = adapted_weight
                        adapted_layers.append(f"{name} (transformer input: {param.shape[1]}->{current_param.shape[1]})")
                        
                        # Handle bias
                        mapped_bias_name = bias_name
                        for old_prefix, new_prefix in name_mapping.items():
                            if bias_name.startswith(old_prefix):
                                mapped_bias_name = bias_name.replace(old_prefix, new_prefix, 1)
                                break
                        
                        if mapped_bias_name in current_state_dict and adapted_bias is not None:
                            current_state_dict[mapped_bias_name] = adapted_bias
                            adapted_layers.append(f"{bias_name} (transformer bias)")
                    else:
                        skipped_layers.append(f"{name} (transformer projection - adaptation returned None)")
                except Exception as e:
                    skipped_layers.append(f"{name} (transformer projection adaptation failed: {e})")
            else:
                skipped_layers.append(f"{name} (transformer projection - skipped)")
            continue
        
        # Generic FC layer adaptation (not first layer, already handled)
        if 'weight' in name and len(param.shape) == 2 and len(current_param.shape) == 2:
            if param.shape[0] == current_param.shape[0] and param.shape[1] != current_param.shape[1]:
                # Input dimension mismatch, output dimension matches
                if fc_adaptation_method != "skip":
                    try:
                        bias_name = name.replace('weight', 'bias')
                        pretrained_bias = pretrained_state_dict.get(bias_name)
                        ...
                        if adapted_weight is not None:
                            current_state_dict[mapped_name] = adapted_weight
                            adapted_layers.append(f"{name} (FC: {param.shape[1]}->{current_param.shape[1]})")
                            ...
                            if adapted_bias is not None:
                                ...
                                for old_prefix, new_prefix in name_mapping.items():
                                    if bias_name.startswith(old_prefix):
                                        mapped_bias_name = bias_name.replace(old_prefix, new_prefix, 1)
                                        break
                                if mapped_bias_name in current_state_dict:
                                    current_state_dict[mapped_bias_name] = adapted_bias
                    except Exception as e:
                        skipped_layers.append(f"{name} (FC adaptation failed: {e})")
                else:
                    skipped_layers.append(f"{name} (FC - skipped)")
                continue
        
        # Couldn't adapt - skip
        skipped_layers.append(f"{name} (shape mismatch: {param.shape} vs {current_param.shape})")
    ...
    print(f"  Total pretrained layers: {len(pretrained_state_dict)}")
    print(f"  Loaded unchanged: {len(loaded_layers)}")
    print(f"  Adapted: {len(adapted_layers)}")
    ...
    if skipped_layers:
        print(f"\n  Skipped layers (showing first 15):")
        for layer in skipped_layers[:15]:
            print(f"    - {layer}")
        if len(skipped_layers) > 15:
            print(f"    ... and {len(skipped_layers) - 15} more")
    
    # STEP 3: Apply freezing strategy
    print("\nStep 3: Applying freezing strategy...")
    frozen_params = 0
    trainable_params = 0
    
    for name, param in model.named_parameters():
        freeze_this_param = False
        
        # Freeze convolutional layers
        if freeze_conv and 'conv_layers' in name:
            freeze_this_param = True
        
        # Freeze FC layers (except output)
        if freeze_fc and 'shared_layers' in name:
            freeze_this_param = True
        
        # Freeze transformer
        if freeze_transformer and 'transformer_block' in name:
            freeze_this_param = True
        
        # Never freeze output layers
        if any(keyword in name for keyword in ['output_layer', 'disease_heads']):
            freeze_this_param = False
        
        param.requires_grad = not freeze_this_param
    ...
    print(f"\n  Parameter summary:")
    print(f"    Frozen: {frozen_params:,} ({frozen_params/(frozen_params+trainable_params)*100:.1f}%)")
    print(f"    Trainable: {trainable_params:,} ({trainable_params/(frozen_params+trainable_params)*100:.1f}%)")
    print(f"    Total: {frozen_params + trainable_params:,}")
    
    if freeze_conv or freeze_fc or freeze_transformer:
        print(f"\n  Freezing strategy:")
        print(f"    Conv layers: {'FROZEN ' if freeze_conv else 'trainable '}")
        print(f"    FC layers: {'FROZEN ' if freeze_fc else 'trainable '}")
        print(f"    Transformer: {'FROZEN ' if freeze_transformer else 'trainable '}")
        print(f"    Output layers: Always trainable")
    else:
        print(f"    All layers trainable (no freezing)")
...
class WarmupExponential:
    ...
    def get_lr(self):
        if self.current_step >= self.t_total:
            ...
            # Warmup phase
            return self.start_lr * math.exp(
                math.log(self.peak_lr / self.start_lr) * (self.current_step / self.warmup_steps)
            )
        else:
            # Decay phase
...
class CovariateNormalizer:
    """Flexible covariate normalization"""
...
def main():
    ...
    if args.pretrained_model_path:
        model = load_pretrained_model_for_transfer(
            model,
            args.pretrained_model_path,
            freeze_conv=bool(args.freeze_conv_layers),
            freeze_fc=bool(args.freeze_fc_layers),
            freeze_transformer=bool(args.freeze_transformer),
            device=device,
            fc_adaptation_method=args.fc_adaptation,
            channel_adaptation_method=args.conv_channel_adaptation
        )
        
        # Use lower learning rate for transfer learning
        if args.transfer_learning_lr:
            learning_rate = args.transfer_learning_lr
            print(f"Using transfer learning rate: {learning_rate}")
        else:
    ...
    optimizer_map = {
        ...
        "adamw": optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=args.wd),
        "rmsprop": optim.RMSprop(model.parameters(), lr=learning_rate),
        "sgd": optim.SGD(model.parameters(), lr=learning_rate),
        "adam": optim.Adam(model.parameters(), lr=learning_rate, weight_decay=args.wd)
    }
    ...
    hyperparameters = {
        ...
        # Add after existing hyperparameters
        'Transfer_Learning': bool(args.pretrained_model_path),
        'Pretrained_Path': args.pretrained_model_path if args.pretrained_model_path else 'N/A',
        'Freeze_Conv': bool(args.freeze_conv_layers) if args.pretrained_model_path else 'N/A',
        'Freeze_FC': bool(args.freeze_fc_layers) if args.pretrained_model_path else 'N/A',
        'Freeze_Transformer': bool(args.freeze_transformer) if args.pretrained_model_path else 'N/A',
        'Channel_Adaptation': args.conv_channel_adaptation if args.pretrained_model_path else 'N/A',
        'FC_Adaptation': args.fc_adaptation if args.pretrained_model_path else 'N/A',
        'Transfer_LR': args.transfer_learning_lr if args.pretrained_model_path else 'N/A',
