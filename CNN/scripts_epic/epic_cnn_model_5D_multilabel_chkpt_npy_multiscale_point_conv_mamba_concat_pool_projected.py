...
def parse_args():
    ...
    parser.add_argument("-pooling_strategy", type=str, default="mean", choices=["mean", "max", "attention", "concat", "chunked", "multiscale", "multihead", "conv", "hierarchical"], help="Pooling strategy for final sequence representation")
    ...
    # Channel projection parameters after sequence modeling
    parser.add_argument("-use_channel_projection", type=int, default=0, choices=[0, 1], help="Whether to use pointwise conv to reduce Mamba channels (0: no, 1: yes)")
    parser.add_argument("-projection_channels", type=int, default=128, help="Number of channels after pointwise projection (if enabled)")
    parser.add_argument("-projection_location", type=str, default="after_pooling", choices=["before_pooling", "after_pooling"], help="Where to apply channel projection: before or after pooling")
...
def calculate_pooling_output_dim(pooling_strategy, d_model, pooling_kwargs, 
                                 use_projection=False, projection_channels=None):
    """
    Calculate output dimension for different pooling strategies.
    
    Returns:
        tuple: (num_elements, channels_after_projection)
        
    Where:
        - num_elements: Number of sequence elements after pooling (L_updated)
        - channels_after_projection: Channels per element (d_model or projection_channels)
    """
    # Calculate number of elements based on pooling strategy
    if pooling_strategy == "chunked":
        num_elements = pooling_kwargs['num_chunks']
    elif pooling_strategy == "multiscale":
        num_elements = len(pooling_kwargs['window_sizes'])
    elif pooling_strategy == "multihead":
        num_elements = pooling_kwargs['num_heads']
    elif pooling_strategy == "conv":
        num_elements = pooling_kwargs['target_length']
    elif pooling_strategy == "hierarchical":
        num_elements = len(pooling_kwargs['levels'])
    elif pooling_strategy in ["mean", "max", "attention"]:
        num_elements = 1
    else:  # concat
        return None, None  # Will be seq_len (calculated dynamically)
    
    # Determine final channels per element
    if use_projection:
        channels = projection_channels
    else:
        channels = d_model
    
    return num_elements, channels

# ============================================================================
# UPDATED POOLING CLASSES - All return [batch, N, d_model] unflattened
# ============================================================================

class ChunkedPooling(nn.Module):
    """Efficient non-overlapping chunked pooling - returns structured output"""
    ...
    def forward(self, x):
        ...
        if seq_len < self.num_chunks:
            ...
            return x  # [batch_size, num_chunks, d_model] - UNFLATTENED
...
class MultiScalePooling(nn.Module):
    """Apply different pooling window sizes - returns structured output"""
    def __init__(self, d_model, window_sizes=[4, 8, 16, 32, 64, 128, 256]):
    ...
    def forward(self, x):
        ...
        output = torch.stack(scale_features, dim=1)  # [batch, num_scales, d_model]
        
        return output  # UNFLATTENED ✓

class HierarchicalPooling(nn.Module):
    """Progressive abstraction levels - returns structured output"""
    def __init__(self, d_model, levels=[1024, 512, 256, 128, 64, 32]):
    ...
    def forward(self, x):
        ...
        # Stack instead of concat
        output = torch.stack(hierarchical_features, dim=1)  # [batch, num_levels, d_model]
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        return weighted # [batch_size, input_dim]

class MultiHeadPooling(nn.Module):
    """Multiple attention-based pooling heads - returns structured output"""
    ...
    def forward(self, x):
        ...
        for i in range(self.num_heads):
            ...
            # Apply attention - keep full d_model
            weighted = torch.sum(x * attention_weights, dim=1)  # [batch_size, d_model]
            head_outputs.append(weighted)
        
        # Stack instead of concat
        output = torch.stack(head_outputs, dim=1)  # [batch, num_heads, d_model]
        
        return output  # UNFLATTENED 

class ConvolutionalPooling(nn.Module):
    """Use 1D convolutions to intelligently downsample - returns structured output"""
    def __init__(self, d_model, target_length=512, conv_layers=1):
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2)  # [batch_size, target_length, d_model]
...
class GenomicMambaBlock(nn.Module):
    """Genomic Mamba processing block with flexible channel projection.
    
    Architecture options:
    1. No projection: 
       Mamba → Pool → Flatten → [batch, L_updated * d_model]
       
    2. Before pooling:
       Mamba → Project → Pool → Flatten → [batch, L_updated * projection_channels]
       
    3. After pooling:
       Mamba → Pool → Project → Flatten → [batch, L_updated * projection_channels]"""
    ...
    def __init__(self, input_dim, d_model, d_state=16, d_conv=4, expand=2, 
                 ...
                 use_channel_projection=False, projection_channels=128,
                 projection_location="after_pooling",
                 **pooling_kwargs):
        ...
        self.use_channel_projection = use_channel_projection
        ...
        # Channel projection BEFORE pooling (if enabled)
        self.channel_projection_before = None
        effective_d_model = d_model
        
        if use_channel_projection and projection_location == "before_pooling":
            self.channel_projection_before = nn.Sequential(
                nn.Conv1d(d_model, projection_channels, kernel_size=1),
                nn.BatchNorm1d(projection_channels),
                nn.GELU()
            )
            effective_d_model = projection_channels
            print(f"    - Channel projection BEFORE pooling: {d_model} → {projection_channels}")
        
        # Initialize pooling layers (using effective_d_model)
        if pooling_strategy == "chunked":
            self.custom_pooling = ChunkedPooling(**pooling_kwargs)
        elif pooling_strategy == "multiscale":
            self.custom_pooling = MultiScalePooling(d_model=effective_d_model, **pooling_kwargs)
        elif pooling_strategy == "multihead":
            self.custom_pooling = MultiHeadPooling(d_model=effective_d_model, **pooling_kwargs)
        elif pooling_strategy == "conv":
            self.custom_pooling = ConvolutionalPooling(d_model=effective_d_model, **pooling_kwargs)
        elif pooling_strategy == "hierarchical":
            self.custom_pooling = HierarchicalPooling(d_model=effective_d_model, **pooling_kwargs)
        elif pooling_strategy == "attention":
            self.attention_pooling = AttentionPooling(effective_d_model)
        
        self.channel_projection_after = None
        
        if use_channel_projection and projection_location == "after_pooling":
            # Pointwise convolution to reduce channels AFTER pooling
            # Operates on pooled sequence [batch, L_updated, d_model]
            self.channel_projection_after = nn.Sequential(
                nn.Conv1d(d_model, projection_channels, kernel_size=1),
                nn.BatchNorm1d(projection_channels),
                nn.GELU()
            )
            print(f"    - Channel projection AFTER pooling: {d_model} → {projection_channels}")
        
        # =================================================================
        # Calculate final output dimensions
        # =================================================================
        num_elements, final_channels = calculate_pooling_output_dim(
            pooling_strategy, d_model, pooling_kwargs,
            use_channel_projection, projection_channels
        )
        
        if num_elements is not None:
            self.num_pooled_elements = num_elements
            self.final_channels = final_channels
            self.final_output_dim = num_elements * final_channels
            
            print(f"  GenomicMambaBlock output structure:")
            print(f"    - Pooled elements (L_updated): {num_elements}")
            print(f"    - Channels per element: {final_channels}")
            print(f"    - Final flattened dimension: {self.final_output_dim}")
        else:
            # Concat strategy - dimensions depend on sequence length
            self.num_pooled_elements = None
            self.final_channels = final_channels if use_channel_projection else d_model
            self.final_output_dim = None
            print(f"  GenomicMambaBlock output structure:")
            print(f"    - Pooled elements (L_updated): Variable (concat strategy)")
            print(f"    - Channels per element: {self.final_channels}")
            print(f"    - Final dimension: seq_len × {self.final_channels}")
    ...
    def forward(self, x, covariates=None):
        ...
        Returns:
            Flattened features: [batch_size, final_output_dim]
        """
        ...
        # Apply channel projection BEFORE pooling if enabled
        if self.channel_projection_before is not None:
            # Transpose for Conv1d: [batch, seq_len, d_model] → [batch, d_model, seq_len]
            x = x.transpose(1, 2)
            x = self.channel_projection_before(x)  # [batch, projection_channels, seq_len]
            # Transpose back: [batch, projection_channels, seq_len] → [batch, seq_len, projection_channels]
            x = x.transpose(1, 2)
        
        # Apply pooling - returns [batch_size, N, channels]
        if self.pooling_strategy in ["chunked", "multiscale", "multihead", "conv", "hierarchical"]:
            pooled = self.custom_pooling(x)  # [batch, N, channels]
        elif self.pooling_strategy == "mean":
            pooled = torch.mean(x, dim=1).unsqueeze(1)  # [batch, 1, channels]
        elif self.pooling_strategy == "max":
            pooled, _ = torch.max(x, dim=1)
            pooled = pooled.unsqueeze(1)  # [batch, 1, channels]
        elif self.pooling_strategy == "attention":
            pooled = self.attention_pooling(x).unsqueeze(1)  # [batch, 1, channels]
        elif self.pooling_strategy == "concat":
            pooled = x  # [batch, seq_len, channels] - keep full sequence
        else:
            # Fallback to mean pooling
            pooled = torch.mean(x, dim=1).unsqueeze(1)
        
        # Apply channel projection AFTER pooling if enabled
        if self.channel_projection_after is not None:
            # Transpose for Conv1d: [batch, L_updated, d_model] → [batch, d_model, L_updated]
            pooled = pooled.transpose(1, 2)
            pooled = self.channel_projection_after(pooled)  # [batch, projection_channels, L_updated]
            # Transpose back: [batch, projection_channels, L_updated] → [batch, L_updated, projection_channels]
            pooled = pooled.transpose(1, 2)
            # Now pooled: [batch, L_updated, projection_channels]
        
        # =================================================================
        # Flatten to [batch, L_updated * channels]
        # =================================================================
        output = pooled.reshape(batch_size, -1)
...
class MultilabelGenomicMambaModel(nn.Module):
    def __init__(self, input_size, num_diseases, kernel_sizes, stride, conv_channels, fc_layers, act, dropout_rate, 
                 ...
                 use_mamba=True, mamba_layers=4, mamba_d_model=256, mamba_d_state=16, 
                 mamba_d_conv=4, mamba_expand=2, mamba_dropout=0.1, use_mamba_norm=True,
                 use_covariate_tokens=False, covariate_embed_dim=64, 
                 pooling_strategy="mean", pooling_kwargs=None,
                 use_channel_projection=False, projection_channels=128, projection_location="after_pooling"):
        ...
        if self.use_mamba:
            ...
            # Create Mamba block with projection parameters
            self.mamba_block = GenomicMambaBlock(
                ...
                use_channel_projection=use_channel_projection,
            ...
            num_elements, final_channels = calculate_pooling_output_dim(
                pooling_strategy, mamba_d_model, pooling_kwargs,
                use_channel_projection, projection_channels
            )
            
            if num_elements is not None:
                # Standard pooling strategies (mean, max, chunked, etc.)
                feature_size_for_fc = num_elements * final_channels
                
                print(f"  - Mamba → Pooling/Projection → Output:")
                print(f"    Structure: [batch, {num_elements}, {final_channels}]")
                print(f"    Flattened: [batch, {feature_size_for_fc}]")
                
            else:
                # Concat strategy (no pooling)
                if use_covariate_tokens:
                    seq_after_mamba = conv_seq_len + 1
                else:
                    seq_after_mamba = conv_seq_len
                
                if use_channel_projection:
                    feature_size_for_fc = seq_after_mamba * projection_channels
                    print(f"  - Mamba → Concat → Projection:")
                    print(f"    Structure: [batch, {seq_after_mamba}, {projection_channels}]")
                    print(f"    Flattened: [batch, {feature_size_for_fc}]")
                else:
                    feature_size_for_fc = seq_after_mamba * mamba_d_model
                    print(f"  - Mamba → Concat (no projection):")
                    print(f"    Structure: [batch, {seq_after_mamba}, {mamba_d_model}]")
                    print(f"    Flattened: [batch, {feature_size_for_fc}]")
            ...
            feature_size_for_fc = self.conv_output_info['flattened_size']
        ...
        # Print model configuration - ADD projection info
        architecture_info = []
        ...
        if use_mamba:
            architecture_info.append(f"Mamba: {mamba_layers} layers, d_model={mamba_d_model}")
            ...
            # Add projection info
            if use_channel_projection:
                architecture_info.append(f"Channel projection: {mamba_d_model} → {projection_channels} ({projection_location})")
            else:
                architecture_info.append(f"Channel projection: Disabled")
    ...
    # Forward method stays the same - no changes needed!
    def forward(self, x, covariates=None):
        ...
        if self.use_mamba:
            if self.use_covariate_tokens:
                ...
                x = self.mamba_block(x)  # Transform: (batch_size, channels, seq_len) -> (batch_size, d_model)x = self.mamba_block(x)
        else:
...
def main():
    ...
    model = MultilabelGenomicMambaModel(
        ...
        use_channel_projection=bool(args.use_channel_projection),
        projection_channels=args.projection_channels,
        projection_location=args.projection_location
    )
    ...
    hyperparameters = {
        ...
        'Use_Channel_Projection': bool(args.use_channel_projection) if args.use_mamba else 'N/A',
        'Projection_Channels': args.projection_channels if (args.use_mamba and args.use_channel_projection) else 'N/A',
        'Projection_Location': args.projection_location if (args.use_mamba and args.use_channel_projection) else 'N/A',
