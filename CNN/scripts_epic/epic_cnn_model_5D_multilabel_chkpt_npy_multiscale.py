...
def parse_args():
    ...
    parser.add_argument("-genotype_dir", type=str, default='/mnt/fast/datasets/ucdatasets/gwas/gwas_mono_rm/sampled_data_5M_unq_npy', help="Directory containing genotype files")
    ...
    parser.add_argument("-multi_scale_fusion", type=str, default="cross_scale", choices=["cross_scale", "parallel"], help="Multi-scale fusion strategy: cross_scale (branches see all scales) or parallel (independent branches)")
    ...
    parser.add_argument("-hardcoded_strides", type=parse_nested_int_list, default='4,32,128;4,32,128;4,32,128',
    ...
    parser.add_argument("-pointwise_channels", type=int, default=8, help="Number of output channels for pointwise convolution (applied to each branch)")

    # Data-specific parameters
    parser.add_argument("-cov", type=int, default=0, choices=[0, 1], help="Whether to include PC as covariates in the model (0: no, 1: yes)")
...
def main():
    ...
    if len(filtered_file_list) != len(phenotype_samples):
        print(f"Warning: Number of files ({len(filtered_file_list)}) in {args.genotype_dir} does not match number of samples ({len(phenotype_samples)}) in phenotype data.")
    ...
    model, history, final_metrics, all_preds, all_labels, completed_epochs = train_multilabel_model(
        model, dataloaders, criterion, optimizer, scheduler, args.epochs, disease_labels, 
        device, early_stopping=early_stopping,
