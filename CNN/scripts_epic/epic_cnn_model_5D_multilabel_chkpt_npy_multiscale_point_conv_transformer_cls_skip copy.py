...
"""
ENHANCED MULTILABEL GENOTYPE MODEL WITH RESIDUAL CONNECTIONS

This script implements residual connections after every convolution layer in both 
standard and multi-scale architectures:

1. STANDARD CONVOLUTION:
   Input → Conv1 + Residual1 → Conv2 + Residual2 → ... → Output
   
2. CROSS-SCALE MULTI-SCALE:
   Input → [Branch1_Conv + Residual1, Branch2_Conv + Residual2, Branch3_Conv + Residual3] → Concat
   
3. PARALLEL MULTI-SCALE:
   Layer 1: Input → [Branch1_Conv + Residual1, Branch2_Conv + Residual2, Branch3_Conv + Residual3]
   Layer 2: Prev → [Branch1_Conv + Residual1, Branch2_Conv + Residual2, Branch3_Conv + Residual3]
   Final: → Concat

Each residual connection automatically handles:
- Channel dimension mismatches (via 1x1 conv projection)
- Sequence length mismatches (via adaptive pooling)
- Optional dropout on residual paths

Usage:
  -use_residual 1              # Enable residual connections
  -residual_dropout 0.1        # Dropout rate for residual paths
"""
...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_01_sch_epoch_expo", help="ID of the experiment")
    ...
    parser.add_argument("-df", type=float, default=0.9, help="Decay factor for custom schedulers")
    ...
    parser.add_argument("-use_residual", type=int, default=0, choices=[0, 1], help="Whether to use residual connections in CNN layers (0: no, 1: yes)")
    ...
    parser.add_argument("-use_transformer", type=int, default=0, choices=[0, 1], help="Whether to use transformer layers after convolution (0: no, 1: yes)")
    ...
    parser.add_argument("-transformer_dropout", type=float, default=0.05, help="Transformer dropout rate")
    ...
    parser.add_argument("-init_from_pretrained", type=int, default=0, choices=[0, 1], help="Whether to initialize transformer weights from pretrained model (0: no, 1: yes)")
    ...
    parser.add_argument("-covariate_token_strategy", type=str, default="separate", choices=["separate", "combined"], help="Covariate tokenization strategy: 'separate' (one token per covariate) or 'combined' (single token for all covariates)")
...
class ResidualConnection(nn.Module):
    """Residual connection with automatic dimension matching"""
    ...
    def forward(self, x, residual):
        """
        Args:
            x: Output from main path [batch_size, out_channels, seq_len_out]
            residual: Input to be added as residual [batch_size, in_channels, seq_len_in]
        """
        try:
            ...
            # Project residual to match x dimensions
            projected_residual = self.projection(residual)
            
            # Handle potential sequence length mismatch due to different processing
            if projected_residual.size(2) != x.size(2):
                # Use adaptive pooling to match sequence lengths
                if x.size(2) > 0 and projected_residual.size(2) > 0:
                    projected_residual = F.adaptive_avg_pool1d(projected_residual, x.size(2))
                else:
                    # Handle edge case where sequence length is 0
                    print(f"Warning: Zero sequence length detected - x: {x.shape}, residual: {projected_residual.shape}")
                    return x
            
            # Verify shapes before addition
            if x.shape != projected_residual.shape:
                print(f"Shape mismatch: x {x.shape} vs projected_residual {projected_residual.shape}")
                # Try to fix common shape issues
                if x.size(0) == projected_residual.size(0) and x.size(1) == projected_residual.size(1):
                    # Only sequence length differs
                    projected_residual = F.adaptive_avg_pool1d(projected_residual, x.size(2))
                else:
                    print(f"Cannot fix shape mismatch, returning x without residual")
                    return x
            
            # Apply dropout and add residual connection
            dropped_residual = self.dropout(projected_residual)
            
            # Ensure contiguous memory layout before addition
            x = x.contiguous()
            dropped_residual = dropped_residual.contiguous()
            
            result = x + dropped_residual
            return result
            
        except Exception as e:
            print(f"Error in ResidualConnection forward: {e}")
            print(f"x shape: {x.shape}, residual shape: {residual.shape}")
            print(f"x device: {x.device}, residual device: {residual.device}")
            # Return x without residual connection in case of error
            return x
...
class MultiScaleConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes, strides, act, dropout_rate, pool_type="max", 
                use_pointwise_conv=False, pointwise_channels=16, is_final_layer=False, use_residual=False, residual_dropout=0.1):
        super(MultiScaleConvBlock, self).__init__()
        
        self.branches = nn.ModuleList()
        self.branch_residuals = nn.ModuleList()  # Residual connections for each branch
        ...
        for i, (kernel_size, stride) in enumerate(zip(kernel_sizes, strides)):
            ...
            # Add residual connection for each branch
            if self.use_residual:
                branch_residual = ResidualConnection(
                    in_channels, branch_channels, stride, residual_dropout
                )
                self.branch_residuals.append(branch_residual)
                print(f"    Added branch {i} residual connection: {in_channels} → {branch_channels} (stride={stride})")
            else:
                self.branch_residuals.append(nn.Identity())
        ...
        if self.use_pointwise_conv and self.is_final_layer:
            ...
            self.pointwise_residuals = nn.ModuleList()  # Residual connections for pointwise conv
            ...
            for i in range(self.num_branches):
                ...
                # Add residual connection for pointwise conv if enabled
                if self.use_residual:
                    pointwise_residual = ResidualConnection(
                        out_channels, pointwise_channels, stride=1, dropout_rate=residual_dropout
                    )
                    self.pointwise_residuals.append(pointwise_residual)
                    print(f"    Added pointwise branch {i} residual connection: {out_channels} → {pointwise_channels}")
                else:
                    self.pointwise_residuals.append(nn.Identity())
    ...
    def forward(self, x):
        ...
        # Process each branch through main convolutions with individual residual connections
        for i, (branch, branch_residual) in enumerate(zip(self.branches, self.branch_residuals)):
            # Apply convolution
            branch_output = branch(x)
            
            # Apply residual connection for this branch
            if self.use_residual:
                branch_output = branch_residual(branch_output, x)
            
            branch_outputs.append(branch_output)
            output_lengths.append(branch_output.size(2))
        ...
        if self.use_pointwise_conv and self.is_final_layer:
            pointwise_outputs = []
            for i, (output, pointwise_branch, pointwise_residual) in enumerate(zip(standardized_outputs, self.pointwise_branches, self.pointwise_residuals)):
                # Store input for pointwise residual
                pointwise_input = output
                
                # Apply pointwise convolution
                pointwise_output = pointwise_branch(output)
                
                # Apply pointwise residual connection
                if self.use_residual:
                    pointwise_output = pointwise_residual(pointwise_output, pointwise_input)
...
class ParallelMultiScaleConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes, strides, act, dropout_rate, layer_idx, is_final_layer, 
                pool_type="max", use_pointwise_conv=False, pointwise_channels=16, use_residual=False, residual_dropout=0.1):
        ...
        for branch_idx, (kernel_size, stride) in enumerate(zip(kernel_sizes, strides)):
            ...
            if self.use_residual:
                branch_residual = ResidualConnection(
                    in_channels, out_channels, stride, residual_dropout
                )
                self.branch_residuals.append(branch_residual)
                print(f"    Added parallel layer {layer_idx} branch {branch_idx} residual: {in_channels} → {out_channels} (stride={stride})")
            else:
                self.branch_residuals.append(nn.Identity())
        ...
        if self.use_pointwise_conv and self.is_final_layer:
            ...
            for i in range(self.num_branches):
                ...
                if self.use_residual:
                    ...
                    self.pointwise_residuals.append(pointwise_residual)
                    print(f"    Added parallel pointwise branch {i} residual: {out_channels} → {pointwise_channels}")
                else:
                    self.pointwise_residuals.append(nn.Identity())
    ...
    def forward(self, x):
        ...
        # Process each branch through main convolutions with individual residual connections
        for branch_idx, (branch, branch_input, branch_residual) in enumerate(zip(self.branches, branch_inputs, self.branch_residuals)):
            # Apply convolution
            branch_output = branch(branch_input)
            
            # Apply residual connection for this branch
            if self.use_residual:
                branch_output = branch_residual(branch_output, branch_input)
        ...
        if self.is_final_layer:
            ...
            if self.use_pointwise_conv:
                pointwise_outputs = []
                for i, (output, pointwise_branch, pointwise_residual) in enumerate(zip(branch_outputs, self.pointwise_branches, self.pointwise_residuals)):
                    # Store input for pointwise residual
                    pointwise_input = output
...
class ResidualConvBlock(nn.Module):
    """Standard convolution block with residual connection"""
    
    def __init__(self, in_channels, out_channels, kernel_size, stride, act, dropout_rate, 
                 use_pointwise_conv=False, pointwise_channels=16, is_final_layer=False, 
                 use_residual=False, residual_dropout=0.1):
        ...
        if self.use_pointwise_conv and self.is_final_layer:
            layers.extend([
                nn.Conv1d(in_channels=out_channels, out_channels=pointwise_channels,
                         kernel_size=1, stride=1, padding=0),
                nn.BatchNorm1d(pointwise_channels),
                self.get_activation(act)
            ])
            final_out_channels = pointwise_channels
        else:
        ...
        if self.use_residual:
            self.residual_connection = ResidualConnection(
                in_channels, final_out_channels, stride, residual_dropout
            )
            print(f"    Added standard residual connection: {in_channels} → {final_out_channels} (stride={stride})")
    ...
    def forward(self, x):
        ...
        if self.use_residual:
            output = self.residual_connection(output, residual)
...
class GenomicTransformerBlock(nn.Module):
    ...
    def _fallback_xavier_init(self, custom_layer):
        ...
        try:
            ...
            print(f"      Xavier initialization completed for layer")
...
class MultilabelGenotypeModel(nn.Module):
    def __init__(self, input_size, num_diseases, kernel_sizes, stride, conv_channels, fc_layers, act, dropout_rate, 
        ...
        if use_residual:
            print(f"  Residual dropout: {residual_dropout}")
            print(f"  Residual connection strategy: After every convolution layer")
        ...
        if self.use_multi_scale:
            ...
            # Print residual connection architecture
            if use_residual:
                if self.multi_scale_fusion == "cross_scale":
                    print(f"  Cross-scale residual architecture:")
                    print(f"    Input → [Branch1_Conv + Residual1, Branch2_Conv + Residual2, ...] → Concat")
                else:  # parallel
                    print(f"  Parallel residual architecture:")
                    print(f"    Layer i: [Branch1_Conv + Residual1, Branch2_Conv + Residual2, ...]")
                    print(f"    Final layer: → Concat")
            ...
            # Print residual connection architecture for standard conv
            if use_residual:
                print(f"  Standard residual architecture:")
                print(f"    Input → Conv1 + Residual1 → Conv2 + Residual2 → ... → Output")
        ...
        architecture_info.append(f"Residual connections: {use_residual} ({'per-layer strategy' if use_residual else 'disabled'})")
        if use_residual:
            architecture_info.append(f"Residual dropout: {residual_dropout}")
    ...
    def _create_multi_scale_conv_layers_cross_scale(self, conv_channels, kernel_sizes, stride, dropout_rate, act):
        """Create multi-scale convolutional layers with cross-scale fusion and residual connections"""
        ...
        for i in range(len(conv_channels)):
            ...
            multi_scale_block = MultiScaleConvBlock(
                ...
                self.use_residual, self.residual_dropout  # Add residual parameters
            )
    ...
    def _create_multi_scale_conv_layers_parallel(self, conv_channels, kernel_sizes, stride, dropout_rate, act):
        """Create parallel multi-scale convolutional layers with residual connections"""
        layers = nn.ModuleList()
    ...
    def _create_conv_layers(self, conv_channels, kernel_sizes, stride, dropout_rate, act):
        """Create traditional sequential convolutional layers with residual connections"""
        layers = []
        current_length = self.input_size
        ...
        for i in range(len(conv_channels)):
            ...
            # Create residual convolution block
            residual_block = ResidualConvBlock(
                ...
                use_pointwise_conv=self.use_pointwise_conv,
...
def get_scheduler(scheduler_name, optimizer, args, train_files):
    ...
    if scheduler_name.lower() == "none" or scheduler_name is None:
        ...
        return ExponentialDecay(optimizer, start_lr=args.lr, final_lr=args.final_lr, 
                                total_steps=args.epochs, decay_factor= args.df)
...
def main():
    ...
    model = MultilabelGenotypeModel(
        ...
        use_residual=bool(args.use_residual),  # Add residual parameter
        residual_dropout=args.residual_dropout,  # Add residual dropout parameter
    ...
    hyperparameters = {
        ...
        'Residual_Strategy': 'Per_Layer' if args.use_residual else 'N/A',
        'Residual_Dropout': args.residual_dropout if args.use_residual else 'N/A',
        'Use_Disease_Attention': bool(args.use_disease_attention),
        'Use_Separate_Heads': bool(args.use_separate_heads),
