...
def parse_args():
    parser = argparse.ArgumentParser(description="Multilabel Genotype Model Training with Permutation Tests")
    ...
    parser.add_argument("-pooling_strategy", type=str, default="mean", 
                    ...
                    help="Pooling strategy for transformer output")
    ...
    parser.add_argument("-permutation_test", type=int, default=0, choices=[0, 1], help="Enable permutation testing (0: disabled, 1: enabled)")
    parser.add_argument("-n_permutations", type=int, default=5, help="Number of permutation runs to perform (default: 5)")
    parser.add_argument("-permutation_seed_start", type=int, default=100, help="Starting seed for permutation tests (default: 100)")
    parser.add_argument("-run_real_first", type=int, default=1, choices=[0, 1], help="Run real labels training before permutations (0: no, 1: yes)")
    parser.add_argument("-permutation_strategy", type=str, default="independent", choices=["independent", "synchronized"], help="Permutation strategy: 'independent' (shuffle each disease separately) or 'synchronized' (keep disease co-occurrence patterns)")

    
    return parser.parse_args()


def permute_labels_trainonly(phenotype_data, train_sample_ids, disease_labels, 
                             strategy="independent", seed=42):
    """
    Permute disease labels for negative control analysis.
    
    Args:
        phenotype_data: DataFrame containing phenotype information
        train_sample_ids: List of sample IDs that are in the training set
        disease_labels: List of disease column names to permute
        strategy: 'independent' (shuffle each disease separately) or 
                 'synchronized' (shuffle rows together to keep co-occurrence)
    ...
    permuted_data = phenotype_data.copy()
    
    # Identify which rows are training samples
    train_mask = permuted_data['new_order'].isin(train_sample_ids)
    
    # Extract training data
    train_data = permuted_data[train_mask].copy()
    
    print(f"  Permuting {len(train_data)} training samples (seed={seed})")
    print(f"  Keeping {(~train_mask).sum()} test samples with REAL labels")
    
    if strategy == "independent":
        # Shuffle each disease label independently in training set
        # This breaks any disease-disease correlations
        for disease in disease_labels:
            original_values = train_data[disease].values
            permuted_values = np.random.permutation(original_values)
            train_data[disease] = permuted_values
            
            # Sanity check: verify permutation actually changed the order
            changes = (original_values != permuted_values).sum()
            print(f"    {disease}: {changes}/{len(original_values)} labels changed position")
    
    elif strategy == "synchronized":
        # Shuffle rows together (keeping disease co-occurrence patterns)
        # This tests if the specific individual-disease associations matter
        disease_columns = train_data[disease_labels].copy()
        shuffled_indices = np.random.permutation(len(disease_columns))
        train_data[disease_labels] = disease_columns.iloc[shuffled_indices].values
        
        # Sanity check
        original_first_row = phenotype_data[train_mask][disease_labels].iloc[0].values
        permuted_first_row = train_data[disease_labels].iloc[0].values
        changed = not np.array_equal(original_first_row, permuted_first_row)
        print(f"    First training row changed: {changed}")
    
    # Update only the TRAINING samples in the full dataset
    # Test samples remain UNCHANGED with their real labels
    permuted_data.loc[train_mask, disease_labels] = train_data[disease_labels].values
    
    return permuted_data


def compare_permutation_results(real_metrics, permutation_metrics_list, disease_labels, output_dir):
    """
    Compare real model performance against permutation test results.
    
    Args:
        real_metrics: Dict of metrics from real labels training
        permutation_metrics_list: List of metric dicts from permutation runs
    ...
    # Metrics to compare
    metric_names = ['auc', 'pr_auc', 'f1', 'acc']
    phases = ['train', 'test']
    ...
    for phase in phases:
        ...
        for disease in disease_labels:
            print(f"\n{disease.upper()}:")
            comparison_results[phase][disease] = {}
            
            for metric in metric_names:
                # Get real performance
                real_value = real_metrics[phase][disease][metric]
                
                # Get permutation performances
                perm_values = [pm[phase][disease][metric] for pm in permutation_metrics_list]
                perm_mean = np.mean(perm_values)
                perm_std = np.std(perm_values)
                perm_max = np.max(perm_values)
                
                # Calculate p-value (one-tailed test: is real > permuted?)
                # Count how many permutations achieved equal or better performance
                n_better = sum(1 for pv in perm_values if pv >= real_value)
                p_value = (n_better + 1) / (len(perm_values) + 1)  # +1 for pseudocount
                
                # Calculate effect size (Cohen's d-like measure)
                if perm_std > 0:
                    effect_size = (real_value - perm_mean) / perm_std
                else:
                    effect_size = float('inf') if real_value > perm_mean else 0
                
                # Store results
                comparison_results[phase][disease][metric] = {
                    'real': real_value,
                    'perm_mean': perm_mean,
                    'perm_std': perm_std,
                    'perm_max': perm_max,
                ...
                print(f"  {metric.upper():8s}: Real={real_value:.4f} | "
                      f"Perm={perm_mean:.4f}±{perm_std:.4f} (max={perm_max:.4f}) | "
                      f"p={p_value:.4f} {significance} | ES={effect_size:.2f}")
    
    # Save detailed comparison to JSON
    json_path = os.path.join(output_dir, 'permutation_comparison.json')
    with open(json_path, 'w') as f:
        # Convert numpy values to Python types for JSON serialization
        serializable_results = {}
        for phase in comparison_results:
            serializable_results[phase] = {}
            for disease in comparison_results[phase]:
                serializable_results[phase][disease] = {}
                for metric in comparison_results[phase][disease]:
                    metric_data = comparison_results[phase][disease][metric]
                    serializable_results[phase][disease][metric] = {
                        'real': float(metric_data['real']),
                        'perm_mean': float(metric_data['perm_mean']),
                        'perm_std': float(metric_data['perm_std']),
                        'perm_max': float(metric_data['perm_max']),
                        'perm_values': [float(v) for v in metric_data['perm_values']],
                        'p_value': float(metric_data['p_value']),
                        'effect_size': float(metric_data['effect_size']) if not np.isinf(metric_data['effect_size']) else 999.0
                    }
        json.dump(serializable_results, f, indent=2)
    
    print(f"\nDetailed comparison saved to: {json_path}")
    
    # Create visualization
    plot_permutation_comparison(comparison_results, disease_labels, output_dir)
    
    return comparison_results


def plot_permutation_comparison(comparison_results, disease_labels, output_dir):
    """
    Create visualization comparing real vs permutation test performance.
    """
    metric_names = ['auc', 'pr_auc', 'f1', 'acc']
    metric_labels = ['ROC-AUC', 'PR-AUC', 'F1 Score', 'Accuracy']
    phases = ['test']  # Focus on test set for main visualization
    
    for phase in phases:
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        axes = axes.flatten()
        
        for idx, (metric, metric_label) in enumerate(zip(metric_names, metric_labels)):
            ax = axes[idx]
            
            x_positions = np.arange(len(disease_labels))
            width = 0.35
            ...
            for disease in disease_labels:
                metric_data = comparison_results[phase][disease][metric]
                real_values.append(metric_data['real'])
                perm_means.append(metric_data['perm_mean'])
                perm_stds.append(metric_data['perm_std'])
                p_values.append(metric_data['p_value'])
            
            # Plot real values
            bars1 = ax.bar(x_positions - width/2, real_values, width, 
                          label='Real Labels', color='#2ecc71', alpha=0.8)
            
            # Plot permutation values with error bars
            bars2 = ax.bar(x_positions + width/2, perm_means, width, 
                          yerr=perm_stds, label='Permuted Labels (mean±std)', 
                          color='#e74c3c', alpha=0.8, capsize=5)
            
            # Add significance stars
            for i, (real, perm_mean, perm_std, p_val) in enumerate(zip(real_values, perm_means, perm_stds, p_values)):
                if p_val < 0.05:
                    y_pos = max(real, perm_mean + perm_std) + 0.02
                    stars = "***" if p_val < 0.001 else "**" if p_val < 0.01 else "*"
                    ax.text(i, y_pos, stars, ha='center', va='bottom', fontsize=14, fontweight='bold')
            
            ax.set_xlabel('Disease', fontsize=12, fontweight='bold')
            ax.set_ylabel(metric_label, fontsize=12, fontweight='bold')
            ax.set_title(f'{metric_label} - Real vs Permuted', fontsize=14, fontweight='bold')
            ...
            # Add chance level line for AUC metrics
            if 'auc' in metric.lower():
                ax.axhline(y=0.5, color='gray', linestyle='--', alpha=0.5, label='Chance Level')
        
        plt.tight_layout()
        save_path = os.path.join(output_dir, f'permutation_comparison_{phase}.png')
    ...
    create_distribution_plots(comparison_results, disease_labels, output_dir)


def create_distribution_plots(comparison_results, disease_labels, output_dir):
    """
    Create violin/box plots showing distribution of permutation results.
    """
    phase = 'test'
    ...
    for i, disease in enumerate(disease_labels):
        ...
        # Add permutation distribution
        all_data.append(metric_data['perm_values'])
        positions.append(i * 2)
        colors.append('#e74c3c')
        labels.append(f"{disease}\n(Permuted)")
        
        # Add real value as a single point (for visualization)
        all_data.append([metric_data['real']])
    ...
    ax.set_title('Distribution of Permutation Test Results vs Real Performance', 
    ...
    # Set x-tick labels
    ax.set_xticks([i * 2 + 0.4 for i in range(len(disease_labels))])
    ax.set_xticklabels(disease_labels, rotation=45, ha='right')
    
    plt.tight_layout()
    save_path = os.path.join(output_dir, 'permutation_distributions.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_permutation_summary(real_metrics, permutation_metrics_list, comparison_results, 
                            disease_labels, output_dir, args):
    """
    Save a comprehensive summary of permutation test results.
    """
    summary_path = os.path.join(output_dir, 'permutation_test_summary.txt')
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"  Permutation strategy: {args.permutation_strategy}\n")
        f.write(f"  Disease labels: {', '.join(disease_labels)}\n")
        f.write(f"  Seed start: {args.permutation_seed_start}\n\n")
        
        f.write("="*80 + "\n")
        f.write("INTERPRETATION GUIDE\n")
        f.write("="*80 + "\n\n")
        f.write("P-values:\n")
        f.write("  p < 0.001 (***): Extremely strong evidence that real performance > chance\n")
        f.write("  p < 0.01 (**):   Strong evidence that real performance > chance\n")
        f.write("  p < 0.05 (*):    Moderate evidence that real performance > chance\n")
        f.write("  p >= 0.05 (ns):  No significant evidence that real performance > chance\n\n")
        
        f.write("Effect Size (Cohen's d-like):\n")
        f.write("  ES > 0.8:  Large effect (real substantially better than permuted)\n")
        f.write("  ES > 0.5:  Medium effect\n")
        f.write("  ES > 0.2:  Small effect\n")
        f.write("  ES <= 0.2: Negligible effect\n\n")
        ...
        for disease in disease_labels:
            ...
            for metric in ['auc', 'pr_auc', 'f1', 'acc']:
                metric_data = comparison_results[phase][disease][metric]
                
                f.write(f"{metric.upper()}:\n")
                f.write(f"  Real:           {metric_data['real']:.4f}\n")
                f.write(f"  Permuted mean:  {metric_data['perm_mean']:.4f} ± {metric_data['perm_std']:.4f}\n")
                f.write(f"  Permuted max:   {metric_data['perm_max']:.4f}\n")
                f.write(f"  P-value:        {metric_data['p_value']:.4f}\n")
                f.write(f"  Effect size:    {metric_data['effect_size']:.2f}\n")
                
                # Interpretation
                if metric_data['p_value'] < 0.05:
                    if metric_data['effect_size'] > 0.8:
                        interpretation = "STRONG: Real signal substantially above chance"
                    elif metric_data['effect_size'] > 0.5:
                        interpretation = "MODERATE: Real signal moderately above chance"
                    else:
                        interpretation = "WEAK: Real signal slightly above chance"
                else:
                    interpretation = "NOT SIGNIFICANT: No evidence of signal above chance"
        ...
        total_count = 0
        for disease in disease_labels:
            metric_data = comparison_results[phase][disease]['auc']
            total_count += 1
            if metric_data['p_value'] < 0.05:
                sig_count += 1
        
        f.write(f"Significant results (ROC-AUC, p<0.05): {sig_count}/{total_count} diseases\n\n")
        
        if sig_count == 0:
            f.write("WARNING: No diseases showed performance significantly better than chance.\n")
            f.write("This suggests the model may be overfitting or learning spurious patterns.\n")
        elif sig_count < total_count / 2:
            f.write("CAUTION: Only some diseases show performance above chance.\n")
            f.write("Review model architecture and data quality for non-significant diseases.\n")
        else:
            f.write("VALIDATED: Most diseases show performance significantly above chance.\n")
            f.write("This provides evidence that the model is learning real biological signals.\n")
    
    print(f"\nPermutation test summary saved to: {summary_path}")
...
def main():
    print("Starting multilabel disease prediction model with permutation testing...")
    ...
    if args.permutation_test:
        # Create subdirectories for real and permutation runs
        base_exp_dir = os.path.join(args.exp_dir, id)
        real_exp_dir = os.path.join(base_exp_dir, "real_labels")
        perm_exp_dir = os.path.join(base_exp_dir, "permutations")
        os.makedirs(real_exp_dir, exist_ok=True)
        os.makedirs(perm_exp_dir, exist_ok=True)
        print(f"Permutation test mode enabled")
        print(f"Results will be saved to: {base_exp_dir}")
        print(f"  - Real labels: {real_exp_dir}")
        print(f"  - Permutations: {perm_exp_dir}")
    else:
    ...
    filtered_file_list = [f for f in file_list 
                          if int(f.split('sample_')[1].split('.npy')[0]) in phenotype_samples]
    print(f"Number of filtered genotype files matching phenotype data: {len(filtered_file_list)}")

    # Get input size
    first_genotype_file = filtered_file_list[0]
    input_size = get_input_size(first_genotype_file)
    print(f"Dynamically determined input size: {input_size}")

    # Split data (using same split for all runs for fair comparison)
    ...
    # Extract sample IDs from file lists
    train_sample_ids = [int(f.split('sample_')[1].split('.npy')[0]) for f in train_files]
    test_sample_ids = [int(f.split('sample_')[1].split('.npy')[0]) for f in test_files]
    
    print(f"Training sample IDs: {len(train_sample_ids)}")
    print(f"Test sample IDs: {len(test_sample_ids)}")
    ...
    if args.permutation_test:
        print("\n" + "="*80)
        print("PERMUTATION TEST MODE")
        print(f"Strategy: {args.permutation_strategy}")
        print(f"Number of permutations: {args.n_permutations}")
        print(f"Seed start: {args.permutation_seed_start}")
        print("="*80)
        
        # Store results from all runs
        real_metrics = None
        permutation_metrics_list = []

        # ==================== STEP 1: Train with REAL labels ====================
        # Run with real labels first if requested
        if args.run_real_first:
            print("\n" + "="*80)
            print("STEP 1: Training with REAL labels")
            print("="*80)
            
            real_metrics = run_single_experiment(
                args=args,
                train_files=train_files,
                test_files=test_files,
                phenotype_data=phenotype_data,   # Original, unmodified data
                disease_labels=disease_labels,
                input_size=input_size,
                device=device,
                experiment_dir=real_exp_dir,
                is_permutation=False,
                permutation_idx=None
            )
        
            print(f"\nReal labels training completed!")
            print(f"Results saved to: {real_exp_dir}")
        
        # ==================== STEP 2: Train with PERMUTED labels ====================
        # Run permutation tests
        print("\n" + "="*80)
        print(f"STEP 2: Running {args.n_permutations} PERMUTATION tests")
        print(f"Strategy: {args.permutation_strategy}")
        print("="*80)
        
        for perm_idx in range(args.n_permutations):
            perm_seed = args.permutation_seed_start + perm_idx
            
            print(f"\n{'-'*80}")
            print(f"PERMUTATION {perm_idx + 1}/{args.n_permutations} (seed={perm_seed})")
            print(f"{'-'*80}")
            
            # Create permuted phenotype data
            permuted_phenotype = permute_labels(
                phenotype_data=phenotype_data,
                train_sample_ids=train_sample_ids,  # Only these get permuted
                disease_labels=disease_labels,
                strategy=args.permutation_strategy,
                seed=perm_seed
            )
            
            # Verify that test labels are unchanged
            test_mask = permuted_phenotype['new_order'].isin(test_sample_ids)
            original_test_labels = phenotype_data[test_mask][disease_labels].values
            permuted_test_labels = permuted_phenotype[test_mask][disease_labels].values
            assert np.array_equal(original_test_labels, permuted_test_labels), \
                "ERROR: Test labels were modified! They should remain unchanged."
            print("  ✓ Verified: Test labels are unchanged (real)")


            # Create permutation-specific directory
            perm_run_dir = os.path.join(perm_exp_dir, f"perm_{perm_idx+1:03d}_seed{perm_seed}")
            os.makedirs(perm_run_dir, exist_ok=True)
            
            # Run training with permuted labels
            perm_metrics = run_single_experiment(
                args=args,
                train_files=train_files,
                test_files=test_files,
                phenotype_data=permuted_phenotype,  # Training labels shuffled, test labels real
                disease_labels=disease_labels,
                input_size=input_size,
                device=device,
                experiment_dir=perm_run_dir,
                is_permutation=True,
                permutation_idx=perm_idx + 1
            )
            
            permutation_metrics_list.append(perm_metrics)
        ...
        if real_metrics is None:
            print("WARNING: Real labels were not run. Cannot perform comparison.")
            print("Set -run_real_first 1 to enable comparison.")
        else:
            comparison_results = compare_permutation_results(
                real_metrics=real_metrics,
                permutation_metrics_list=permutation_metrics_list,
                disease_labels=disease_labels,
            ...
            print(f"\nResults saved to: {base_exp_dir}")
            print("\nKey files to review:")
            print(f"  1. {os.path.join(base_exp_dir, 'permutation_test_summary.txt')}")
            print(f"  2. {os.path.join(base_exp_dir, 'permutation_comparison_test.png')}")
            print(f"  3. {os.path.join(base_exp_dir, 'permutation_distributions.png')}")
    
    else:
        # ==================== STANDARD TRAINING (NO PERMUTATION) ====================
        print("\n" + "="*80)
        print("STANDARD TRAINING MODE (no permutation testing)")
        print("="*80)
...
def run_single_experiment(args, train_files, test_files, phenotype_data, disease_labels, 
                         input_size, device, experiment_dir, is_permutation=False, 
                         permutation_idx=None):
    """
    Run a single training experiment (either with real or permuted labels).
    
    This function encapsulates all the training logic that was previously in main().
    Returns the final metrics for comparison.
    """
    
    if is_permutation:
        print(f"\n--- Training with PERMUTED labels (Permutation {permutation_idx}) ---")
    else:
    ...
    train_dataset = MultilabelGenotypeDataset(
        train_files, 
        phenotype_data,   # Training samples will have permuted labels (if permutation mode)
        disease_labels, 
        use_covariates=bool(args.cov),
        use_age=bool(args.use_age),
        use_gender=bool(args.use_gender),
    ...
    test_dataset = MultilabelGenotypeDataset(
        test_files,
        phenotype_data,  # Test samples will have REAL labels (unchanged in permutation mode)
        disease_labels, 
        use_covariates=bool(args.cov),
        use_age=bool(args.use_age),
        use_gender=bool(args.use_gender),
    ...
    hyperparameters = {
        ...
        'Permutation_Idx': permutation_idx if is_permutation else 'N/A',
        'Permutation_strategy': args.permutation_strategy if is_permutation else 'N/A'
