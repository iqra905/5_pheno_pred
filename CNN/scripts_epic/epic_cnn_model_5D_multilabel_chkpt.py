...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_1", help="ID of the experiment")
    ...
    parser.add_argument("-patience", type=int, default=70, help="Patience for early stopping")
...
def train_multilabel_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, disease_labels, 
    ...
    # Debug print to check history object
    ...
    for disease in disease_labels:
        for phase in ['train', 'test']:
            required_keys.append(f'{phase}_{disease}_acc')
            required_keys.append(f'{phase}_{disease}_auc')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            for i in range(len(dataloaders[phase])):
                ...
                # Store predictions and labels for each disease - STORE ONLY FOR CURRENT EPOCH
                for j, disease in enumerate(disease_labels):
                    running_corrects[disease] += torch.sum(preds[:, j] == labels[:, j])

                    # Store predictions and true labels for current epoch only
    ...
    # Use phase_preds and phase_labels directly for computing final metrics
    return model, history, compute_final_metrics(phase_labels, phase_preds, disease_labels), phase_preds, phase_labels, completed_epochs

# Updated compute_final_metrics function to add more diagnostics
def compute_final_metrics(all_labels, all_preds, disease_labels):
    ...
    for phase in ['train', 'test']:
        ...
        for disease in disease_labels:
            ...
            # Print diagnostics about the dataset for this disease
...
def main():
    ...
    if args.resume:
        # Use the updated function with prefer_best=True (the default)
