...
def parse_args():
    ...
    parser.add_argument("-num_seeds", type=int, default=3, help="Number of different seeds to run (default: 3)")
    parser.add_argument("-seed_start", type=int, default=42, help="Starting seed value (default: 42)")
    parser.add_argument("-seed_list", type=str, default=None, help="Comma-separated list of specific seeds to use (e.g., '42,123,456'). If provided, overrides num_seeds and seed_start")
    parser.add_argument("-run_mode", type=str, default="multi_seed", choices=["single_seed", "multi_seed"], help="Run mode: 'single_seed' for traditional single run, 'multi_seed' for multiple seeds with statistics")
    parser.add_argument("-aggregate_only", type=int, default=0, choices=[0, 1], help="If 1, only aggregate existing seed results without retraining (useful for re-analysis)")
...
def run_single_seed_experiment(args, seed, experiment_dir):
    """
    Run a complete training experiment for a single seed.
    This contains all the logic from the original main() function.
    ...
    Returns:
        dict: Contains metrics, training_time, and other results
    """
    # Set the seed
    set_seed(seed)
    ...
    # Extract metrics in a clean format
    for phase in ['train', 'test']:
        seed_results['metrics'][phase] = {}
        for disease in disease_labels:
            seed_results['metrics'][phase][disease] = {
                'acc': float(final_metrics[phase][disease]['acc']),
                'auc': float(final_metrics[phase][disease]['auc']),
                'pr_auc': float(final_metrics[phase][disease]['pr_auc']),
                'f1': float(final_metrics[phase][disease]['f1']),
                'sens': float(final_metrics[phase][disease]['sens']),
                'spec': float(final_metrics[phase][disease]['spec'])
            }
    
    return seed_results
    
def aggregate_multi_seed_results(all_seed_results, disease_labels, args):
    """
    Aggregate results across multiple seeds and compute statistics
    
    Args:
        all_seed_results: List of result dictionaries from each seed
        disease_labels: List of disease names
        args: Command-line arguments
        
    Returns:
        dict: Aggregated statistics including mean, std, min, max for all metrics
    """
    print(f"\n{'='*80}")
    ...
    aggregated = {
        'num_seeds': num_seeds,
        'seeds_used': [r['seed'] for r in all_seed_results],
        'training_times': {
            'values': [r['training_time'] for r in all_seed_results],
            'mean': 0.0,
            'std': 0.0,
            'min': 0.0,
            'max': 0.0
    ...
    times = aggregated['training_times']['values']
    aggregated['training_times']['mean'] = np.mean(times)
    aggregated['training_times']['std'] = np.std(times)
    aggregated['training_times']['min'] = np.min(times)
    aggregated['training_times']['max'] = np.max(times)
    
    # Aggregate metrics for each phase and disease
    for phase in ['train', 'test']:
        aggregated['metrics'][phase] = {}
        
        for disease in disease_labels:
            aggregated['metrics'][phase][disease] = {}
            
            # For each metric, collect values across seeds
            metric_names = ['acc', 'auc', 'pr_auc', 'f1', 'sens', 'spec']
            ...
            for metric in metric_names:
                ...
                for seed_result in all_seed_results:
                    try:
                        val = seed_result['metrics'][phase][disease][metric]
                        values.append(val)
                    except (KeyError, TypeError):
                        print(f"Warning: Missing {metric} for {disease} in {phase} phase")
                        continue
                
                if values:
                    aggregated['metrics'][phase][disease][metric] = {
                        'values': values,
                        'mean': np.mean(values),
                        'std': np.std(values),
                        'min': np.min(values),
...
def print_aggregated_results(aggregated, disease_labels):
    ...
    print(f"Seeds used: {aggregated['seeds_used']}")
    print(f"Training time: {aggregated['training_times']['mean']:.2f} ± {aggregated['training_times']['std']:.2f}s")
    print(f"  (min: {aggregated['training_times']['min']:.2f}s, max: {aggregated['training_times']['max']:.2f}s)\n")
    ...
    for phase in ['train', 'test']:
        ...
        for disease in disease_labels:
            print(f"\n{disease.upper()}:")
            metrics = aggregated['metrics'][phase][disease]
            
            for metric_name in ['acc', 'auc', 'pr_auc', 'f1', 'sens', 'spec']:
                if metric_name in metrics:
                    m = metrics[metric_name]
                    print(f"  {metric_name.upper():8s}: {m['mean']:.4f} ± {m['std']:.4f} "
                          f"(min: {m['min']:.4f}, max: {m['max']:.4f})")

def write_aggregated_results(aggregated, disease_labels, hyperparameters, save_dir):
    """Write aggregated results to files"""
    os.makedirs(save_dir, exist_ok=True)
    
    # Write detailed text report
    with open(os.path.join(save_dir, 'aggregated_results.txt'), 'w') as f:
        f.write("="*80 + "\n")
        f.write("MULTI-SEED EXPERIMENT RESULTS\n")
        f.write("="*80 + "\n\n")
        
        f.write(f"Number of seeds: {aggregated['num_seeds']}\n")
        f.write(f"Seeds used: {aggregated['seeds_used']}\n\n")
        ...
        f.write("-" * 80 + "\n")
        times = aggregated['training_times']
        f.write(f"Mean:   {times['mean']:.2f}s\n")
        f.write(f"Std:    {times['std']:.2f}s\n")
        f.write(f"Min:    {times['min']:.2f}s\n")
        f.write(f"Max:    {times['max']:.2f}s\n")
        ...
        for phase in ['train', 'test']:
            ...
            for disease in disease_labels:
                ...
                for metric_name in ['acc', 'sens', 'spec', 'auc', 'pr_auc', 'f1']:
                    if metric_name in metrics:
                        m = metrics[metric_name]
                        f.write(f"  {metric_name.upper():12s}: "
                               f"{m['mean']:.5f} ± {m['std']:.5f} "
                               f"[{m['min']:.5f}, {m['max']:.5f}]\n")
    
    # Write CSV with statistics
    csv_path = os.path.join(save_dir, 'aggregated_results.csv')
    with open(csv_path, 'w', newline='') as csvfile:
        # Prepare fieldnames
        fieldnames = ['metric_type', 'phase', 'disease', 'statistic']
        fieldnames.extend(['acc', 'sens', 'spec', 'auc', 'pr_auc', 'f1'])
        
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        
        # Write statistics for each phase and disease
        for phase in ['train', 'test']:
            for disease in disease_labels:
                metrics = aggregated['metrics'][phase][disease]
                ...
                for metric_name in ['acc', 'sens', 'spec', 'auc', 'pr_auc', 'f1']:
                    if metric_name in metrics:
                        row_mean[metric_name] = metrics[metric_name]['mean']
                ...
                for metric_name in ['acc', 'sens', 'spec', 'auc', 'pr_auc', 'f1']:
                    if metric_name in metrics:
                        row_std[metric_name] = metrics[metric_name]['std']
                writer.writerow(row_std)
    
    # Save complete aggregated data as JSON
    json_path = os.path.join(save_dir, 'aggregated_results.json')
    with open(json_path, 'w') as f:
        json.dump(aggregated, f, indent=4)
    
    print(f"\nAggregated results saved to:")
    print(f"  - {os.path.join(save_dir, 'aggregated_results.txt')}")
    print(f"  - {csv_path}")
    print(f"  - {json_path}")


def plot_aggregated_metrics(aggregated, disease_labels, save_dir):
    """Create plots showing mean ± std across seeds"""
    plots_dir = os.path.join(save_dir, 'aggregated_plots')
    os.makedirs(plots_dir, exist_ok=True)
    ...
    for idx, (metric, name) in enumerate(zip(metrics_to_plot, metric_names)):
        ...
        for disease in disease_labels:
            train_means.append(aggregated['metrics']['train'][disease][metric]['mean'])
            train_stds.append(aggregated['metrics']['train'][disease][metric]['std'])
            test_means.append(aggregated['metrics']['test'][disease][metric]['mean'])
            test_stds.append(aggregated['metrics']['test'][disease][metric]['std'])
        ...
        ax.bar(x - width/2, train_means, width, yerr=train_stds,
               label='Train', alpha=0.8, capsize=5, error_kw={'linewidth': 2})
        ax.bar(x + width/2, test_means, width, yerr=test_stds,
               label='Test', alpha=0.8, capsize=5, error_kw={'linewidth': 2})
        
        ax.set_title(f'{name} by Disease (Mean ± Std across {aggregated["num_seeds"]} seeds)', 
                    fontsize=12)
    ...
    plt.suptitle(f'Performance Metrics Comparison (n={aggregated["num_seeds"]} seeds)', 
                 fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(plots_dir, 'metrics_comparison_with_error_bars.png'), 
    ...
    # Prepare table data
    table_data = []
    headers = ['Phase', 'Disease'] + [f'{m.upper()}\n(mean±std)' for m in metrics_to_plot]
    ...
    for phase in ['Train', 'Test']:
        ...
        for disease in disease_labels:
            ...
            for metric in metrics_to_plot:
                m = aggregated['metrics'][phase_lower][disease][metric]
                row.append(f'{m["mean"]:.3f}±{m["std"]:.3f}')
            table_data.append(row)
    
    table = ax.table(cellText=table_data, cellLoc='center', loc='center',
                    colWidths=[0.1, 0.15] + [0.15]*len(metrics_to_plot))
    ...
    plt.title(f'Aggregated Results Table (n={aggregated["num_seeds"]} seeds)', 
             fontsize=16, fontweight='bold', pad=20)
    plt.savefig(os.path.join(plots_dir, 'results_table.png'), 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Individual disease comparison plots
    for disease in disease_labels:
        ...
        for phase_idx, phase in enumerate(['train', 'test']):
            ...
            metrics = ['auc', 'pr_auc', 'f1', 'acc']
            means = [aggregated['metrics'][phase][disease][m]['mean'] for m in metrics]
            stds = [aggregated['metrics'][phase][disease][m]['std'] for m in metrics]
            
            x_pos = np.arange(len(metrics))
            bars = ax.bar(x_pos, means, yerr=stds, capsize=10, 
                         color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A'],
                         alpha=0.8, error_kw={'linewidth': 2, 'elinewidth': 2})
            
            ax.set_xlabel('Metric', fontsize=12)
            ax.set_ylabel('Score', fontsize=12)
            ax.set_title(f'{phase.capitalize()} Set', fontsize=13, fontweight='bold')
            ax.set_xticks(x_pos)
            ax.set_xticklabels(['ROC AUC', 'PR AUC', 'F1', 'Accuracy'])
            ax.set_ylim(0, 1)
            ax.grid(alpha=0.3, axis='y')
        ...
        plt.suptitle(f'{disease.upper()} - Performance Metrics (n={aggregated["num_seeds"]} seeds)',
                    fontsize=14, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        plt.savefig(os.path.join(plots_dir, f'{disease}_metrics.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
    
    print(f"\nAggregated plots saved to: {plots_dir}")

def run_single_seed_mode(args):
    """Run experiment in single-seed mode (original behavior)"""
    print("\n>>> Running in SINGLE SEED mode")
    
    seed = args.seed_start
    experiment_dir = os.path.join(args.exp_dir, str(args.ID))
    os.makedirs(experiment_dir, exist_ok=True)
    
    run_single_seed_experiment(args, seed, experiment_dir)
    
    print("\nExperiment complete!")

def run_multi_seed_mode(args):
    """Run experiment across multiple seeds"""
    print("\n>>> Running in MULTI-SEED mode")
    
    # Determine seeds to use
    if args.seed_list:
        seeds = [int(s.strip()) for s in args.seed_list.split(',')]
        print(f"Using custom seed list: {seeds}")
    else:
        seeds = list(range(args.seed_start, args.seed_start + args.num_seeds))
        print(f"Using {len(seeds)} seeds: {seeds}")
    
    # Create main experiment directory
    main_exp_dir = os.path.join(args.exp_dir, str(args.ID))
    ...
    if not args.aggregate_only:
        ...
        print(f"\nRunning {len(seeds)} experiments...")
        total_start = time.time()
        
        for idx, seed in enumerate(seeds):
            print(f"\n{'='*80}")
            print(f"SEED {idx+1}/{len(seeds)}: {seed}")
            print(f"{'='*80}")
            
            try:
                seed_dir = os.path.join(main_exp_dir, f"{args.ID}_seed{seed}")
                os.makedirs(seed_dir, exist_ok=True)
                
                # Run experiment for this seed
                seed_results = run_single_seed_experiment(args, seed, seed_dir)
                all_seed_results.append(seed_results)
                
                # Save seed results
                with open(os.path.join(seed_dir, 'seed_results.json'), 'w') as f:
        ...
        print(f"\n{'='*80}")
        print(f"All seeds completed in {total_time:.2f}s")
        print(f"Successful: {len(all_seed_results)}/{len(seeds)}")
        print(f"{'='*80}")
    
    else:
        # Load existing results
        print("\nLoading existing seed results...")
        for seed in seeds:
            seed_dir = os.path.join(main_exp_dir, f"{args.ID}_seed{seed}")
            results_file = os.path.join(seed_dir, 'seed_results.json')
    ...
    # Aggregate results
    print("\nAggregating results...")
    aggregated = aggregate_multi_seed_results(all_seed_results, args.disease_labels)
    
    # Print results
    print_aggregated_results(aggregated, args.disease_labels)
    
    # Write results
    write_aggregated_results(aggregated, args.disease_labels, main_exp_dir)
    
    # Create plots
    plot_aggregated_metrics(aggregated, args.disease_labels, main_exp_dir)
    
    print(f"\n{'='*80}")
    print("MULTI-SEED EXPERIMENT COMPLETE!")
    print(f"Results in: {main_exp_dir}")
    print(f"{'='*80}")
...
def main():
    ...
    # Route to appropriate function based on run mode
    if args.run_mode == "single_seed":
        run_single_seed_mode(args)
    else:  # multi_seed
        run_multi_seed_mode(args)
