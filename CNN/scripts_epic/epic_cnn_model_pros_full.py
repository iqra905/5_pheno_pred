...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="01_1", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/pros/sets/01/', help="Directory to save experiment results")
    ...
    parser.add_argument("-sch", type=str, default="plateau", choices=["none","plateau", "cosine", "step","multistep","explr","warmup_exponential", "exponential_decay"], help="Learning rate scheduler")
    parser.add_argument("-peak_lr", type=float, default=1e-1, help="Peak learning rate for WarmupExponential scheduler")
    parser.add_argument("-final_lr", type=float, default=1e-5, help="Final learning rate for custom schedulers")
    parser.add_argument("-df", type=float, default=0.5, help="Decay factor for custom schedulers")
    ...
    parser.add_argument('-layers', type=int, default=3, help='Number of convolution layers')
    parser.add_argument("-conv_channels", type=parse_int_list, default=[32,64,128], help="Convolution channels")
    parser.add_argument("-fc_layers", type=parse_int_list, default=[1024,128], help="Fully connected layers")
...
class GenotypeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # if idx % 100 == 0:  # Only print for every 100th item
        ...
        try:
            ...
            # if idx % 100 == 0:
            #     print(f"Label: {label}, Covariates shape: {covariates.shape}")
        except IndexError:
            print(f"IndexError: No matching sample_id {sample_id} found in phenotype_data.")

            raise ValueError(f"IndexError: No matching sample_id {sample_id} found in phenotype_data.")
        
        # if idx % 100 == 0:
        #     print(f"Loading genotype data from {genotype_file}")
        with gzip.open(genotype_file, 'rt') as f:
            data = pd.read_csv(f, sep=r'\s+', header=None)

        genotype_tensor = torch.tensor(data.values.T, dtype=torch.float32)
        # if idx % 100 == 0:
        #     print(f"Genotype tensor shape: {genotype_tensor.shape}")
...
class GenotypeModel(nn.Module):
    def __init__(self, input_shape, num_layers, conv_channels, fc_layers, dropout_prob, act, num_covariates=10):
        super(GenotypeModel, self).__init__()
        self.in_channels = input_shape[1]  # Infer number of input channels from input shape
        self.num_layers = num_layers
        self.conv_layers = self._create_conv_layers(conv_channels, dropout_prob, act, input_shape)
        
        # Calculate the input size to the fully connected layer
        conv_output_size = self._get_conv_output_size(input_shape)
        fc_input_size = conv_output_size + num_covariates
    ...
    def _create_conv_layers(self, out_channels, dropout_prob, act, input_shape):
        layers = []
        kernel_size = input_shape[2] # Set the kernel size of the first layer to input size
        for i in range(self.num_layers):
            layers.append(nn.Conv1d(in_channels=self.in_channels if i == 0 else out_channels[i-1],
            ...
            # Update kernel size to match the output size of the current layer
            kernel_size = 1 
        return nn.Sequential(*layers)
    
    # def _create_conv_layers(self, out_channels, dropout_prob, act, input_shape):
    #     layers = []
    #     kernel_size = input_shape[2] // 2  # Set the kernel size to half of the input size
    #     stride = input_shape[2] // 2  # Set the stride to half of the input size
    #     out_size = input_shape[2]
    #     for i in range(self.num_layers):
    #         layers.append(nn.Conv1d(in_channels=self.in_channels if i == 0 else out_channels[i-1],
    #                                 out_channels=out_channels[i],
    #                                 kernel_size=kernel_size,
    ...
    #         if i > 0:  # Apply dropout after the first layer
    #             layers.append(nn.Dropout(dropout_prob))
    #         # Update kernel size and stride to half of the output size of the current layer
    ...
    def _get_conv_output_size(self, input_shape):
        x = torch.zeros(*input_shape)  # Create a dummy input tensor
        x = self.conv_layers(x)  # Pass the input through the convolutional layers
        return x.numel() // x.size(0)  # Return the flattened size
    ...
    def count_parameters(self):
        conv_params = sum(p.numel() for name, p in self.named_parameters() if 'conv' in name and p.requires_grad)
        fc_params = sum(p.numel() for name, p in self.named_parameters() if 'fc' in name and p.requires_grad)
        total_params = conv_params + fc_params
        return {
            'Total': total_params,
            'Convolutional': conv_params,
            'Fully Connected': fc_params
        }
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            for inputs, covariates, labels in dataloaders[phase]:
...
def main():
    ...
    input_shape = (args.bs, 3, input_size)  # (batch_size, channels, input_size)
    ...
    print(f"Convolutional parameters: {param_counts['Convolutional']:,}")
    print(f"Fully Connected parameters: {param_counts['Fully Connected']:,}")
    ...
    hyperparameters = {
        ...
        'Conv_Parameters': param_counts['Convolutional'],
        'FC_Parameters': param_counts['Fully Connected']
    }
    # Write results
