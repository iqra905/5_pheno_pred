...
def parse_args():
    ...
    parser.add_argument("-layer_config", type=str, default="M,M,T,T,M,M", help="Layer configuration for hybrid mode. Use T for transformer, M for mamba. Example: 'T,M,T,M'")
    ...
    parser.add_argument("-num_layers", type=int, default=6, help="Total number of sequence layers")
    ...
    parser.add_argument("-pooling_strategy", type=str, default="mean", choices=["mean", "max", "attention", "concat"], help="Pooling strategy for final representation")
...
def validate_sequence_config(args):
    ...
    if args.sequence_mode == "hybrid":
        ...
        print(f"Hybrid mode validated with configuration: {layer_config}")
        
        # Check if mamba layers are present but library not available
        if 'mamba' in layer_config and not MAMBA_AVAILABLE:
            print("Warning: Mamba layers requested but mamba-ssm not available. Using custom implementation.")
...
# Multi-scale convolution components 
