...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_01_skip_standard", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/mnt/fast/nobackup/users/if00208/5_disease_experiments/CNN/results/5d_multilabel/multiscale', help="Directory to save experiment results")
    ...
    # Add residual connection arguments
    parser.add_argument("-use_residual", type=int, default=1, choices=[0, 1], help="Whether to use residual connections in CNN layers (0: no, 1: yes)")
...
class ResidualConnection(nn.Module):
    """Residual connection with automatic dimension, device, and dtype matching.
       Uses interpolation (linear up/downsampling) instead of pooling to align sequence lengths.
    """
    def __init__(self, in_channels, out_channels, stride=1, dropout_rate=0.1):
        super().__init__()
        self.dropout = nn.Dropout(dropout_rate) if dropout_rate > 0 else nn.Identity()

        # Projection if channels or stride differ
        if in_channels != out_channels or stride != 1:
            self.projection = nn.Sequential(
                nn.Conv1d(in_channels, out_channels, kernel_size=1, stride=stride, padding=0, bias=False),
                nn.BatchNorm1d(out_channels)
    ...
    def forward(self, x, residual):
        """
        x: [B, out_channels, L_out]
        residual: [B, in_channels, L_in]
        """
        try:
            # Align device & dtype
            if residual.device != x.device:
                residual = residual.to(x.device, non_blocking=True)
            if residual.dtype != x.dtype:
                residual = residual.to(x.dtype)

            # Project residual to match channel dims
            projected_residual = self.projection(residual)

            # Interpolate if sequence lengths differ
            if projected_residual.size(2) != x.size(2):
                projected_residual = F.interpolate(
                    projected_residual, size=x.size(2), mode="linear", align_corners=False
                )

            # Final sanity check
            if projected_residual.shape != x.shape:
                print(f"[ResidualConnection] Shape mismatch: x {x.shape} vs proj {projected_residual.shape}. Skipping residual.")
                return x

            # Apply dropout and add
            dropped = self.dropout(projected_residual)
            return x.contiguous() + dropped.contiguous()

        except Exception as e:
            print(f"[ResidualConnection] Error: {e}")
            print(f"x shape: {tuple(x.shape)}, residual shape: {tuple(residual.shape)}")
            return x
