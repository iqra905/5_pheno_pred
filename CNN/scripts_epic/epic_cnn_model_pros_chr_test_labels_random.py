...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/pros/pros_0.1/chr_wise/test_labels_random/', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/pros_can_0.1', help="Directory containing genotype files")
    ...
    parser.add_argument("-snp_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/meta_data/first_5_columns_pros_can_0.1.gen', help="Path to SNP information file")
...
class ChromosomeWiseGenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, snp_info, label_column, use_covariates=True, is_test=False):
    ...
    def __getitem__(self, idx):
        ...
        return genotype_dict, covariates_tensor, label
...
def main():
   ...
   # Load SNP info
    ...
    train_dataset = ChromosomeWiseGenotypeDataset(
        ...
        snp_info, label_column=args.label_col, 
        use_covariates=bool(args.cov),
        is_test=False  # Training set uses real labels
        )
    val_dataset = ChromosomeWiseGenotypeDataset(
        val_files, phenotype_data, 
        snp_info, label_column=args.label_col,
        use_covariates=bool(args.cov),
        is_test=False  # Validation set uses real labels
        )
    test_dataset = ChromosomeWiseGenotypeDataset(
        test_files, phenotype_data, 
        snp_info, label_column=args.label_col, 
        use_covariates=bool(args.cov),
        is_test=True  # Test set uses random labels
        )
    ...
    # top_csv_path = os.path.join(experiment_dir,f'top_{top}_indices.csv')
    # df.to_csv(top_csv_path, index=False)

    # print(f"Top {top} indices saved to {experiment_dir} + top_{top}_indices.csv")
