...
# Option 1: Nature Classic (Most Recommended)
# Professional, high contrast, excellent for grayscale conversion
disease_colors_option1 = {
    'Prostate Cancer': '#1f77b4',    # Deep Blue
    'Pancreatic Cancer': '#ff7f0e',  # Orange 
    'Colon Cancer': '#2ca02c',       # Green
    'Breast Cancer': '#d62728',      # Red
    'T2D': '#9467bd'                 # Purple
}

# Option 2: Nature Muted (Sophisticated)
# Softer tones, very professional, easy on the eyes
disease_colors_option2 = {
    'Prostate Cancer': '#4e79a7',    # Steel Blue
    'Pancreatic Cancer': '#f28e2c',  # Burnt Orange
    'Colon Cancer': '#e15759',       # Coral Red
    'Breast Cancer': '#76b7b2',      # Teal
    'T2D': '#59a14f'                 # Forest Green
}

# Option 3: Nature Scientific (Traditional)
# Based on common scientific figure conventions
disease_colors_option3 = {
    'Prostate Cancer': '#003f5c',    # Dark Navy
    'Pancreatic Cancer': '#ff6361',  # Bright Coral
    'Colon Cancer': '#58508d',       # Deep Purple
    'Breast Cancer': '#bc5090',      # Magenta
    'T2D': '#ffa600'                 # Golden Yellow
}

# Option 4: Nature Minimal (Clean & Modern)
# High contrast, minimalist approach
disease_colors_option4 = {
    'Prostate Cancer': '#2E86AB',    # Ocean Blue
    'Pancreatic Cancer': '#A23B72',  # Burgundy
    'Colon Cancer': '#F18F01',       # Amber
    'Breast Cancer': '#C73E1D',      # Crimson
    'T2D': '#5D737E'                 # Slate Gray
}
...
# Option 6: Nature Vibrant (Eye-catching but Professional)
# For presentations and digital-first publications
disease_colors_option6 = {
    'Prostate Cancer': '#1B9E77',    # Teal
    'Pancreatic Cancer': '#D95F02',  # Dark Orange
    'Colon Cancer': '#7570B3',       # Purple Blue
    'Breast Cancer': '#E7298A',      # Pink
    'T2D': '#66A61E'                 # Olive Green
}
...
class NatureGWASVisualizer:
    def __init__(self, excel_path, output_dir):
        ...
        # self.disease_colors = {
        #     'Prostate Cancer': '#2E74C9',    # Blue
        #     'Pancreatic Cancer': '#ff7f0e',  # Orange
        #     'Colon Cancer': '#7f7f7f',       # Gray
        #     'Breast Cancer': '#bcbd22',                 # Olive
        #     'T2D': '#9467bd'    # Red
        # }
        ...
        self.method_colors = {
            ...
            'Disease-wise Single Scale': '#1f77b4',            # Blue
            'Disease-wise Multiscale': '#d62728', #Red
            'Multilabel': '#ff7f0e',           # Orange  
    ...
    def lighten_color(self, hex_color, factor):
        ...
        lightened = [min(255, c + (255 - c) * factor) for c in rgb]
        return self.rgb_to_hex(lightened)
    
    def darken_color(self, hex_color, factor):
        """Darken a hex color by a factor (0-1)"""
        rgb = self.hex_to_rgb(hex_color)
        darkened = [max(0, c * (1 - factor)) for c in rgb]
    ...
    def create_figure01_baseline(self):
        ...
        #ax.set_title('Baseline Method Comparison Across Architectures without Covariates', fontweight='bold', fontsize=10)
        ax.set_xticks(x)
    ...
    def create_figure02_data_leakage(self):
        """Figure 2: Data leakage analysis (Setting02)"""
        ...
        # Subplot 1: Without Covariates
        x = np.arange(len(diseases))
        width = 0.32  # Reduced width for better spacing
        spacing = 1.1  # Add spacing between bars
        
        # Use disease-based colors for bars
        for i, disease in enumerate(diseases):
            base_color = self.disease_colors[disease]
            
            # Population (leakage) - darker shade
            ax1.bar(x[i] - width/2 * spacing, pop_no_cov[i], width, 
                color=self.darken_color(base_color, 0.35), edgecolor='black', linewidth=0.05)
                
            # Label for population bar
            ax1.text(x[i] - width/2 * spacing, pop_no_cov[i] - 0.02, f"{pop_no_cov[i]:.3f}", 
            ha='center', va='top', fontweight='bold', fontsize=5.5)
            
            # Train set (clean) - lighter shade  
            ax1.bar(x[i] + width/2 * spacing, train_no_cov[i], width, 
                color=base_color, edgecolor='black', linewidth=0.05)
            
            # Label for train bar
            ax1.text(x[i] + width/2 * spacing, train_no_cov[i] - 0.02, f"{train_no_cov[i]:.3f}", 
            ha='center', va='top', fontweight='bold', fontsize=5.5)

        # Create custom legend for subplot 1
        legend_elements_1 = [
            plt.Rectangle((0,0),1,1, facecolor=self.darken_color(base_color, 0.35), edgecolor='black', 
                        linewidth=0.05, label='Population Dataset'),
            plt.Rectangle((0,0),1,1, facecolor=base_color, edgecolor='black', 
                        linewidth=0.05, label='Train Set Only')
        ]
        ...
        ax1.set_title('A. Data Leakage Effects (Without Covariates)', fontweight='bold', fontsize=10)
        ax1.set_xticks(x)
        ax1.set_xticklabels(diseases, fontsize=7, color='black', fontweight='bold')
        #ax1.legend(handles=legend_elements_1, fontsize=8)
        ax1.legend(
        ...
        for i, disease in enumerate(diseases):
            ...
            # Label for population bar
            ax2.text(x[i] - width/2 * spacing, pop_cov[i] - 0.02, f"{pop_cov[i]:.3f}", 
            ha='center', va='top', fontweight='bold', fontsize=5.5)
            ...
            # Label for train bar
            ax2.text(x[i] + width/2 * spacing, train_cov[i] - 0.02, f"{train_cov[i]:.3f}", 
            ha='center', va='top', fontweight='bold', fontsize=5.5)
        ...
        ax2.set_title('B. Data Leakage Effects (With Covariates)', fontweight='bold', fontsize=10)
        ax2.set_xticks(x)
        ax2.set_xticklabels(diseases, fontsize=7, color='black', fontweight='bold')
        #ax2.legend(handles=legend_elements_2, fontsize=8)
        ax2.legend(
    ...
    def create_figure03_multilabel(self):
        """Figure 3: Multilabel framework performance"""
        ...
        # Use disease-based colors with different shades
        for i, disease in enumerate(diseases):
            base_color = self.disease_colors[disease]
            
            # Without covariates - lighter shade
            bar1 = ax.bar(x[i] - width/2 * spacing, without_cov[i], width, 
                        color=base_color, edgecolor='black', linewidth=0.05)
            
            # With covariates - darker shade
            bar2 = ax.bar(x[i] + width/2 * spacing, with_cov[i], width, 
                        color=self.darken_color(base_color, 0.3), edgecolor='black', linewidth=0.05)
        ...
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor=base_color, edgecolor='black', 
                        linewidth=0.05, label='No Covariate'),
            plt.Rectangle((0,0),1,1, facecolor=self.darken_color(base_color, 0.3), edgecolor='black', 
                        linewidth=0.05, label='With Covariates')
        ]
        ...
        #ax.set_title('Multi-Disease Framework Performance', fontweight='bold', fontsize=10)
        ax.set_xticks(x)
    ...
    def create_figure04_multiscale(self):
        """Figure 4: Multiscale architecture performance"""
        fig, ax = plt.subplots(figsize=(8, 2.5))
        ...
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor=base_color, edgecolor='black', 
                        linewidth=0.05, label='No Covariates'),
            plt.Rectangle((0,0),1,1, facecolor=self.darken_color(base_color, 0.3), edgecolor='black', 
                        linewidth=0.05, label='With Covariates')
        ]
        ...
        ax.set_ylabel('AUC', fontweight='bold', fontsize=10)
        ax.set_xlabel('Disease', fontweight='bold', fontsize=10)
        #ax.set_title('Multi-Scale Convolutional Architecture Performance', fontweight='bold', fontsize=10)
        ax.set_xticks(x)
        ax.set_xticklabels(diseases, fontsize=8, color='black', fontweight='bold')
        # Apply black color to disease labels
        self.apply_disease_colors_to_labels(ax, diseases)
    ...
    def create_radar_plot(self, data_dict, title, filename, show_covariates_label=False):
        ...
        for method in methods:
            ...
            ax.plot(angles, values, color=self.method_colors[method], linewidth=1.2, 
                marker='o', markersize=3, label=method)
            
            # Add value labels at each point with improved positioning to avoid overlap
            for i, (angle, value, disease) in enumerate(zip(angles[:-1], values[:-1], diseases)):
                
                if method =='Disease-wise':
                    label_radius = value -0.09
                elif method == 'Multilabel':
                    label_radius = value - 0.08
                else:
                    label_radius = value + 0.01
        ...
        ax.set_yticks(np.arange(0.4,1.1, 0.1)) 
        #ax.set_yticklabels(['0.4', '0.6', '0.8', '1.0'], fontsize=6)
        ax.set_yticklabels([f"{tick:.1f}" for tick in np.arange(0.4,1.1, 0.1)], fontsize=6)
        ax.grid(True, alpha=0.4, linewidth=.5)
        ...
        # Add legend
        ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=3, fontsize=8, framealpha=False)
    ...
    def create_figure05_radar_no_cov(self):
        ...
        for i, disease in enumerate(diseases):
            data_dict['Disease-wise Single Scale'][disease] = df.iloc[i]['Disease-wise Single Scale']
            #data_dict['Disease-wise Multiscale'][disease] = df.iloc[i]['Disease-wise Multiscale']
            data_dict['Multilabel'][disease] = df.iloc[i]['Multilabel'] 
            data_dict['Multilabel + Multiscale'][disease] = df.iloc[i]['Multiscale']
