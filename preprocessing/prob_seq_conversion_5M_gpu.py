...
parser.add_argument('-reference_file', type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/sampled_data_5M_disease_wise_pruned/first_5_columns_5M_updated_unq_gen_pros_0.01.gen', help='Path to the folder containing .mat sample files')
parser.add_argument('-sample_folder', type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/sampled_data_5M_disease_wise_pruned/pros/0.01', help='Path to the output folder')
parser.add_argument('-output_base_folder', type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/sampled_data_5M_disease_wise_pruned/pros_seq/0.01', help='Path to the metadata file')
parser.add_argument('-chunk_size', type=int, default=300, help='Number of samples to process in each chunk')
parser.add_argument('-batch_size', type=int, default=50000, help='Number of SNPs to process at once on GPU')
parser.add_argument('-max_processes', type=int, default=None, help='Maximum number of processes to use')
parser.add_argument('-use_gpu', type=int, default=0, choices=[0, 1], help='Use GPU acceleration (0=off, 1=on)')
...
def load_reference_data(reference_file):
    ...
    chunk_size = 50000  # Read 1M lines at a time
    ...
    #print(f"Reference data loaded in {elapsed_time:.2f} seconds.")
    print(f"Found data for {len(chr_to_line_map)} chromosomes: {', '.join(sorted(chr_to_line_map.keys()))}")
...
def process_sample_file_gpu(args):
    """Process a sample file using GPU acceleration with PyTorch."""
    sample_file, ref_temp_file, output_base_folder, gpu_id, batch_size = args
    
    # Set the GPU device
    device = torch.device(f'cuda:{gpu_id}' if torch.cuda.is_available() else 'cpu')
    if device.type == 'cpu':
        print(f"Warning: GPU not available, falling back to CPU processing for {sample_file}")
    ...
    try:
        ...
        # Process the file in batches for GPU processing
        with gzip.open(sample_file, 'rt') as in_f:
            lines_read = 0
            processed_lines = 0
            ...
            while True:
                ...
                if batch_lines:
                    # Create arrays for CPU->GPU transfer
                    batch_size_actual = len(batch_lines)
                    
                    # Prepare array of probabilities (3 probs per SNP)
                    probs_array = np.zeros((batch_size_actual, 3), dtype=np.float32)
                    
                    # Extract probabilities from each line
                    for i, line_data in enumerate(batch_lines):
                        if len(line_data) >= 3:  # Ensure we have at least 3 values
                            probs = list(map(float, line_data))[:3]  # Take first 3 values
                            probs_array[i, :len(probs)] = probs
                    
                    # Transfer to GPU using PyTorch
                    d_probs = torch.tensor(probs_array, device=device)
                    
                    # Find max probability position (0=hom_dom, 1=het, 2=hom_rec)
                    max_indices = torch.argmax(d_probs, dim=1).cpu().numpy()
                    
                    # Process results back on CPU
                    for i, max_idx in enumerate(max_indices):
                        line_idx = batch_indices[i]
                        ...
                        try:
                            ...
                            # Determine genotype based on max probability position
                            if max_idx == 0:  # Homozygous dominant
                                genotype = ref_allele + ref_allele
                            elif max_idx == 1:  # Heterozygous
                            ...
                            print(f"Warning: Error processing line for chromosome {chromosome}: {e}")
                    
                    processed_lines += batch_size_actual
                
                # # Report progress
                # if total_lines > 0:
                #     print(f"\rProcessing {sample_file}: {processed_lines}/{total_lines} lines "f"({processed_lines/total_lines*100:.1f}%)", end="")
                # else:
    ...
    for chromosome, genotypes in sorted(chr_genotypes.items(), 
    ...
    elapsed_time = time.time() - start_time
    #print(f"Processed file {base_name} in {elapsed_time:.2f} seconds")
    
    return results

def process_sample_file_cpu(args):
    """Process a sample file using CPU."""
...
def process_files_in_chunks(sample_files, ref_temp_file, output_base_folder, chunk_size, max_processes, use_gpu=0, gpu_id=0, batch_size=50000):
    ...
    # If using GPU, detect available GPUs
    if use_gpu == 1:
        try:
            # Check if PyTorch can see any GPUs
            if torch.cuda.is_available():
                n_gpus = torch.cuda.device_count()
                #print(f"Detected {n_gpus} CUDA-capable GPU(s)")
                if gpu_id >= n_gpus:
                   # print(f"Warning: Requested GPU ID {gpu_id} not available. Using GPU 0 instead.")
                    gpu_id = 0
                
                # Get GPU info
                device = torch.cuda.get_device_properties(gpu_id)
                print(f"Using GPU {gpu_id}: {device.name} with {device.total_memory/1024/1024/1024:.1f} GB memory")
                
                # Use a single process when using GPU to avoid memory conflicts
                if n_processes > 1:
                    print(f"Note: Using single process mode with GPU acceleration")
                    n_processes = 1
            else:
                print("Warning: No CUDA-capable GPUs detected. Falling back to CPU processing.")
                use_gpu = 0
        except Exception as e:
    ...
    #print(f"Processing with {n_processes} processes, chunk size: {chunk_size}, {'GPU' if use_gpu == 1 else 'CPU'} mode")
    ...
    for i in range(0, len(sample_files), chunk_size):
        ...
        if use_gpu == 1:
            # For GPU processing, add GPU ID and batch size to args
            args_list = [(file, ref_temp_file, output_base_folder, gpu_id, batch_size) for file in chunk]
            
            # Process files sequentially or in parallel depending on number of GPUs
            if n_processes > 1:
                with mp.Pool(processes=n_processes) as pool:
                    chunk_results = pool.map(process_sample_file_gpu, args_list)
            else:
                # Process sequentially on GPU
                chunk_results = [process_sample_file_gpu(args) for args in args_list]
        else:
            # For CPU processing
            ...
            with mp.Pool(processes=n_processes) as pool:
                chunk_results = pool.map(process_sample_file_cpu, args_list)
        ...
        files_per_second = len(chunk) / chunk_time if chunk_time > 0 else 0
        elapsed_total = time.time() - total_start_time
        est_remaining = (total_files - files_processed) / files_per_second if files_per_second > 0 else 0
        
        print(f"\nProgress: {files_processed}/{total_files} files ({files_processed/total_files*100:.1f}%)")
        #print(f"Chunk processing time: {chunk_time:.2f} seconds ({files_per_second:.2f} files/second)")
       # print(f"Elapsed time: {elapsed_total:.2f} seconds")
       # print(f"Estimated remaining time: {est_remaining:.2f} seconds")
   ...
   # print(f"Average processing speed: {files_processed/total_time:.2f} files/second")
    print("=" * 80)

def main(reference_file, sample_folder, output_base_folder, chunk_size=1000, max_processes=None, use_gpu=0, gpu_id=0, batch_size=50000):
    """Main function to process all sample files."""
    # Load reference data
    ...
    try:
        ...
        # Process files in chunks
        process_files_in_chunks(sample_files, ref_temp_file, output_base_folder, 
                               chunk_size, max_processes, use_gpu, gpu_id, batch_size)
