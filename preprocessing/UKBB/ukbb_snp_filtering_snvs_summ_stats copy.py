...
OUTPUT_BASE = "/mnt/fast/datasets/ucdatasets/gwas/ukbb/samples_chr_wise_uint8/snvs_summ_stats_filtered_0.05"
...
def stage2_gwas_filter(chr_num, disease_key,
    ...
    Matching: chromosome + bp position only (alleles ignored).
    ...
    print(f"    Matching: position-only (chr + bp), alleles ignored")
    ...
    # ------------------------------------------------------------------
    # Build a lookup from the SNV-filtered variants:
    #   key  = (chromosome_str, bp_int)
    #   value = row-position inside snv_variants (0-based)
    # ------------------------------------------------------------------
    snv_bp = pd.to_numeric(snv_variants['bp'], errors='coerce').fillna(-1).astype(np.int32)
    snv_chr = snv_variants['chromosome'].astype(str)

    snv_lookup = {}   # (chr_str, bp_int) -> list of snv-row positions
    for row_pos, (ch, bp) in enumerate(zip(snv_chr, snv_bp)):
        key = (ch, int(bp))
        if key not in snv_lookup:
            snv_lookup[key] = row_pos   # keep first match (duplicates unlikely after SNV filter)

    # ------------------------------------------------------------------
    # Stream GWAS file in chunks; count before/after threshold; collect matches
    # ------------------------------------------------------------------
    total_gwas_before = 0
    total_gwas_after  = 0
    matched_snv_positions = set()
    matched_gwas_rows     = []
    ...
    for gwas_chunk in gwas_reader:
        ...
        # Match against SNV lookup
        for _, gwas_row in gwas_chunk.iterrows():
            key = (str(gwas_row[chr_col]), int(gwas_row[pos_col]))
            if key in snv_lookup:
                snv_pos = snv_lookup[key]
                if snv_pos not in matched_snv_positions:
                    matched_snv_positions.add(snv_pos)
                    matched_gwas_rows.append((snv_pos, gwas_row))
    ...
    stats = {
        ...
        'matching_strategy':                  'position_only_two_stage',
    ...
    pvalues = []
    for snv_pos, gwas_row in matched_gwas_rows:
        try:
    ...
    if 'gen' in output_formats:
        ...
        gwas_extras = []
        for snv_pos, gwas_row in sorted(matched_gwas_rows, key=lambda x: x[0]):
