#!/usr/bin/env python3
"""
GWAS Disease-Specific SNP Filtering - Tiered Matching with Complete Statistics

Matching tiers:
  1. Position + SNP ID (chr + bp + rsid) for SNPs with valid rsids
  2. Position + Alleles (chr + bp + alleles) for multiallelic disambiguation
  3. Position-only (chr + bp) as fallback
...
OUTPUT_BASE = "/mnt/fast/datasets/ucdatasets/gwas/ukbb/samples_chr_wise_uint8/filtered_by_disease_gwas_summ_stats_5e8_threshold_top_snps"
...
def process_chromosome_with_snp_id(chr_num, disease_key, base_path, output_base,
                                    use_threshold, pvalue_threshold,
                                    output_formats=['npy', 'gen'], chunksize=500000):
    """
    Process using TIERED matching strategy:
    1. Position + SNP ID (chr + bp + rsid) for SNPs with valid rsids
    2. Position + Alleles (chr + bp + alleles) for SNPs with missing rsids at multiallelic sites
    3. Position-only (chr + bp) fallback as last resort
    ...
    print(f"    1. Position + SNP ID (chr + bp + rsid) for valid rsids")
    print(f"    2. Position + Alleles (chr + bp + a1 + a2) for multiallelic disambiguation")
    print(f"       (Skipped if UKBB alleles contain commas or other invalid chars)")
    print(f"    3. Position-only (chr + bp) as absolute fallback")
    if snp_id_col:
        print(f"    SNP ID column in GWAS: {snp_id_col}")

    print(f"  Note: UKBB alleles often contain ',' placeholders (multiallelic)")    
    ...
    for ukbb_chunk in ukbb_reader:
        ...
        for gwas_chunk in gwas_reader2:
            ...
            # Strategy 1: Try matching with SNP ID if available and valid
            merged_df = pd.DataFrame()
            matched_by_rsid = set()
            
            if snp_id_col and snp_id_col in gwas_chunk.columns and 'rsid' in ukbb_chunk.columns:
                # Standardize SNP ID format
                gwas_chunk[snp_id_col] = gwas_chunk[snp_id_col].astype(str)
                ukbb_chunk['rsid'] = ukbb_chunk['rsid'].astype(str)
                
                # Filter for valid rsids (starts with 'rs' and not just '1')
                gwas_valid_rsid = gwas_chunk[(gwas_chunk[snp_id_col].str.startswith('rs', na=False))].copy()
                
                if len(gwas_valid_rsid) > 0:
                    # Try matching with SNP ID + position
                    merged_df = pd.merge(
                        ukbb_chunk,
                        gwas_valid_rsid,
                        left_on=['chromosome', 'bp', 'rsid'],
                        right_on=[chr_col, pos_col, snp_id_col],
                        how='inner'
                    )
                    if len(merged_df) > 0 and 'SNP_Index' in merged_df.columns:
                        matched_by_rsid.update(merged_df['SNP_Index'].tolist())
            
            # Strategy 2: For SNPs without valid rsids, try matching with alleles for multiallelic disambiguation
            gwas_invalid_rsid = gwas_chunk[~(gwas_chunk[snp_id_col].str.startswith('rs', na=False))].copy() if snp_id_col else gwas_chunk
            
            if len(gwas_invalid_rsid) > 0 and allele1_col in gwas_invalid_rsid.columns and allele2_col in gwas_invalid_rsid.columns:
                # Only attempt allele matching if UKBB alleles are valid (no commas or other placeholders)
                # Check if UKBB alleles contain commas or other invalid characters
                ukbb_has_valid_alleles = (
                    'ref_allele' in ukbb_chunk.columns and 
                    'alt_allele' in ukbb_chunk.columns and
                    ~ukbb_chunk['ref_allele'].astype(str).str.contains(',', na=False).any() and
                    ~ukbb_chunk['alt_allele'].astype(str).str.contains(',', na=False).any()
                )
                
                if ukbb_has_valid_alleles:
                    # Try allele-based matching for multiallelic site disambiguation
                    gwas_invalid_rsid[allele1_col] = gwas_invalid_rsid[allele1_col].astype(str)
                    gwas_invalid_rsid[allele2_col] = gwas_invalid_rsid[allele2_col].astype(str)
                    ukbb_chunk['ref_allele'] = ukbb_chunk['ref_allele'].astype(str)
                    ukbb_chunk['alt_allele'] = ukbb_chunk['alt_allele'].astype(str)
                    
                    # Match 1: Forward strand (GWAS allele1 = UKBB ref, GWAS allele2 = UKBB alt)
                    merged_df_alleles_fwd = pd.merge(
                        ukbb_chunk,
                        gwas_invalid_rsid,
                        left_on=['chromosome', 'bp', 'ref_allele', 'alt_allele'],
                        right_on=[chr_col, pos_col, allele1_col, allele2_col],
                        how='inner'
                    )
                    
                    # Match 2: Reverse strand (GWAS allele1 = UKBB alt, GWAS allele2 = UKBB ref)
                    merged_df_alleles_rev = pd.merge(
                        ukbb_chunk,
                        gwas_invalid_rsid,
                        left_on=['chromosome', 'bp', 'alt_allele', 'ref_allele'],
                        right_on=[chr_col, pos_col, allele1_col, allele2_col],
                        how='inner'
                    )
                    
                    # Combine allele-matched results
                    merged_df_alleles = pd.concat([merged_df_alleles_fwd, merged_df_alleles_rev], ignore_index=True)
                    
                    if len(merged_df_alleles) > 0:
                        # Remove UKBB SNPs already matched by rsid
                        if 'SNP_Index' in merged_df_alleles.columns:
                            merged_df_alleles = merged_df_alleles[~merged_df_alleles['SNP_Index'].isin(matched_by_rsid)]
                        
                        if len(merged_df_alleles) > 0:
                            merged_df = pd.concat([merged_df, merged_df_alleles], ignore_index=True)
            
            # Strategy 3: Fall back to position-only for SNPs without valid rsids or alleles
            if len(merged_df) == 0 or snp_id_col is None:
                # Position-only matching (last resort - may match multiple variants at multiallelic sites)
                merged_df = pd.merge(
                    ukbb_chunk,
                    gwas_chunk,
                ...
                # Also try position-only matching for any remaining unmatched SNPs without valid rsids or alleles
                gwas_pos_only = gwas_invalid_rsid.copy()
                if allele1_col in gwas_pos_only.columns and allele2_col in gwas_pos_only.columns:
                    # Remove SNPs that already matched via alleles
                    if len(merged_df) > 0:
                        # Get GWAS rows that are in merged_df
                        gwas_merged_rows = merged_df[[chr_col, pos_col] + ([allele1_col] if allele1_col in merged_df.columns else [])].drop_duplicates()
                        # This is complex, so as alternative just mark matched indices
                        if 'SNP_Index' in merged_df.columns:
                            matched_indices_sofar = set(merged_df['SNP_Index'].tolist())
                
                merged_df_pos_only = pd.merge(
                    ukbb_chunk,
                    gwas_pos_only,
                    left_on=['chromosome', 'bp'],
                    right_on=[chr_col, pos_col],
                ...
                # Remove rows already matched by rsid or alleles
                if len(merged_df_pos_only) > 0 and 'SNP_Index' in merged_df_pos_only.columns and 'SNP_Index' in merged_df.columns:
                    merged_df_pos_only = merged_df_pos_only[~merged_df_pos_only['SNP_Index'].isin(merged_df['SNP_Index'])]
                
                if len(merged_df_pos_only) > 0:
                    merged_df = pd.concat([merged_df, merged_df_pos_only], ignore_index=True)
    ...
    # Save matched rows for this chromosome as CSV (to be aggregated per-disease later)
    matched_csv = os.path.join(disease_dir, f"chr{chr_num}_matched_{disease_key}.csv")

    output_df = final_matches.copy()
    if 'SNP_Index' in output_df.columns:
        output_df = output_df.rename(columns={'SNP_Index': 'original_idx'})

    # Ensure we write a CSV with standard comma delimiter
    output_df.to_csv(matched_csv, index=False)
    output_files['matched_csv'] = matched_csv
    ...
    stats = {
        ...
        'matching_strategy': 'tiered_rsid_alleles_position',
        'strategy_description': 'Tier 1: rsid+pos, Tier 2: alleles+pos, Tier 3: pos only',
        'snp_id_column': snp_id_col,
...
def create_summary_files(all_stats, diseases, output_base, pvalue_threshold, use_threshold):
    ...
    for disease_key in diseases:
        ...
        summary_json_data = {
            ...
            'matching_strategy': 'tiered_rsid_alleles_position',
            'strategy_description': 'Tier 1: SNP ID + position when rsid valid. Tier 2: Alleles + position for multiallelic disambiguation when rsid missing. Tier 3: Position only as fallback.',
            'p_value_threshold': pvalue_threshold if use_threshold else None,
        ...
        # ------------------------------------------------------------------
        # Aggregate per-chromosome matched CSVs into a single per-disease CSV
        # ------------------------------------------------------------------
        print(f"\n    Aggregating matched SNPs into single CSV for {disease_name}...")
        matched_pattern = os.path.join(disease_dir, 'chr*', f"chr*_matched_{disease_key}.csv")
        matched_files = glob.glob(matched_pattern)

        if not matched_files:
            print(f"    No matched CSVs found for {disease_name}, skipping aggregation.")
            continue
        ...
        if len(parts) == 0:
            print(f"    No valid matched CSVs to aggregate for {disease_name}.")
            continue

        agg_df = pd.concat(parts, ignore_index=True)

        # Ensure chromosome is numeric for proper sorting
        if 'chromosome' in agg_df.columns:
            agg_df['chromosome'] = pd.to_numeric(agg_df['chromosome'], errors='coerce')
        else:
            # Fallback: try to find any column that looks like a chromosome column
            for c in agg_df.columns:
                if c.lower() in ('chr', 'chrom', 'chromosome'):
                    agg_df['chromosome'] = pd.to_numeric(agg_df[c], errors='coerce')
                    break

        pval_col = DISEASE_CONFIGS[disease_key]['column_mappings'].get('pvalue')

        sort_cols = ['chromosome']
        if pval_col and pval_col in agg_df.columns:
            # Ensure p-value numeric
            agg_df[pval_col] = pd.to_numeric(agg_df[pval_col], errors='coerce')
            sort_cols.append(pval_col)

        agg_df = agg_df.sort_values(by=sort_cols, ascending=[True] * len(sort_cols))

        aggregated_csv = os.path.join(disease_dir, f"{disease_key}_filtered_snps.csv")
        agg_df.to_csv(aggregated_csv, index=False)
        print(f"    Saved aggregated SNP CSV: {aggregated_csv}")


def main():
    parser = argparse.ArgumentParser(
        description='Filter UKBB SNPs by GWAS - Position + SNP ID Matching with Statistics'
    )
    ...
    parser.add_argument('-use_threshold', action='store_true', dest='use_threshold', default=True, help='Enable p-value thresholding (default: True)')
    parser.add_argument('-skip_threshold', action='store_false', dest='use_threshold', help='Skip p-value thresholding')
    parser.add_argument('-threshold', type=float, default=5e-8, help='P-value threshold if -use_threshold is enabled (default: 5e-8)')
    ...
    print("\n" + "="*80)
    print("GWAS FILTERING - Tiered Matching (rsid >> alleles >> position)")
    print("="*80)
    print(f"\nMatching Strategy: TIERED")
    print(f"Tier 1: chr + bp + SNP ID (for valid rsids starting with 'rs')")
    print(f"Tier 2: chr + bp + alleles (for multiallelic sites with missing rsids)")
    print(f"        [SKIPPED if UKBB alleles contain commas or other invalid characters]")
    print(f"Tier 3: chr + bp only (fallback for SNPs without rsid or valid allele info)")
    print(f"\nThis handles:")
    print(f"  - Missing rsids in GWAS files (e.g., shows '1' instead of rsid)")
    print(f"  - Invalid UKBB alleles (commas ',' indicating multiallelic variants)")
    print(f"  - Multiallelic sites: Tier 2 disambiguates when alleles are valid")
    print(f"  - Maximum sensitivity: fallback tier catches remaining matches")
    print(f"\nConfiguration:")
    ...
    for disease_key in diseases:
        ...
        for chr_num in chromosomes:
            stats = process_chromosome_with_snp_id(
                chr_num, disease_key,
