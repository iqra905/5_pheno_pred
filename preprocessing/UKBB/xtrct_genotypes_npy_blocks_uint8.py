...
Storage format: uint8 (probabilities scaled 0-255)
- 50% smaller than float16
- To recover probabilities: data.astype(np.float32) / 255.0
- Max precision loss: ~0.004 (negligible for genomic analyses)
"""
...
def process_chromosome(chromosome, base_path, file_pattern, output_dir, variant_block_size=5000):
    ...
    try:
        ...
        with open_bgen(str(filepath)) as bgen:
            ...
            # Estimate memory usage (uint8: 1 byte per value)
            memory_per_block_gb = (n_samples * variant_block_size * 3 * 1) / (1024**3)
            ...
            for sample_idx in tqdm(range(n_samples), desc="Creating files", leave=False):
                ...
                # Create NPY file with header (uint8)
                with open(sample_file, 'wb') as f:
                    header = {
                        'descr': np.dtype(np.uint8).descr[0][1],
                        'fortran_order': False,
                        'shape': (n_variants, 3)
                    }
            ...
            for block_idx in tqdm(range(n_blocks), desc=f"Chr{chromosome}"):
                ...
                # Using uint8 to save 50% memory/storage
                block_data = np.zeros((n_samples, block_n_variants, 3), dtype=np.uint8)
                ...
                for i, var_idx in enumerate(range(start_var, end_var)):
                    ...
                    # Scale probabilities from [0, 1] to [0, 255] and store as uint8
                    block_data[:, i, :] = (probs * 255).astype(np.uint8)
...
Output Structure:
  output_dir/
    chr1/
      sample_<id>_chr1.npy  (n_variants x 3 array of uint8, scaled 0-255)
...
Data Format:
  - Genotype probabilities stored as uint8 (0-255) instead of float
  - 50% smaller files than float16
  - To recover probabilities: data.astype(np.float32) / 255.0
  - Max precision loss: ~0.004 (negligible for genetic analyses)
        """
    )
