...
OUTPUT_BASE = "/mnt/fast/datasets/ucdatasets/gwas/ukbb/samples_chr_wise_uint8/snvs_summ_stats_filtered_0.005"
...
def stage2_gwas_filter(chr_num, disease_key,
    ...
    using a three-tier matching strategy:

      Tier 1 — chr + bp + rsid         (GWAS rows where snp_id starts with 'rs')
      Tier 2 — chr + bp + alleles      (forward AND reverse strand, for rows without
                                        valid rsids; safe because Stage 1 guarantees
                                        clean single-char UKBB alleles)
    ...
    snp_id_col  = col_map.get('snp_id')   # optional — None if not configured

    print(f"\n  [Stage 2] {disease_name} — chr{chr_num}")
    print(f"    Matching strategy: TIERED")
    print(f"      Tier 1: chr + bp + rsid          (valid rsids only)")
    print(f"      Tier 2: chr + bp + alleles        (fwd + rev strand)")
    print(f"      Tier 3: chr + bp position-only    (fallback)")
    if snp_id_col:
        print(f"    GWAS rsid column: {snp_id_col}")
    else:
        print(f"    No snp_id configured — Tier 1 skipped, Tier 2+3 only")
    ...
    snv_rsid  = snv_variants['rsid'].astype(str)
    ...
    for row_pos, (ch, bp, rsid, ref, alt) in enumerate(
            zip(snv_chr, snv_bp, snv_rsid, snv_ref, snv_alt)):
        ch  = str(ch)
        bp  = int(bp)
        ref = str(ref).upper()
        alt = str(alt).upper()
        ...
        # Tier 1 — only for valid rsids
        if rsid.startswith('rs'):
            if (ch, bp, rsid) not in rsid_lookup:
                rsid_lookup[(ch, bp, rsid)] = row_pos

    print(f"    SNV lookup built — "
          f"Tier1: {len(rsid_lookup):,}  "
    ...
    for gwas_chunk in gwas_reader:
        ...
        if snp_id_col and snp_id_col in gwas_chunk.columns:
        ...
        for _, gwas_row in gwas_chunk.iterrows():
            ...
            # --- Tier 1: chr + bp + rsid ---
            if snp_id_col and snp_id_col in gwas_chunk.columns:
                rsid = str(gwas_row[snp_id_col])
                if rsid.startswith('rs'):
                    snv_pos = rsid_lookup.get((ch, bp, rsid))
                    if snv_pos is not None:
                        tier_used = 1

            # --- Tier 2: chr + bp + alleles (forward then reverse) ---
            if snv_pos is None and has_allele_cols:
    ...
    stats = {
        ...
        'matching_strategy':                  'tiered_rsid_allele_position',
        'tier1_rsid_matches':                 tier_counts[1],
...
def create_gwas_summary(all_gwas_stats, diseases, gwas_output_dir,
    ...
    for disease_key in diseases:
        ...
        print(f"        Tier 1 (rsid):     {total_tier1:,}")
...
Examples:
  ...
  %(prog)s -base_path /data/ukbb -output_base /results -output_formats npy,txt,gen
    ...
    print(f"\nStage 2 — GWAS Summary Stats Filtering")
    print(f"  Matching: chromosome + bp position (alleles ignored)")
