...
def extract_significant_snps():
    # File paths
    file1_path = "/vol/research/ucdatasets/gwas/gwas_mono_rm/meta_data/first_5_columns_pros_can.gen"
    file2_path = "/vol/research/ucdatasets/gwas/data_files/5D_snp_info_files/meta_v3_onco_euro_overall_ChrAll_1_release.txt"
    output_path = "/vol/research/ucdatasets/gwas/data_files/5D_snp_info_files/extracted/pros/pros_0.05.csv"
    ...
    file2_df['Pvalue'] = pd.to_numeric(file2_df['Pvalue'], errors='coerce')
    ...
    # Filter file2 for P-values < 0.05
    file2_df = file2_df[file2_df['Pvalue'] < 0.05].copy()
    
    # Print filtered file2 information
    print(f"After filtering for P-value < 0.05: {len(file2_df)} rows")
    print("-" * 50)
    ...
    file2_df['position'] = file2_df['position'].astype(int)
    ...
    file2_df['Allele1_upper'] = file2_df['Allele1'].str.upper()
    file2_df['Allele2_upper'] = file2_df['Allele2'].str.upper()
    ...
    print(f"- Found {len(file2_df)} SNPs with P-value < 0.05 in file 2")
