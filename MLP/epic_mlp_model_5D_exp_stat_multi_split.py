...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/mlp_res_splits/split_100', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/geno_ml_filtered_splits/split_100', help="Directory containing genotype files")
    ...
    parser.add_argument("-train_indices", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/stat_res_splits/split_100/train_indicies.npy', help="Path to training.npy file containing training indices")
    parser.add_argument("-test_indices", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/stat_res_splits/split_100/test_indicies.npy', help="Path to test.npy file containing test indices")
...
def load_split_indices(args):
    """Load train/test split indices from .npy files"""
    try:
        # train_indices = np.load(os.path.join(args.exp_dir, 'training.npy'))
        # test_indices = np.load(os.path.join(args.exp_dir, 'test.npy'))

        train_indices = np.load(args.train_indices)
        test_indices = np.load(args.test_indices)

        # train_indices = np.load('/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/stat_res_splits/split_100/train_indicies.npy')
        # test_indices = np.load('/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/pros/stat_res_splits/split_100/test_indicies.npy')
        print(f"Loaded split indices - Train: {len(train_indices)}, Test: {len(test_indices)}")

        # Check if test_indices is empty
        if len(test_indices) == 0:
            print("Test indices file is empty. Randomly selecting 20% of training data as test set.")
            # Calculate number of samples for test set
            n_test = int(len(train_indices) * 0.2)
            ...
            # Sort indices to maintain original order within each set
            train_indices.sort()
            test_indices.sort()
            
            # Save the new split
            np.save(args.train_indices, train_indices)
            np.save(args.test_indices, test_indices)
            print(f"Saved new random split indices to the original file locations")
        
        print(f"Final split - Train: {len(train_indices)}, Test: {len(test_indices)} samples")
        return train_indices, test_indices
        
    except Exception as e:
        raise Exception(f"Error loading split indices: {e}")
...
def main():
    ...
    train_indices, test_indices = load_split_indices(args)
    ...
    for dataset_name, dataset in [("Train", train_dataset), ("Test", test_dataset)]:
        ...
        for i in range(5):  # Print info for 3 items from each dataset
