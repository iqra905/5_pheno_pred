...
def parse_args():
    ...
    parser.add_argument("-dropout", type=float, default=0.9, help="Dropout rate for the model")
    ...
    parser.add_argument("-wd", type=float, default=5, help="Weight decay for optimizer")
...
class SNPsOnlyModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_pca=True):
        ...
        # Increasing dropout rates for each layer
        dropout_rates = [dropout_rate * (i+1)/len(hidden_sizes) for i in range(len(hidden_sizes))]
        ...
        for i in range(len(hidden_sizes)):
            layers.extend([
                ...
                (f'batchnorm_{i}', nn.BatchNorm1d(hidden_sizes[i])),
                (f'activation_{i}', self.get_activation(act)),
                (f'dropout_{i}', nn.Dropout(dropout_rates[i]))
            ])
            current_size = hidden_sizes[i]
        ...
        print(f"Using progressive dropout rates: {dropout_rates}")
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', save_dir=None, early_stopping=None, covariates_only=False, snps_only=False, l1_lambda=0.0001):  # Add L1 regularization parameter
        ...
        for phase in ['train', 'test']:
            ...
            for batch in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # Apply L1 regularization during training
                    if phase == 'train':
