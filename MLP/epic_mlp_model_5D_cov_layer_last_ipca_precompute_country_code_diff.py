...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="01_snps_diff", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/5d_exp_country_code/col/diff_country/', help="Directory to save experiment results")
    ...
    parser.add_argument("-train_phenotype_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/data_files/disease_pheno/country_coded/col/col_can_4.xlsx', help="Path to phenotype file")
    parser.add_argument("-test_phenotype_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/data_files/disease_pheno/country_coded/col/col_can_7.xlsx', help="Path to phenotype file")
    ...
    parser.add_argument("-pca_features_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/col_pca', help="Directory containing pre-computed PCA features")
...
def main():
    ...
    train_phenotype_file = args.train_phenotype_file 
    test_phenotype_file = args.test_phenotype_file   
    batch_size = args.bs
    ...
    # Load phenotype data for training and testing
    train_phenotype_data = pd.read_excel(train_phenotype_file)
    test_phenotype_data = pd.read_excel(test_phenotype_file)
    print(f"Training phenotype data loaded, shape: {train_phenotype_data.shape}")
    print(f"Testing phenotype data loaded, shape: {test_phenotype_data.shape}")
    ...
    print(f"Total number of genotype files found: {len(file_list)}")

    # Get the unique sample IDs from each phenotype file
    train_samples = set(train_phenotype_data['new_order'].unique())
    test_samples = set(test_phenotype_data['new_order'].unique())
    print(f"Number of unique samples in training phenotype data: {len(train_samples)}")
    print(f"Number of unique samples in testing phenotype data: {len(test_samples)}")

    # Check for any overlap between training and testing samples
    overlap = train_samples.intersection(test_samples)
    if overlap:
        print(f"WARNING: {len(overlap)} samples overlap between training and testing sets")


    # Filter file lists for training and testing
    train_file_list = []
    test_file_list = []
    ...
    for file_path in file_list:
        ...
        elif sample_id in test_samples:
            test_file_list.append(file_path)

    print(f"Number of genotype files for training: {len(train_file_list)}")
    print(f"Number of genotype files for testing: {len(test_file_list)}")

    # Verify we have files for all samples
    if len(train_file_list) != len(train_samples):
        print(f"WARNING: Found {len(train_file_list)} files for {len(train_samples)} training samples")
    if len(test_file_list) != len(test_samples):
        print(f"WARNING: Found {len(test_file_list)} files for {len(test_samples)} testing samples")

    # Get a sample file to determine input size
    first_genotype_file = train_file_list[0] if train_file_list else None
    if first_genotype_file:
        print(f"Using file for input size determination: {first_genotype_file}")
    else:
        raise ValueError("No training genotype files found!")
    ...
    scheduler = get_scheduler(sch, optimizer, args, train_file_list)
