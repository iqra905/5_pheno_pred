...
def parse_args():
    ...
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/pros_can_encoded', help="Directory containing genotype files")
...
class GenotypeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Process covariates [This part remains the same]
        covariates_list = []
        if self.use_covariates:
            pc_data = np.array([
        ...
        # Modified genotype data loading for single column
        mmap_file = self.file_handles[genotype_file]
        mmap_file.seek(0)
        with gzip.GzipFile(fileobj=mmap_file) as f:
            # Now reading single column of encoded genotypes
            data = pd.read_csv(f, sep=r'\s+', header=None)  # Using squeeze=True to get Series
            genotype_data = data.values.reshape(-1)  # Ensure it's a flat array
            genotype_tensor = torch.tensor(genotype_data, dtype=torch.float32)
...
class GenotypeModel(nn.Module):
    ...
    def forward(self, x, covariates=None):
        ...
        if x.dim() == 2 and x.size(1) == 1:
            x = x.squeeze(1)  # Remove the second dimension if it's [N, 1]
...
def debug_shapes(batch_data, batch_idx=0):
    if len(batch_data) == 3:
        genotype, covariates, label = batch_data
        print(f"Batch {batch_idx}:")
        print(f"  Genotype shape: {genotype.shape}")
        print(f"  Covariates shape: {covariates.shape}")
        print(f"  Label shape: {label.shape}")
    else:
        covariates, label = batch_data
        print(f"Batch {batch_idx}:")
        print(f"  Covariates shape: {covariates.shape}")
        print(f"  Label shape: {label.shape}")
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', save_dir=None, early_stopping=None, covariates_only=False):
    ...
    # Add debug prints for the first batch
    first_batch = next(iter(dataloaders['train']))
    print("\nFirst batch shapes:")
    debug_shapes(first_batch)

    # Reset iterator
    dataloaders['train'] = DataLoader(dataloaders['train'].dataset, 
