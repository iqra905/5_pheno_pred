...
def parse_args():
    ...
    parser.add_argument("-use_wandb", type=int, default=1, choices=[0, 1], 
                        help="Whether to use Weights & Biases for tracking (0: no, 1: yes)")
    parser.add_argument("-wandb_project", type=str, default="genotype-disease-prediction with MLP", 
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/5d_exp_5M_pruned/pros/0.01/', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/sampled_data_5M_disease_wise_pruned/pros/0.01', help="Directory containing genotype files")
    parser.add_argument("-phenotype_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/data_files/disease_pheno_updated/pros_can.xlsx', help="Path to phenotype file")
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', 
                ...
                classification_threshold=0.5, use_wandb=True):
        ...
        for phase in ['train', 'test']:
            ...
            if use_wandb:
                metrics_dict = {
                    ...
                    f'{phase}_pr_auc': epoch_pr_auc
                }
                
                # Only log train/test prefix for test phase to avoid duplicate keys
                if phase == 'test':
                    metrics_dict.update({
                        'train/loss': history['train_loss'][-1],
                        'train/acc': history['train_acc'][-1],
                        'train/auc': history['train_auc'][-1],
                        'train/f1': history['train_f1'][-1],
                        'train/pr_auc': history['train_pr_auc'][-1],
                        'test/loss': epoch_loss,
                        'test/acc': epoch_acc.item(),
                        'test/auc': epoch_auc,
                        'test/f1': epoch_f1,
                        'test/pr_auc': epoch_pr_auc,
...
def plot_all_metrics(history, final_metrics, save_dir, use_wandb=True):
    phases = ['train', 'test']
    ...
    plt.grid(True)
    pr_plot_path = os.path.join(save_dir, 'pr_curve.png')
    plt.savefig(pr_plot_path)
    if use_wandb:
        wandb.log({"pr_curve": wandb.Image(pr_plot_path)})
    plt.close()
    
    # Create and log confusion matrix plots if using wandb
    if use_wandb:
        for phase in phases:
            cm = final_metrics[phase]['confusion_matrix']
            if cm.shape == (2, 2):
                ...
                plt.tight_layout()
                cm_plot_path = os.path.join(save_dir, f'{phase}_confusion_matrix.png')
                plt.savefig(cm_plot_path)
                wandb.log({f"{phase}_confusion_matrix": wandb.Image(cm_plot_path)})
                plt.close()
...
def main():
    ...
    if use_wandb:
        ...
        # Convert hidden_sizes list to string for wandb config
        hidden_sizes_str = ','.join(map(str, args.hidden_sizes)) if args.hidden_sizes else ''
        ...
        wandb_config = {
            ...
            'n_components': args.n_components if args.use_pca else None,
            'sampling': args.sampling,
            'sampling_ratio': args.sampling_ratio,
            'classification_threshold': args.threshold,
            'disease': args.label_col,
    ...
    model, history, final_metrics, all_preds, all_labels, completed_epochs = train_model(
        ...
        classification_threshold=args.threshold,
        use_wandb=use_wandb  # Pass the wandb flag
    )

    # Plot metrics
    plot_all_metrics(history, final_metrics, experiment_dir, use_wandb=use_wandb)
    ...
    if use_wandb:
        ...
        # Log final metrics as summary statistics
        for phase in ['train', 'test']:
            for metric in ['acc', 'auc', 'f1', 'pr_auc']:
                metric_name = f"{phase}_{metric}"
                wandb.run.summary[metric_name] = history[metric_name][-1]
                
            # Log sensitivity and specificity
            wandb.run.summary[f"{phase}_sensitivity"] = float(final_metrics[phase]['sensitivity'])
            wandb.run.summary[f"{phase}_specificity"] = float(final_metrics[phase]['specificity'])
        
        # Log hyperparameters as summary
        wandb.run.summary["completed_epochs"] = completed_epochs
        wandb.run.summary["final_lr"] = optimizer.param_groups[0]["lr"]
