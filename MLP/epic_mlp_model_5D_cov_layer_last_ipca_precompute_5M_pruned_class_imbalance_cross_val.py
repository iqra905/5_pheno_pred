...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/5d_exp_5M_pruned', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/pros_can_stats_info_0.01', help="Directory containing genotype files")
    ...
    parser.add_argument("-stratified", type=int, default=1, choices=[0, 1], help="Whether to use stratified cross-validation (0: no, 1: yes)")
...
class SNPsOnlyModel(nn.Module):
    ...
    def forward(self, x):
        # If data is in format [batch_size, n_snps, 3]
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', 
                ...
                classification_threshold=0.5, fold=None):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            print(f'{fold_str}{phase} Loss: {epoch_loss:.4f} - Acc: {epoch_acc:.4f}{auc_str}{f1_str}{pr_auc_str}')
...
def plot_all_metrics(history, final_metrics, save_dir, fold=None):
    phases = ['train', 'test']
    ...
    fig.suptitle(f'Model Performance Metrics{" - Fold " + str(fold) if fold is not None else ""}', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'metrics_plot{fold_str}.png'))
    plt.close()
    ...
    plt.title(f'ROC Curve{" - Fold " + str(fold) if fold is not None else ""}')
    plt.legend(loc='lower right')
    plt.savefig(os.path.join(save_dir, f'roc_curve{fold_str}.png'))
    plt.close()
    ...
    plt.title(f'Precision-Recall Curve{" - Fold " + str(fold) if fold is not None else ""}')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, f'pr_curve{fold_str}.png'))
    plt.close()

def write_fold_results(fold, hyperparameters, results, save_dir):
    fold_file = os.path.join(save_dir, f'fold_{fold}_results.txt')
...
def aggregate_cross_validation_results(all_fold_results, save_dir, hyperparameters):
    """
    Aggregate results from all folds and compute mean and std
    """
    # Metrics to aggregate
    metrics = ['train_acc', 'train_auc', 'train_f1', 'train_pr_auc', 
               'test_acc', 'test_auc', 'test_f1', 'test_pr_auc', 
               'train_sens', 'train_spec', 'test_sens', 'test_spec']
    
    # Initialize aggregated results
    agg_results = {metric: [] for metric in metrics}
    ...
    for fold_results in all_fold_results:
        for metric in metrics:
            if metric in fold_results:
                try:
                    agg_results[metric].append(float(fold_results[metric]))
                except ValueError:
    ...
    for metric in metrics:
        ...
        if values:
            ...
            final_results[f'{metric}_mean'] = f"{mean_val:.5f}"
            final_results[f'{metric}_std'] = f"{std_val:.5f}"
    ...
    with open(os.path.join(save_dir, 'cv_results.txt'), 'w') as f:
        ...
        f.write("\nAccuracy:\n")
        f.write(f"  Train: {final_results.get('train_acc_mean', 'N/A')} ± {final_results.get('train_acc_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_acc_mean', 'N/A')} ± {final_results.get('test_acc_std', 'N/A')}\n")
        
        f.write("\nAUC:\n")
        f.write(f"  Train: {final_results.get('train_auc_mean', 'N/A')} ± {final_results.get('train_auc_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_auc_mean', 'N/A')} ± {final_results.get('test_auc_std', 'N/A')}\n")
        
        f.write("\nF1 Score:\n")
        f.write(f"  Train: {final_results.get('train_f1_mean', 'N/A')} ± {final_results.get('train_f1_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_f1_mean', 'N/A')} ± {final_results.get('test_f1_std', 'N/A')}\n")
        
        f.write("\nPR-AUC:\n")
        f.write(f"  Train: {final_results.get('train_pr_auc_mean', 'N/A')} ± {final_results.get('train_pr_auc_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_pr_auc_mean', 'N/A')} ± {final_results.get('test_pr_auc_std', 'N/A')}\n")
        
        f.write("\nSensitivity:\n")
        f.write(f"  Train: {final_results.get('train_sens_mean', 'N/A')} ± {final_results.get('train_sens_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_sens_mean', 'N/A')} ± {final_results.get('test_sens_std', 'N/A')}\n")
        
        f.write("\nSpecificity:\n")
        f.write(f"  Train: {final_results.get('train_spec_mean', 'N/A')} ± {final_results.get('train_spec_std', 'N/A')}\n")
        f.write(f"  Test:  {final_results.get('test_spec_mean', 'N/A')} ± {final_results.get('test_spec_std', 'N/A')}\n")
    
    # Also save as CSV
    with open(os.path.join(save_dir, 'cv_results.csv'), 'w', newline='') as csvfile:
        fieldnames = list(hyperparameters.keys()) + list(final_results.keys())
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        writer.writeheader()
        combined_dict = {**hyperparameters, **final_results}
...
def main():
    ...
    # Determine input size based on the first file
    orig_input_size = get_input_size(first_genotype_file)
    print(f"Dynamically determined input size: {orig_input_size}")
    ...
    # Create directory for cross-validation results
    #cv_dir = os.path.join(experiment_dir, 'cross_validation')
    cv_dir = experiment_dir
    ...
    for file_path in filtered_file_list:
        sample_id = int(file_path.split('sample_')[1].split('.gen.gz')[0])
        if sample_id in phenotype_samples:
            label = phenotype_data.loc[phenotype_data['new_order'] == sample_id, args.label_col].values[0]
            sample_to_label[file_path] = int(label)
    
    # Prepare for cross-validation
    if args.stratified:
        print(f"Using stratified {args.n_folds}-fold cross-validation")
        kf = StratifiedKFold(n_splits=args.n_folds, shuffle=True, random_state=args.random_seed)
        splits = kf.split(filtered_file_list, [sample_to_label[f] for f in filtered_file_list])
    else:
        print(f"Using standard {args.n_folds}-fold cross-validation")
        kf = KFold(n_splits=args.n_folds, shuffle=True, random_state=args.random_seed)
        splits = kf.split(filtered_file_list)
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        print(f"\n{'='*80}\nProcessing Fold {fold+1}/{args.n_folds}\n{'='*80}")
        
        # Split files for this fold
        train_files = [filtered_file_list[i] for i in train_idx]
        test_files = [filtered_file_list[i] for i in test_idx]
        
        print(f"Fold {fold+1} split: Train {len(train_files)}, Test {len(test_files)}")
        ...
        print("\nSampling from datasets for this fold:")
        if covariates_only:
            for dataset_name, dataset in [("Train", train_dataset), ("Test", test_dataset)]:
                print(f"\n{dataset_name} dataset sample:")
                for i in range(min(3, len(dataset))):  # Print info for 3 items or less from each dataset
        ...
        train_labels = []
        for i in range(len(train_dataset)):  # Use a subset to estimate class weights
            if covariates_only:
                _, label = train_dataset[i]
            elif snps_only:
                _, label = train_dataset[i]
        ...
        print(f"Class distribution in sampled data for fold {fold+1}: Positive: {n_positive} ({pos_ratio:.2%}), Negative: {n_negative} ({neg_ratio:.2%})")
        ...
        print(f"DataLoaders created for fold {fold+1}")
        ...
        with open(os.path.join(fold_dir, 'model_architecture.txt'), 'w') as file:
            file.write(str(model))
            print(model)

        print(f"Model created and moved to device for fold {fold+1}")
        ...
        model, history, final_metrics, all_preds, all_labels, completed_epochs = train_model(
            ...
            device=device, save_dir=fold_dir, early_stopping=early_stopping,
            covariates_only=covariates_only, snps_only=snps_only,
            classification_threshold=args.threshold, fold=fold+1
        )

        # Plot metrics for this fold
        plot_all_metrics(history, final_metrics, fold_dir, fold+1)

        # Save the trained model for this fold
        torch.save(model.state_dict(), os.path.join(fold_dir, f'model_fold_{fold+1}.pth'))
        print(f"Model for fold {fold+1} trained and saved")

        # Update results dictionary for this fold
        fold_results = {
            'fold': fold+1,
    ...
    hyperparameters = {
        ...
        'focal_alpha': args.focal_alpha if args.loss_fn == "focal" else None,
        'focal_gamma': args.focal_gamma if args.loss_fn == "focal" else None
    }
    
    final_results = aggregate_cross_validation_results(all_fold_results, cv_dir, hyperparameters)
    
    # Create visualizations for aggregate results
    plot_aggregate_results(all_fold_results, cv_dir)
    
    print(f"\nCross-validation complete. Results saved to {cv_dir}")
    
    # Print summary of cross-validation results
    print("\nCross-Validation Summary:")
    print(f"Mean Test ACC: {final_results['test_acc_mean']} ± {final_results['test_acc_std']}")
    print(f"Mean Test AUC: {final_results['test_auc_mean']} ± {final_results['test_auc_std']}")
    print(f"Mean Test F1: {final_results['test_f1_mean']} ± {final_results['test_f1_std']}")
    print(f"Mean Test PR-AUC: {final_results['test_pr_auc_mean']} ± {final_results['test_pr_auc_std']}")
    print(f"Mean Test Sensitivity: {final_results['test_sens_mean']} ± {final_results['test_sens_std']}")
    print(f"Mean Test Specificity: {final_results['test_spec_mean']} ± {final_results['test_spec_std']}")

def plot_aggregate_results(all_fold_results, save_dir):
    """
    Generate plots showing performance across all folds
    """
    # Extract metrics across folds
    folds = [r['fold'] for r in all_fold_results]
    train_acc = [float(r['train_acc']) for r in all_fold_results]
    test_acc = [float(r['test_acc']) for r in all_fold_results]
    train_auc = [float(r['train_auc']) for r in all_fold_results]
    test_auc = [float(r['test_auc']) for r in all_fold_results]
    train_f1 = [float(r['train_f1']) for r in all_fold_results]
    test_f1 = [float(r['test_f1']) for r in all_fold_results]
    train_pr_auc = [float(r['train_pr_auc']) for r in all_fold_results]
    test_pr_auc = [float(r['test_pr_auc']) for r in all_fold_results]
    
    # Create a 2x2 grid of plots
    fig, axs = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Performance Across Folds', fontsize=16)
    
    # Accuracy plot
    axs[0, 0].plot(folds, train_acc, 'o-', label='Train')
    axs[0, 0].plot(folds, test_acc, 'o-', label='Test')
    axs[0, 0].set_title('Accuracy')
    axs[0, 0].set_xlabel('Fold')
    axs[0, 0].set_ylabel('Accuracy')
    axs[0, 0].set_xticks(folds)
    axs[0, 0].legend()
    axs[0, 0].grid(True)
    
    # AUC plot
    axs[0, 1].plot(folds, train_auc, 'o-', label='Train')
    axs[0, 1].plot(folds, test_auc, 'o-', label='Test')
    axs[0, 1].set_title('AUC-ROC')
    axs[0, 1].set_xlabel('Fold')
    axs[0, 1].set_ylabel('AUC')
    axs[0, 1].set_xticks(folds)
    axs[0, 1].legend()
    axs[0, 1].grid(True)
    
    # F1 plot
    axs[1, 0].plot(folds, train_f1, 'o-', label='Train')
    axs[1, 0].plot(folds, test_f1, 'o-', label='Test')
    axs[1, 0].set_title('F1 Score')
    axs[1, 0].set_xlabel('Fold')
    axs[1, 0].set_ylabel('F1')
    axs[1, 0].set_xticks(folds)
    axs[1, 0].legend()
    axs[1, 0].grid(True)
    
    # PR-AUC plot
    axs[1, 1].plot(folds, train_pr_auc, 'o-', label='Train')
    axs[1, 1].plot(folds, test_pr_auc, 'o-', label='Test')
    axs[1, 1].set_title('PR-AUC')
    axs[1, 1].set_xlabel('Fold')
    axs[1, 1].set_ylabel('PR-AUC')
    axs[1, 1].set_xticks(folds)
    axs[1, 1].legend()
    axs[1, 1].grid(True)
    
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for title
    plt.savefig(os.path.join(save_dir, 'cross_validation_metrics.png'))
    plt.close()
    
    # Create boxplots for train vs test performance
    metrics = ['Accuracy', 'AUC', 'F1', 'PR-AUC']
    train_data = [train_acc, train_auc, train_f1, train_pr_auc]
    test_data = [test_acc, test_auc, test_f1, test_pr_auc]
    ...
    ax.boxplot(train_data, positions=positions-width/2, widths=width, patch_artist=True,
               boxprops=dict(facecolor='lightblue'), medianprops=dict(color='blue'),
               flierprops=dict(markeredgecolor='blue'), 
               labels=[''] * len(metrics))
    
    ax.boxplot(test_data, positions=positions+width/2, widths=width, patch_artist=True,
    ...
    for i, (train_vals, test_vals) in enumerate(zip(train_data, test_data)):
        # Jitter points horizontally for better visibility
        train_x = np.random.normal(positions[i]-width/2, 0.05, size=len(train_vals))
        test_x = np.random.normal(positions[i]+width/2, 0.05, size=len(test_vals))
        
        ax.scatter(train_x, train_vals, color='blue', alpha=0.6)
        ax.scatter(test_x, test_vals, color='green', alpha=0.6)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'cross_validation_boxplot.png'))
    plt.close()
