...
# Function to safely calculate AUC when NaN values might be present
def safe_roc_auc_score(y_true, y_pred):
    """Calculate ROC AUC score safely handling potential NaN values"""
    # Check for NaN values
    nan_mask = np.isnan(y_pred)
    if np.any(nan_mask):
        # Log warning about NaN values
        warnings.warn(f"Found {np.sum(nan_mask)} NaN values in predictions. Replacing with 0.5")
        
        # Create a copy and replace NaN values with 0.5 (neutral prediction)
        y_pred_clean = np.copy(y_pred)
        y_pred_clean[nan_mask] = 0.5
        
        return roc_auc_score(y_true, y_pred_clean)
    else:
...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/col/col_full/full_covariates/Last_layer/exp_cov/HS_2/', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/col_can', help="Directory containing genotype files")
    #parser.add_argument("-genotype_dir", type=str, default='/vol/research/ucdatasets/gwas/gwas_mono_rm/stat_test_exp_split/col/geno_ml_filtered_splits_random/split_80', help="Directory containing genotype files")
    ...
    parser.add_argument("-opt", type=str, default="sgd", choices=["adam", "adamw", "sgd"], help="Optimizer to use")
    ...
    parser.add_argument("-hidden_sizes", type=parse_int_list, default=[32], help="Hidden layer sizes for MLP")
    ...
    parser.add_argument("-model_type", type=str, default="covariates_only", choices=["full", "covariates_only"], help="Type of model to use (full: SNPs + covariates, covariates_only: only covariates)")
    
    # Add gradient clipping
    parser.add_argument("-clip_grad", type=float, default=0.5, help="Gradient clipping value (0 to disable)")
    
    # Reduced model complexity options
    parser.add_argument("-use_layernorm", type=int, default=1, choices=[0, 1], help="Use LayerNorm instead of BatchNorm (0: no, 1: yes)")
    parser.add_argument("-use_xavier_init", type=int, default=1, choices=[0, 1], help="Use Xavier/Glorot initialization (0: no, 1: yes)")
...
class CovariatesOnlyModel(nn.Module):
    def __init__(self, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        self.input_size = 0
        if use_covariates:
            self.input_size += num_covariates  # PCs
        if use_age:
            self.input_size += 1  # Age
        if use_gender:
            self.input_size += 1  # Gender

        if self.input_size == 0:
            raise ValueError("At least one type of covariate must be enabled")
            
        # For backward compatibility - IMPORTANT
        self.model = nn.Sequential(OrderedDict([
            ('linear1', nn.Linear(self.input_size, 32)),
            ('relu1', nn.ReLU()),
        ...
        # Simple model (same as self.model for compatibility)
        self.simple_model = self.model
        
        # Second option: standard model with normalizations
        layers = []
        current_size = self.input_size
        
        # Regular hidden layers with added safeguards
        for i in range(len(hidden_sizes)):
            layers.append((f'linear_{i}', nn.Linear(current_size, hidden_sizes[i])))
            
            # Add normalization if enabled
            layers.append((f'norm_{i}', nn.LayerNorm(hidden_sizes[i])))
            
            # Use a more stable activation function
        ...
        self.regular_model = nn.Sequential(OrderedDict(layers))
        
        # Initialize weights carefully for both models
        self._initialize_weights()
        
        print(f"CovariatesOnlyModel initialized (using PCs: {use_covariates}, age: {use_age}, gender: {use_gender})")
    
    def forward(self, x):
        # Pre-processing: checking for NaN values and normalizing inputs
        if torch.isnan(x).any():
            print(f"WARNING: NaN detected in input data! Replacing with zeros.")
            x = torch.nan_to_num(x, nan=0.0)
        
        # Apply batch normalization to input to stabilize
        if x.dim() == 1:
            x = x.unsqueeze(0)  # Add batch dimension if only a single sample
        
        # Check feature dimension matches expected input size
        ...
        try:
            ...
            # Proceed without normalization if it fails
        
        # Choose between simple and regular model based on stability
        try:
            # First try with the simple model
            out = self.simple_model(x)
            
            # Check if output has NaN values
            if torch.isnan(out).any():
                print("WARNING: Simple model produced NaN outputs, trying regular model")
                out = self.regular_model(x)
        except Exception as e:
            print(f"Error in simple model: {e}, trying regular model")
            out = self.regular_model(x)
    ...
    def _initialize_weights(self):
        """Initialize weights using Xavier/Glorot initialization with extra care"""
        # Initialize simple model
        for name, module in self.simple_model.named_modules():
            if isinstance(module, nn.Linear):
                # Use a smaller initialization range for more stability
                nn.init.xavier_uniform_(module.weight, gain=0.5)
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
                    
        # Initialize regular model
        for name, module in self.regular_model.named_modules():
            if isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight, gain=0.5)
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        for i in range(len(hidden_sizes)):
            layers.extend([
                ...
                (f'layernorm_{i}', nn.LayerNorm(hidden_sizes[i])),  # LayerNorm instead of BatchNorm
        ...
        self.final_processing = nn.Sequential(OrderedDict([
            ...
            ('final_layernorm', nn.LayerNorm(last_hidden_size)),  # LayerNorm instead of BatchNorm
    ...
    def _initialize_weights(self):
        ...
        for name, module in self.main_layers.named_modules():
            if isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight)
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
                    
        # Initialize final processing layers
        for name, module in self.final_processing.named_modules():
            if isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight)
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
...
def analyze_dataset(dataset, name="unnamed"):
    """Analyze a dataset to find potential issues with the data"""
    ...
    # Analyze 100 random samples or all if less than 100
    sample_size = min(100, len(dataset))
    indices = np.random.choice(len(dataset), sample_size, replace=False)
    
    for idx in indices:
        if dataset.covariates_only:
            covariates, label = dataset[idx]
            all_values.append(covariates.numpy())
        else:
            # Handle full model case
            genotype, covariates, label = dataset[idx]
            all_values.append(covariates.numpy())
        ...
        if dataset.covariates_only:
            nan_count += np.isnan(covariates.numpy()).sum()
            inf_count += np.isinf(covariates.numpy()).sum()
        else:
            nan_count += np.isnan(covariates.numpy()).sum()
            inf_count += np.isinf(covariates.numpy()).sum()
    ...
    # Basic statistics
    print(f"Data shape: {all_values.shape}")
    ...
    for i in range(all_values.shape[1]):
        feature_data = all_values[:, i]
        corr = np.corrcoef(feature_data, all_labels)[0, 1]
    ...
    return {
        ...
        "inf_count": inf_count,
        "feature_stats": {
            "min": np.min(all_values, axis=0),
            "max": np.max(all_values, axis=0),
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', 
               save_dir=None, early_stopping=None, covariates_only=False, clip_grad=0.5):
    ...
    print(f"Gradient clipping value: {clip_grad}")
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            for i in range(len(dataloaders[phase])):
                ...
                with autocast():
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        # Debug: Check for NaN values in logits
                        if torch.isnan(logits).any():
                            print(f"WARNING: NaN detected in logits! Phase: {phase}, Batch: {i}")
                            # Replace NaN values with 0 to allow training to continue
                ...
                # Debug: Check for NaN values in predictions
                if np.isnan(probs_cpu).any():
                    print(f"WARNING: NaN values detected in predictions! Phase: {phase}, Batch: {i}")
                    nan_count = np.isnan(probs_cpu).sum()
                    total_count = probs_cpu.size
                    print(f"NaN count: {nan_count}/{total_count} ({nan_count/total_count*100:.2f}%)")
                
                all_labels[phase].extend(labels_cpu)
                all_preds[phase].extend(probs_cpu)
            ...
            nan_count = np.isnan(np.array(all_preds[phase])).sum()
            if nan_count > 0:
                print(f"WARNING: Found {nan_count} NaN values in {phase} predictions before AUC calculation")
                print(f"Will use safe_roc_auc_score to handle NaNs")
                epoch_auc = safe_roc_auc_score(np.array(all_labels[phase]), np.array(all_preds[phase]))
            else:
...
def compute_final_metrics(all_labels, all_preds):
    ...
    for phase in ['train', 'test']:
        ...
        if np.isnan(y_pred_proba).any():
            print(f"WARNING: Found NaN values in {phase} predictions, replacing with 0.5")
        ...
        try:
            ...
            print(f"Error calculating ROC curve for {phase}: {e}")
...
def main():
    ...
    # Force more stable normalization methods if not specified
    if args.norm_age == "none":
        args.norm_age = "robust"  # More robust to outliers
    if args.norm_pcs == "none":
        args.norm_pcs = "robust"
    
    print(f"\nUsing normalizations: Age={args.norm_age}, PCs={args.norm_pcs}, Gender={args.norm_gender}")
    ...
    # Run diagnostic analysis on datasets to check for problems
    print("\n========== DATASET DIAGNOSTICS ==========")
    train_stats = analyze_dataset(train_dataset, "Training")
    test_stats = analyze_dataset(test_dataset, "Testing")
    
    # Check for significant differences between training and test distributions
    print("\n========== DISTRIBUTION COMPARISON ==========")
    for i in range(len(train_stats["feature_stats"]["mean"])):
        train_mean = train_stats["feature_stats"]["mean"][i]
        test_mean = test_stats["feature_stats"]["mean"][i]
        train_std = train_stats["feature_stats"]["std"][i]
        test_std = test_stats["feature_stats"]["std"][i]
        
        # Calculate normalized difference (in terms of standard deviations)
        if train_std > 0:
            diff = abs(train_mean - test_mean) / train_std
            print(f"Feature {i}: Train mean={train_mean:.4f}, Test mean={test_mean:.4f}, Diff={diff:.4f} stds")
            
            # Alert on significant distribution shifts
            if diff > 0.5:
                print(f"  WARNING: Feature {i} has a significant distribution shift between train and test!")
    
    # Alert if NaN or Inf values were found
    if train_stats["nan_count"] > 0 or test_stats["nan_count"] > 0:
        print(f"\nWARNING: Found {train_stats['nan_count']} NaN values in training and "
              f"{test_stats['nan_count']} NaN values in testing!")
        
    if train_stats["inf_count"] > 0 or test_stats["inf_count"] > 0:
        print(f"\nWARNING: Found {train_stats['inf_count']} Inf values in training and "
              f"{test_stats['inf_count']} Inf values in testing!")
    ...
    print(f"Using weight decay: {wd}")
    print(f"Using gradient clipping: {args.clip_grad}")
    
    # Force smaller learning rate for stability
    adjusted_lr = min(args.lr, 1e-5)
    if adjusted_lr != args.lr:
        print(f"WARNING: Reducing learning rate from {args.lr} to {adjusted_lr} for stability")
        args.lr = adjusted_lr
    ...
    if opt == "adamw":
        # Use AMSGrad variant of AdamW which has improved stability
        ...
        print("Using AdamW with AMSGrad for improved stability")
    elif opt == "sgd":
        # Use SGD with momentum and nesterov for better performance
        optimizer = optim.SGD(
            model.parameters(),
            lr=args.lr,
            momentum=0.9,
        ...
        print("Using SGD with momentum and nesterov acceleration")
    else:
        # Default optimizers
        optimizer = {
            ...
            "rmsprop": optim.RMSprop(model.parameters(), lr=args.lr, momentum=0.9),
            "sgd": optim.SGD(model.parameters(), lr=args.lr),
            "adam": optim.Adam(model.parameters(), lr=args.lr, weight_decay=wd, eps=1e-8)
        }.get(opt, None)
