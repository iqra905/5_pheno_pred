...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="Exp_only_snps_no_covs_patience_15_pros_l1_0.0001", help="ID of the experiment")
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10, snp_l1_lambda=0.01):
        super(GenotypeModel, self).__init__()
        self.use_covariates = use_covariates
        self.use_age = use_age
        self.use_gender = use_gender
        self.snp_l1_lambda = snp_l1_lambda  # L1 regularization strength
    ...
    def forward(self, x, covariates=None):
        ...
        # Calculate L1 regularization for SNP weights
        self.snp_l1_reg = self.snp_l1_lambda * torch.norm(self.pointwise_conv.weight, p=1)
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', save_dir=None, early_stopping=None, covariates_only=False):
    ...
    history = {
        ...
        'learning_rates': [],
        'l1_reg_loss': []  # New field to track L1 regularization loss
    }
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            running_corrects = 0
            total_samples = 0
            epoch_l1_loss = 0.0  # Track L1 loss for the epoch
            ...
            for i in range(len(dataloaders[phase])):
                ...
                with autocast():
                    with torch.set_grad_enabled(phase == 'train'):
                        if covariates_only:
                            ...
                            # Add L1 regularization loss if available
                            total_loss = criterion(logits, labels)
                            if hasattr(model, 'snp_l1_reg'):
                                total_loss += model.snp_l1_reg
                                epoch_l1_loss += model.snp_l1_reg.item()
                ...
                running_loss += total_loss.item() * (covariates.size(0) if covariates_only else inputs.size(0))
            ...
            # Calculate average L1 loss for the epoch
            avg_l1_loss = epoch_l1_loss / len(dataloaders[phase])
            if phase == 'train':
                history['l1_reg_loss'].append(avg_l1_loss)

            print(f'{phase} Loss: {epoch_loss:.4f} - Acc: {epoch_acc:.4f} - AUC: {epoch_auc:.4f}')
            if not covariates_only and phase == 'train':
                print(f'L1 Reg Loss: {avg_l1_loss:.4f}')
