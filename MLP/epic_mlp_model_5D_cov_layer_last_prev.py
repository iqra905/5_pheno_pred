...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', save_dir=None, early_stopping=None, covariates_only=False):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            # Add right before the problematic line in train_model():
            print(f"Phase: {phase}")
            print(f"Contains NaN in predictions: {np.isnan(all_preds[phase]).any()}")
            print(f"Contains NaN in labels: {np.isnan(all_labels[phase]).any()}")
            if np.isnan(all_preds[phase]).any():
                nan_indices = np.where(np.isnan(all_preds[phase]))[0]
