...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/pros/pros_full/full_covariates/Separate_layers/', help="Directory to save experiment results")
...
class GenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, label_column, use_covariates=True, use_age=True, use_gender=True, norm_age="standard", norm_pcs="standard", norm_gender="none", fit_normalizers=True, normalizers=None):
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        # Pointwise convolution layer for SNPs
        self.pointwise_conv = nn.Conv1d(in_channels=3, out_channels=1, kernel_size=1)
        
        # SNP processing layers
        self.snp_layers = nn.ModuleList()
        current_snp_size = input_size
        
        for hidden_size in hidden_sizes:
            self.snp_layers.append(nn.Sequential(
                nn.Linear(current_snp_size, hidden_size),
                nn.BatchNorm1d(hidden_size),
                self.get_activation(act),
                nn.Dropout(dropout_rate)
            ))
            current_snp_size = hidden_size
            
        # Covariate processing layers (if using covariates)
        if self.total_covariates > 0:
            self.covariate_layers = nn.ModuleList()
            current_cov_size = self.total_covariates
            
            # Create same number of layers as SNP pathway
            for _ in range(len(hidden_sizes)):
                self.covariate_layers.append(nn.Sequential(
                    nn.Linear(current_cov_size, self.total_covariates),  # Keep same size as number of covariates
                    #nn.BatchNorm1d(self.total_covariates),
                    #self.get_activation(act),
                    #nn.Dropout(dropout_rate)
                ))
                current_cov_size = self.total_covariates
        ...
        self.final_linear = nn.Linear(
            current_snp_size + (self.total_covariates if self.total_covariates > 0 else 0),
            current_snp_size
        )
        self.final_bn = nn.BatchNorm1d(current_snp_size)
        self.final_activation = self.get_activation(act)
        self.final_dropout = nn.Dropout(dropout_rate)
        
        # Output layer
        self.output_layer = nn.Linear(current_snp_size, 1)
        #self.output_activation = nn.Sigmoid()
        
        print(f"GenotypeModel initialized with parallel processing (using covariates: {use_covariates}, age: {use_age}, gender: {use_gender})")
        
    def forward(self, x, covariates=None):
        # Process SNPs
        x = self.pointwise_conv(x).squeeze(1)
        
        # Pass through SNP layers
        for layer in self.snp_layers:
            x = layer(x)
        ...
        if covariates is not None and self.total_covariates > 0:
            ...
            # Concatenate processed SNPs and covariates
    ...
    def forward_lrp(self, x, covariates=None):
        ...
        # Process through SNP layers without convolution
        for layer in self.snp_layers:
            x = layer(x)
...
def main():
    ...
    fit_normalizers=True # True for training set
    )
    ...
    test_dataset = GenotypeDataset(
        ...
        normalizers=fitted_normalizers  # Pass the fitted normalizers
    )
