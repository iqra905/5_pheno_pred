...
def parse_args():
    ...
    parser.add_argument("-wd", type=float, default=0.1, help="Weight decay for optimizer")
    parser.add_argument("-df", type=float, default=0.1, help="Decay factor for custom schedulers")

    parser.add_argument("-hidden_sizes", type=parse_int_list, default=[64,32], help="Hidden layer sizes for MLP")
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        self.final_processing = nn.Sequential(OrderedDict([
            ...
            #('final_bn', nn.BatchNorm1d(last_hidden_size, momentum=0.1, eps=1e-5)),
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', 
               save_dir=None, early_stopping=None, covariates_only=False, clip_grad=1.0):
