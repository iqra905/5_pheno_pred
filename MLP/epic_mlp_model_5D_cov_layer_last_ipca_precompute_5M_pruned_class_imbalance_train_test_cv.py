...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/5d_exp_5M_stratified_kfold/t2d', help="Directory to save experiment results")
    parser.add_argument("-genotype_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/data_new_study_stratified_kfold/sampled_data_5M_disease_wise_pruned/t2d/fold_5', help="Directory containing genotype files")
    parser.add_argument("-phenotype_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/data_files/disease_pheno_updated/t2d.xlsx', help="Path to phenotype file")
    ...
    parser.add_argument("-train_indices", type=str, default='/vol/research/fmodal_mmmed/Codes/stat_analysis_lr/results_new_study_stratified_kfold/t2d/fold_5/train_indices.npy', help="Path to .npy file containing indices for training set")
    parser.add_argument("-test_indices", type=str, default='/vol/research/fmodal_mmmed/Codes/stat_analysis_lr/results_new_study_stratified_kfold/t2d/fold_5/test_indices.npy', help="Path to .npy file containing indices for test set")
...
# def load_split_indices(args, phenotype_data, filtered_file_list):
#     """Load train/test split indices and return corresponding files"""
#     if not args.train_indices or not args.test_indices:
#         return None, None
...
#         print(f"Loaded split indices - Train: {len(train_indices)}, Test: {len(test_indices)}")
        
#         # Get sample IDs from phenotype data using these indices
#         # Assuming 'new_order' is the column with sample IDs in your phenotype data
#         train_sample_ids = phenotype_data.iloc[train_indices]['new_order'].values
#         test_sample_ids = phenotype_data.iloc[test_indices]['new_order'].values
        
#         # Create mapping from sample IDs to files
#         sample_id_to_file = {}
#         for file_path in filtered_file_list:
#             sample_id = int(file_path.split('sample_')[1].split('.gen.gz')[0])
#             sample_id_to_file[sample_id] = file_path
        
#         # Get files for training and testing
#         train_files = [sample_id_to_file[sample_id] for sample_id in train_sample_ids 
#                       if sample_id in sample_id_to_file]
#         test_files = [sample_id_to_file[sample_id] for sample_id in test_sample_ids 
#                      if sample_id in sample_id_to_file]
        
#         print(f"Final split - Train: {len(train_files)}, Test: {len(test_files)} files")
...
#     """Load train/test split indices and return corresponding files"""
#     if not args.train_indices or not args.test_indices:
#         print("Train or test indices file paths not provided")
#         return None, None
...
#         if max(np.max(train_indices), np.max(test_indices)) >= len(phenotype_data):
#             print(f"ERROR: Indices out of range. Max index: {max(np.max(train_indices), np.max(test_indices))}, "
#                   f"Phenotype rows: {len(phenotype_data)}")
#             return None, None
...
#         # Sort the sample IDs for easier reading
#         train_sample_ids = np.sort(train_sample_ids)
#         test_sample_ids = np.sort(test_sample_ids)
        
#         print(f"Extracted sample IDs - Train: {len(train_sample_ids)}, Test: {len(test_sample_ids)}")
#         print(f"Sample ID examples - Train: {train_sample_ids[:10]}..., Test: {test_sample_ids[:10]}...")
...
#         print(f"Found {len(sample_id_to_file)} unique sample IDs in genotype files")
...
#         missing_train = set(train_sample_ids) - set(sample_id_to_file.keys())
#         missing_test = set(test_sample_ids) - set(sample_id_to_file.keys())
        
#         if missing_train:
#             print(f"WARNING: {len(missing_train)} train samples don't have genotype files")
#             print(f"Missing examples: {list(missing_train)[:10]}...")
        
#         if missing_test:
#             print(f"WARNING: {len(missing_test)} test samples don't have genotype files")
#             print(f"Missing examples: {list(missing_test)[:10]}...")
        
#         print(f"Final split - Train: {len(train_files)}, Test: {len(test_files)} files")
        
#         # Verify stratification is maintained
#         if args.label_col in phenotype_data.columns:
#             train_labels = [
#                 phenotype_data.loc[phenotype_data['new_order'] == int(f.split('sample_')[1].split('.gen.gz')[0]), 
#                                    args.label_col].values[0]
...
#             print(f"Label distribution - Train: {train_pos}/{len(train_labels)} positive ({train_pos/len(train_labels)*100:.2f}%)")
#             print(f"Label distribution - Test: {test_pos}/{len(test_labels)} positive ({test_pos/len(test_labels)*100:.2f}%)")
...
def load_split_indices(args, phenotype_data, filtered_file_list):
    ...
    try:
        ...
        test_indices = np.load(args.test_indices)
        print(f"Loaded split indices - Train: {len(train_indices)}, Test: {len(test_indices)}")
        print(f"First few indices from file - Train: {train_indices[:5]}, Test: {test_indices[:5]}")
        
        # Direct file lookup by position approach
        # This works if indices are row positions in the phenotype file
        # Sort the indices to ensure we get consistent results
        train_files = [filtered_file_list[i] for i in sorted(train_indices) if i < len(filtered_file_list)]
        test_files = [filtered_file_list[i] for i in sorted(test_indices) if i < len(filtered_file_list)]
        
        # Get the sample IDs from the selected files
        train_sample_ids = [int(f.split('sample_')[1].split('.gen.gz')[0]) for f in train_files]
        test_sample_ids = [int(f.split('sample_')[1].split('.gen.gz')[0]) for f in test_files]
        
        print(f"Files selected - Train: {len(train_files)}, Test: {len(test_files)}")
        print(f"Sample ID examples (actual values from files) - Train: {sorted(train_sample_ids)[:10]}..., Test: {sorted(test_sample_ids)[:10]}...")
        
        # Verify stratification is maintained
        if args.label_col in phenotype_data.columns:
            train_labels = []
            for sample_id in train_sample_ids:
                # Find the label for this sample ID
                label_rows = phenotype_data.loc[phenotype_data['new_order'] == sample_id, args.label_col]
                if not label_rows.empty:
                    train_labels.append(label_rows.values[0])
            ...
            for sample_id in test_sample_ids:
                ...
                label_rows = phenotype_data.loc[phenotype_data['new_order'] == sample_id, args.label_col]
                if not label_rows.empty:
                    test_labels.append(label_rows.values[0])
...
def main():
    ...
    if len(filtered_file_list) != len(phenotype_samples):
     ...
     # Select training and testing files based on loaded indices
    train_files, test_files = load_split_indices(args, phenotype_data, filtered_file_list)
