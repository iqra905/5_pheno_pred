...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/exp_residuals/results/pros/chr_wise/', help="Directory to save experiment results")
    ...
    parser.add_argument("-chr_file", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/meta_data/first_5_columns_pros_can_pruned.gen.gz', help="Path to phenotype file")
    ...
    parser.add_argument("-label_col", type=str, default="pros01_res", help="Column name in phenotype file to use as label (e.g., 'pan01_res', etc.)")
...
class GenotypeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return chromosome_data, label_tensor
...
class GenotypeModel(nn.Module):
    def __init__(self, chromosome_sizes, hidden_sizes, dropout_rate, act):
        super().__init__()
        ...
        self.final_layers = nn.Sequential(OrderedDict([
            ('linear1', nn.Linear(total_chrom_output, hidden_sizes[-1])),
    ...
    def forward(self, chromosome_data):
        chrom_outputs = []
    ...
    def forward_lrp(self, chromosome_data):
        chrom_outputs = []
        for chrom, data in chromosome_data.items():
            # Apply pointwise convolution manually
            #data = self.chromosome_mlps[chrom].pointwise_conv(data).squeeze(1)
...
def custom_lrp(model, chromosome_data, target=1, epsilon=1e-9):
    model.eval()  
...
def perform_lrp_analysis(model, chromosome_data, target=1):
    snp_importance = custom_lrp(model, chromosome_data, target)
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            for chromosome_data, labels in dataloaders[phase]:
                # Move data to device
...
def main():
    ...
    train_dataset = GenotypeDataset(train_files, phenotype_data, chromosome_info, label_column=args.label_col)
    val_dataset = GenotypeDataset(val_files, phenotype_data, chromosome_info, label_column=args.label_col)
    test_dataset = GenotypeDataset(test_files, phenotype_data, chromosome_info, label_column=args.label_col)
    ...
    for dataset_name, dataset in [("Train", train_dataset), ("Validation", val_dataset), ("Test", test_dataset)]:
        ...
        for i in range(1):  # Print info for 3 items from each dataset
            chromosome_data, label = dataset[i]
            print(f"Item {i}:")
    ...
    chromosome_data, _ = test_batch
    chromosome_data = {k: v.to(device) for k, v in chromosome_data.items()}

    snp_importance = perform_lrp_analysis(model, chromosome_data)
    top_snps = get_top_snps(snp_importance, chromosome_info)
