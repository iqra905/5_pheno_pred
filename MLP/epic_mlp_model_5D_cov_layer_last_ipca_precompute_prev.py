...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/t2d/t2d_pca', help="Directory to save experiment results")
    ...
    parser.add_argument("-hidden_sizes", type=parse_int_list, default=[128,64,32], help="Hidden layer sizes for MLP")
    ...
    parser.add_argument("-label_col", type=str, default="t2dm", help="Column name in phenotype file to use as label (e.g., 't2d01', etc.)")
    ...
    parser.add_argument("-use_pca", type=int, default=1, choices=[0, 1], help="Whether to apply PCA to SNP data (0: no, 1: yes)")
    parser.add_argument("-n_components", type=int, default=100, help="Number of PCA components to keep")
    parser.add_argument("-pca_whiten", type=int, default=1, choices=[0, 1], help="Whether to whiten the PCA components (0: no, 1: yes)")
...
class SNPPCATransformer:
    """Class to apply PCA transformation to SNP data"""
    ...
    def fit(self, X):
        ...
        if not self.fitted:
            print(f"Examining SNP data shape: {X.shape}")
            
            # PCA components cannot exceed min(n_samples, n_features)
            max_possible_components = min(X.shape[0], X.shape[1]) - 1
            
            if self.n_components > max_possible_components:
                print(f"WARNING: Requested components ({self.n_components}) exceeds maximum possible ({max_possible_components})")
                print(f"Adjusting to use {max_possible_components} components instead")
                self.n_components = max_possible_components
            
            # Initialize PCA with proper number of components and solver
            # For smaller datasets, use 'full' SVD solver, for larger ones use 'randomized'
            svd_solver = 'randomized' if X.shape[1] > 500 else 'full'
            
            print(f"Fitting PCA with {self.n_components} components (whiten={self.whiten}, solver={svd_solver})...")
            self.pca = PCA(n_components=self.n_components, whiten=self.whiten, svd_solver=svd_solver)
            self.pca.fit(X)
            ...
            if len(self.pca.explained_variance_ratio_) > 5:
                top5_variance = self.pca.explained_variance_ratio_[:5].sum() * 100
...
class SNPsOnlyModel(nn.Module):
    ...
    def forward(self, x):
        # Ensure input shape is [batch_size, features]
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        # Pointwise convolution layer for 3D input
    ...
    def forward(self, x, covariates=None):
        # Apply pointwise convolution if x has 3 dimensions (batch, channels, features)
...
class GenotypeDataset(Dataset):
    def __init__(self, file_list, phenotype_data, label_column, use_covariates=True, use_age=True, 
                 ...
                 snps_only=False, use_pca=False, pca_transformer=None, fit_pca=False,
                 pca_features_dir=None):
        ...
        self.fit_pca = fit_pca and self.use_pca
        ...
        # If we need to fit the PCA to the SNP data and we're not using pre-computed features, do it now
        if self.fit_pca and self.pca_transformer is not None and self.pca_features_dir is None:
            print("Preparing to fit PCA on SNP data...")
            self._fit_pca_to_snp_data()
        ...
        if not covariates_only:
            ...
            if use_pca:
                ...
                elif pca_transformer:
                    print(f"- PCA components: {pca_transformer.n_components}")
                    print(f"- PCA whitening: {pca_transformer.whiten}")
                    print(f"- Fitting PCA: {fit_pca}")
    ...
    def _fit_pca_to_snp_data(self):
        """Fit PCA transformer to SNP data"""
        print("Loading SNP data for PCA fitting...")
        print("NOTE: Using a subset of data for PCA fitting to manage memory constraints")
        
        # To save memory, we'll collect SNP data from a subset of files
        sample_size = min(100, len(self.file_list))  # Increased to 100 samples for better PCA results
        sampled_files = np.random.choice(self.file_list, sample_size, replace=False)
        
        print(f"Using {sample_size} samples for PCA fitting")
        
        # Each row should be a sample, and each column should be a feature (SNP)
        all_snp_data = []
        ...
        # Get the first file to examine data structure
        first_file = sampled_files[0]
        mmap_file = self.file_handles[first_file]
        mmap_file.seek(0)
        with gzip.GzipFile(fileobj=mmap_file) as f:
            first_data = pd.read_csv(f, sep=r'\s+', header=None)
        
        print(f"Raw SNP data shape: {first_data.shape}")
        
        # Load SNP data from sampled files
        for i, file_path in enumerate(sampled_files):
            if i % 10 == 0:
                print(f"Loading file {i+1}/{len(sampled_files)}...")
            
            mmap_file = self.file_handles[file_path]
            mmap_file.seek(0)
            with gzip.GzipFile(fileobj=mmap_file) as f:
            ...
            if sample_data.shape[1] == 3:  # Typical genotype matrix with 3 values per SNP (AA, AB, BB probabilities)
                print(f"Converting SNP data with shape {sample_data.shape} to dosage encoding...") if i == 0 else None
                
                # Convert to a dosage encoding (0*AA + 1*AB + 2*BB) for each SNP
                # This creates a single value per SNP that represents the expected number of minor alleles
                dosage = sample_data[:, 0] * 0 + sample_data[:, 1] * 1 + sample_data[:, 2] * 2
                
                # Add this sample with converted SNP values
                all_snp_data.append(dosage)
            else:
                # If it's already a single value per SNP, use as is
                all_snp_data.append(sample_data)
        
        # Convert to numpy array
        print("Stacking all processed SNP data...")
        stacked_data = np.vstack(all_snp_data)
        
        print(f"Final data shape for PCA: {stacked_data.shape}")
        
        # Fit PCA
        self.pca_transformer.fit(stacked_data)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_pca and self.pca_features_dir is not None:
            ...
            sample_id = str(sample_id_str).zfill(5)
            ...
            # Process genotype data for full or SNPs-only mode
            ...
            if self.use_pca and self.pca_transformer is not None and self.pca_transformer.fitted:
                data_values = data.values
                
                # Apply the same dosage encoding as during PCA fitting
                if data_values.shape[1] == 3:  # If data is in (SNPs, 3) format
                    # Convert to dosage encoding (0*AA + 1*AB + 2*BB)
                    dosage = data_values[:, 0] * 0 + data_values[:, 1] * 1 + data_values[:, 2] * 2
                    
                    # Reshape to a 2D array (1 sample, many features)
                    dosage = dosage.reshape(1, -1)
                    pca_features = self.pca_transformer.transform(dosage)
                else:
                    # If data is already properly formatted, use as is
                    pca_features = self.pca_transformer.transform(data_values)
        ...
        for phase in ['train', 'test']:
            ...
            for batch in dataloaders[phase]:
                if covariates_only:
                    ...
                    inputs, labels = batch
                    # Make sure the input is properly shaped for batch processing
                    if inputs.dim() == 2:  # If shape is [batch_size, features]
                        pass  # This is the correct shape
                    elif inputs.dim() == 3 and inputs.size(1) == 1:  # If shape is [batch_size, 1, features]
                        inputs = inputs.squeeze(1)  # Remove the middle dimension
...
def main():
    ...
    train_files, test_files = train_test_split(
        file_list, test_size=0.2, random_state=args.random_seed
    )
    ...
    # Initialize PCA transformer if using PCA
    pca_transformer = None
    input_size = orig_input_size
    
    # If using pre-computed PCA features, load the model to get component info
    if args.pca_features_dir is not None:
        ...
        if os.path.exists(pca_model_path):
            ...
            print(f"Warning: PCA model file not found at {pca_model_path}")
            # Try to determine components from a sample PCA file
            sample_files = glob.glob(os.path.join(args.pca_features_dir, "sample_*_pca.npy"))
            if not sample_files:
                raise ValueError(f"No PCA feature files found in {args.pca_features_dir}")
                
            sample_pca = np.load(sample_files[0])
            if sample_pca.ndim > 1:
                input_size = sample_pca.shape[1]
            else:
                input_size = len(sample_pca)
                
            print(f"Determined PCA components from sample file: {input_size}")
    elif args.use_pca and not covariates_only:
        # Only create PCA transformer if not using pre-computed features
        print("Initializing PCA transformer...")
        pca_transformer = SNPPCATransformer(n_components=args.n_components, whiten=bool(args.pca_whiten))
        input_size = args.n_components  # This will be adjusted during PCA fitting if needed
        print(f"Using PCA with {args.n_components} components. Initial input size: {input_size}")
    ...
    train_dataset = GenotypeDataset(
        ...
        fit_pca=(args.pca_features_dir is None),  # Only fit PCA if not using pre-computed features
    ...
    # Update the input size if PCA was used and number of components was adjusted
    if args.use_pca and pca_transformer is not None and pca_transformer.fitted:
        input_size = pca_transformer.n_components
        print(f"PCA adjusted number of components to {input_size}")

    # Create test dataset with the fitted normalizers and PCA transformer
    test_dataset = GenotypeDataset(
        ...
        snps_only=snps_only,
        use_pca=bool(args.use_pca),
        pca_transformer=pca_transformer,  # Use the fitted PCA transformer
        fit_pca=False,  # Don't re-fit PCA
        pca_features_dir=args.pca_features_dir
    )
    ...
    hyperparameters = {
        ...
        'use_pca': bool(args.use_pca),
        'pca_components': input_size if args.use_pca else None,
        'pca_whiten': bool(args.pca_whiten) if args.use_pca else None,
        'pca_features_dir': args.pca_features_dir,
