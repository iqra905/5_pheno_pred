...
def parse_args():
    ...
    parser.add_argument("-ID", type=str, default="797", help="ID of the experiment")
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/MLP/results/pros/pros_pca', help="Directory to save experiment results")
    ...
    parser.add_argument("-final_lr", type=float, default=1e-4, help="Final learning rate for custom schedulers")
    parser.add_argument("-wd", type=float, default=0.7, help="Weight decay for optimizer")
    parser.add_argument("-df", type=float, default=0.1, help="Decay factor for custom schedulers")

    parser.add_argument("-hidden_sizes", type=parse_int_list, default=[64,16], help="Hidden layer sizes for MLP")
    ...
    parser.add_argument("-use_pca", type=int, default=1, choices=[0, 1], help="Whether to use pre-computed PCA features (0: no, 1: yes)")
    parser.add_argument("-n_components", type=int, default=200, help="Number of PCA components to use")
    parser.add_argument("-pca_features_dir", type=str, default='/vol/vssp/SF_ucdatasets/gwas/gwas_mono_rm/disease_wise_sampled_data/pros_can_pca', help="Directory containing pre-computed PCA features")
    ...
    parser.add_argument("-clip_grad", type=float, default=0, help="Gradient clipping value (0 to disable)")
    ...
    def __init__(self, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10):
        ...
        for i in range(len(hidden_sizes)):
            layers.extend([
                ...
                (f'layernorm_{i}', nn.LayerNorm(hidden_sizes[i])),
...
class GenotypeModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout_rate, act, use_covariates=True, use_age=True, use_gender=True, num_covariates=10, use_pca=True):
        ...
        self.final_processing = nn.Sequential(OrderedDict([
            ...
            (f'final_ln', nn.LayerNorm(last_hidden_size)),
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', save_dir=None, early_stopping=None, covariates_only=False, snps_only=False, clip_grad=1.0):
...
def main():
    ...
    model, history, final_metrics, all_preds, all_labels, completed_epochs = train_model(
        ...
        covariates_only=covariates_only, snps_only=snps_only,clip_grad=args.clip_grad
    )
