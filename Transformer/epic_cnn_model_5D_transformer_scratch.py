...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/Transformer/results_scratch/pros/full/', help="Directory to save experiment results")
    ...
    parser.add_argument("-monitor", type=str, default="val_loss", choices=["val_loss", "val_auc"], help="Quantity to monitor for early stopping")
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, min_delta=0, mode='min', verbose=False):
        ...
        Args:
            ...
            mode (str): 'min' for loss, 'max' for accuracy/AUC.
    ...
    def __call__(self, score):
        if self.mode == 'min':
            score_diff = self.val_loss_min - score
        else:
            score_diff = score - self.val_loss_min

        if self.best_score is None:
            self.best_score = score
            self.val_loss_min = score
        elif score_diff < self.delta:
            self.counter += 1
            ...
            self.val_loss_min = score
            if self.verbose:
                print(f'Validation metric improved ({self.val_loss_min:.6f})')
            self.counter = 0
    ...
    #     if self.verbose:
    #         print(f'Validation metric improved ({self.val_loss_min:.6f} --> {score:.6f}).  Saving model ...')
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, device='cuda', early_stopping_params=None):
    ...
    if early_stopping_params is not None:
        early_stopper = EarlyStopping(
            ...
            mode='min' if early_stopping_params['monitor'] == 'val_loss' else 'max',
            verbose=True
        )
    ...
    for epoch in range(num_epochs):
        ...
        if early_stopper is not None:
            monitor_value = (history['val_loss'][-1] if early_stopping_params['monitor'] == 'val_loss' 
                           else history['val_auc'][-1])
            early_stopper(monitor_value)
            # early_stopper(monitor_value, model, 
            #              os.path.join(early_stopping_params['save_dir'], 'checkpoint.pt'))
    ...
    final_metrics = compute_final_metrics(all_labels, all_preds)
...
def compute_final_metrics(all_labels, all_preds):
    ...
    for phase in ['train', 'val', 'test']:
        ...
        final_metrics[phase] = {
            ...
            'sensitivity': f'{sensitivity:.4f}',  # True Positive Rate / Recall
            'specificity': f'{specificity:.4f}',  # True Negative Rate
            'accuracy': f'{accuracy:.4f}',
            #'ppv': f'{ppv:.4f}',        # Precision
            #'npv': f'{npv:.4f}',
            #'f1_score': f'{f1:.4f}',
...
def main():
    ...
    model, history, final_metrics, all_preds, all_labels = train_model(
        model, dataloaders, criterion, optimizer, scheduler, args.epochs, device, early_stopping_params=early_stopping_params
    )
