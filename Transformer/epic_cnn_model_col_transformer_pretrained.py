...
def parse_args():
    ...
    parser.add_argument("-exp_dir", type=str, default='/vol/research/fmodal_mmmed/Codes/5_disease_experiments/CNN/results/col/pretrained/1_BS_32', help="Directory to save experiment results")
    ...
    parser.add_argument("-pool", type=str, default="mean", help="Pooling type for transformer output")
...
class GenotypeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.phenotype_data.loc[self.phenotype_data['new_order'] == sample_id, 'crc'].values[0]
...
class GenotypeModelWithPretrainedViT(nn.Module):
    def __init__(self, input_size, kernel_sizes, stride, conv_channels, act, dropout_rate, num_covariates=10, layer_indices=None, pooling_type='cls', print_dimensions=False):
        super(GenotypeModelWithPretrainedViT, self).__init__()
        ...
        # Load pretrained ViT-Small configuration and model
        try:
        ...
        total_layers = len(vit_model.encoder.layer)
        print(f"Total number of layers in ViT-Small: {total_layers}")
        ...
        self.transformer = nn.ModuleList([
            vit_model.encoder.layer[i] for i in layer_indices
        ])
        ...
        # Linear layer to project conv output to ViT hidden size
    ...
    def forward(self, x, covariates):
        ...
        # Transpose the tensor to (batch_size, sequence_length, channels)

        x = x.transpose(1, 2)
        if self.print_dimensions and not self.has_printed_dimensions:
            print(f"After transpose: {x.shape}")

        # Project to ViT hidden size
        #x = self.proj(x)
...
def main():
    ...
    model = GenotypeModelWithPretrainedViT(
    ...
    pooling_type=args.pool,  # Choose from 'cls', 'last', 'mean', 'attention', 'flatten'
